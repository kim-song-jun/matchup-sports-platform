...
| 삼각 구도 (영상 04장) | **3/10** — `auth-welcome` · `chat-empty` · `matches-empty` 만 오브젝트 3개 |

삼각이 없는 7장 중 `sport-*` 4장은 76px 썸네일로도 쓰이므로 위 규칙대로 **둘이 맞다**.
큰 자리 전용인데 둘뿐인 것은 `auth-notice`(나침반+콘) · `journey-done`(트로피+코인) ·
`landing-hero`(스톱워치+콘) 셋이다 — 다시 만들 때 서브를 하나 더 넣는다.

측정 한계: 알파 채널 덩어리 수는 **겹친 오브젝트를 하나로 센다**. 개수는 육안으로 세고,
자동 측정은 면적비(깊이)에만 쓴다.
