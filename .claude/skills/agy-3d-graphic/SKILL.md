...
**다시 만들 자산은 없다** (2026-09-07 재확인). 삼각이 없는 6장은 전부 규칙대로다:

| 자산 | 오브젝트 | 렌더 크기 (실측) | 판정 |
|---|---|---|---|
| `sport-*` 4장 | 2 | 76~132px 썸네일 | 규칙대로 (작은 자리는 둘) |
| `auth-notice` | 2 (나침반+콘) | `.tm-auth-illustration` **160px** (≤767px 136px) | 규칙대로 — 176px 미만 |
| `journey-done` | 2 (트로피+코인) | `.tm-empty-illustration` **160px** (≤767px 136px) | 규칙대로 — 176px 미만 |

### 2026-09-06 감사가 틀렸던 두 가지

1. **`landing-hero` 는 원래 오브젝트 3개다.** 스톱워치 + 공 + 콘 — 매니페스트의 SUBJECT 가
   셋을 명시하고 렌더에도 셋이 있다. 공이 스톱워치 뒤에 절반쯤 가려져 있어 놓쳤다.
2. **`auth-notice` · `journey-done` 은 "큰 자리"가 아니다.** 둘 다 160px(작은 화면 136px)로
...
`.tm-empty-illustration` 의 `width` 를 읽어야 알 수 있다.
