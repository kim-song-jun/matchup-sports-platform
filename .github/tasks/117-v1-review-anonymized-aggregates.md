...
매치·팀매치 상호평가 리뷰(`V1PostEventReview`)를 개별 리뷰(누가 몇 점을 줬는지) 노출 없이, 종목별 × 기간별(전체/월 선택) **집계 수치**로만 대상자에게 보여주도록 재구성한다. 상대가 리뷰를 냈는지 모른 채 먼저 내가 리뷰를 내는 구조는 그대로 두되(현재도 그렇다), 서로 리뷰가 실제로 공개(집계 반영)되는 시점을 지연시켜 보복성 리뷰를 방지한다. 대회 후기는 2026-08-14 후속 결정에 따라 작성자 식별정보를 제거한 개별 별점·태그로 공개하며 같은 reveal gate를 적용한다. 팀 신뢰점수 계산에서 팀매치 리뷰와 대회후기 리뷰가 섞이는 부분은 분리한다.
...
- [ ] 대회 개인·팀 후기는 작성자 사용자·소속팀을 노출하지 않고 별점·태그만 익명 개별 항목으로 보여준다. 상호 제출 전 72시간 동안은 응답에서 제외한다.
...
- 2026-08-14 사용자 결정: 대회 후기도 받은 사람이 작성자를 알 수 없어야 한다. 개인매치·팀매치처럼 집계 전용으로 바꾸지는 않고, 익명 개별 별점·태그를 상호 제출 또는 72시간 뒤 공개한다.
