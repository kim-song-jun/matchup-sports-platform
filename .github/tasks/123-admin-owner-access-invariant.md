...
- [ ] 변경된 두 service spec이 직렬·최소 worker로 통과한다.
- [ ] backend typecheck/integration은 동일 commit GitHub CI에서 통과한다.
- [x] 독립 backend/security review에 blocker가 없다.
...
- 구현: admin/profile service의 active-linked-user, revoke-first, self-lockout, last-owner 직렬화 계약 반영.
...
- 미검증: service spec, backend typecheck/integration, GitHub CI.
