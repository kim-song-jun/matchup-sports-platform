...
### 근본 원인 확정 — `.tm-topbar`는 데스크톱에서 CSS로 항상 숨겨진다 (2026-08-05)

`shell.tsx`를 다시 읽고 `getComputedStyle`로 실측해 확정했다: `<header
className="tm-topbar">`(뒤로가기 `AppBackLink` + 제목을 담은 모바일 헤더)는
`desktopHead`와 무관하게 **항상 JSX에 렌더된다**. 하지만 `getComputedStyle(topbar).display`
가 데스크톱 뷰포트(1440)에서 실측 `"none"`이었다 — `.tm-topbar`를 데스크톱에서
숨기는 미디어쿼리가 CSS에 있다는 뜻이다. 그 자리를 대체하는 것이 `desktopHead &&
title` 조건부 블록(`.tm-desktop-page-head.tm-show-desktop`, 뒤로가기 `AppBackLink`
+ `<h1>`)인데, 이건 **옵션이라 기본값이 꺼져 있다**.

**결론(중요, 스코프 재평가 근거)**: `desktopHead`를 안 쓴 페이지는 예외 없이,
페이지 내용과 무관하게 **데스크톱에서 뒤로가기 버튼과 제목이 100% 사라진다** —
이건 페이지별 우연이 아니라 CSS 미디어쿼리로 강제되는 시스템 전역 규칙이다.
- **A그룹(목록/최상위 페이지)**: nav 탭 자체가 대체 네비게이션이라 실사용
  문제가 낮다 — 기존 판단 유지.
- **B/C그룹(하위 상세/개체 페이지, ~30개)**: tournament-detail·public-profile
  처럼 콘텐츠 자체에 대상 식별 정보가 있어도, **데스크톱에서 이전 화면으로
  돌아갈 명시적 UI 수단이 전혀 없다**(브라우저 뒤로가기나 상단 nav 탭에만
  의존해야 함) — 이건 "경미한 일관성 문제"가 아니라 **확정된 네비게이션
  결함**으로 재평가해야 한다. 이번 세션에서 고친 4건(visitor-03/owner-08
  ×2/public_records-02)과 동일한 근본 원인을 B/C그룹 전체가 공유한다.

**코드는 건드리지 않았다** — 30개 안팎 파일에 `desktopHead` 한 줄씩 추가하는
기계적 작업이라 위험은 낮지만, 여전히 이번 세션이 받은 지시 범위(18건 재검수)를
넘는 별도 스코프이며 규모가 커서 사용자 확인 후 진행하는 게 맞다고 판단했다.
