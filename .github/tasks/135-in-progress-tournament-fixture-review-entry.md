...
- [ ] `in_progress` 대회 상세는 `status=completed`이면서 `result != null`인 fixture만 리뷰 가능 경기로 표시한다.
- [ ] 각 행은 홈/원정 팀, 라운드, 점수를 보여주고 `/my/reviews/tournament_fixture/:fixtureId`로 직접 이동한다.
- [ ] `owner | manager` source는 상대팀+상대 등록 선수를, `member` source는 상대 등록 선수만 반환하며 member의 팀 대상 제출은 서버가 차단한다.
- [ ] 대회 전체 후기는 기존대로 전체 종료 뒤 참가 확정팀 `owner | manager`만 작성한다.
- [ ] 공개 상세의 CTA는 `GET /reviews?tab=pending&tournamentId=:id` 결과를 사용해 비로그인·비참가자·작성 완료·같은 상대 재대결 중복을 숨긴다.
- [ ] 예정·진행·취소 경기와 결과가 없는 완료 fixture에는 리뷰 CTA를 노출하지 않는다.
- [ ] 완료 경기별 UI가 생긴 뒤 기존 `/my/reviews` 일반 리뷰 행을 중복 노출하지 않는다.
- [ ] 대회 전체가 `completed`인 기존 결과·대진표·리뷰 액션 목록은 회귀 없이 유지한다.
- [ ] 모바일/태블릿/데스크톱에서 44px 이상 터치 영역, 다크 모드 토큰, 명확한 텍스트 계층을 유지한다.
- [ ] 승부차기 경기는 정규 점수와 `PK` 점수를 함께 표시하고, `null=비공개`, `TBD=미정` 상태를 구분한다.
- [ ] 관련 단위 테스트, API 통합 문서, 시나리오 문서를 같은 변경에서 동기화한다.
