...
## 탭바 여백 `max()` 변경의 Android 영향 검증 (2026-08-30)

`globals.css`는 Android 앱도 그대로 쓰고, 이 값 주변은 다른 사람이 의도적으로 튜닝한 자리다
(`30f779e7e` edge-to-edge nav, `87c6b8596` FAB 겹침 수정, `e29209a26` safe area). 그래서 바의
높이에 기대어 자리를 잡는 것들이 어긋나지 않는지 인셋 4종에서 실측했다
(`scripts/ios/check-bottom-nav-geometry.mjs`).

| 인셋 | padding-bottom | 바 높이 | 바 상단 | `.tm-scroll-area` | `.tm-floating-fab` | 탭 라벨 |
|---|---|---|---|---|---|---|
| 0px (브라우저) | 16 → 16 | 불변 | 불변 | 불변 | 불변 | 불변 |
| 24px (Android 제스처) | 40 → 24 | 불변 | 불변 | 불변 | 불변 | 40 → 24 |
| 34px (iOS) | 50 → 34 | 불변 | 불변 | 불변 | 불변 | 50 → 34 |
| 48px (Android 3버튼) | 64 → 48 | 불변 | 불변 | 불변 | 불변 | 64 → 48 |

**바의 높이와 상단이 인셋 전 구간에서 불변**이다 — `min-height: calc(nav-height + safe-bottom)`가
높이를 정하고 padding 은 그 안에서 콘텐츠 위치만 바꾸기 때문이다. 따라서 바 높이에 기대는
`.tm-scroll-area`(자리 예약)와 `.tm-floating-fab`(18px 간격)은 **하나도 움직이지 않는다.**
`87c6b8596`이 고친 겹침도 그대로다.

바뀌는 것은 **바 안에서 탭 라벨의 위치 하나**다. 라벨 아래 여백이 `16 + 인셋`에서 `인셋`이 된다.
Android 3버튼(48px)에서도 시스템 내비를 48px 그대로 비우므로 겹침은 생기지 않는다 — 여분의
16px 만 사라진다. 브라우저(인셋 0)는 16px 로 동일해 영향이 없다.
