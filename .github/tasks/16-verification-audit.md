...
- [ ] **1.1.2** `pnpm --filter @matchup/web build` 실행
...
- [ ] **1.4.2** Frontend typecheck:
  ```bash
  pnpm --filter @matchup/web exec tsc --noEmit
  ```
  → 에러 개수, 파일별 카운트 기록
- [ ] **1.4.3** Backend typecheck:
  ```bash
  pnpm --filter @matchup/api exec tsc --noEmit
  ```
- [ ] **1.4.4** Frontend unit (Vitest):
  ```bash
  pnpm --filter @matchup/web test -- --run
  ```
  → suite/case 통과율 기록
- [ ] **1.4.5** Backend unit (Jest):
  ```bash
  pnpm --filter @matchup/api test
  ```
- [ ] **1.4.6** Backend integration (Supertest):
  ```bash
  pnpm --filter @matchup/api test:integration
  ```
