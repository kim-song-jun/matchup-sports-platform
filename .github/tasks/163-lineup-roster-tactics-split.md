...
- **전술보드 포메이션 프리셋** — 대회 경기·친선 팀매치 양쪽에서 `lineupConfig` 를 같은 방법으로
  가져오는 조회 경로가 아직 없어, 전술보드는 지금 `formationOptions={[]}` 로 **자유 배치만**
  연다. 라인업 화면에서 피치 탭을 빼면서 "프리셋 골라 자동 배치" 조작 하나가 제품에서
  사라진다(데이터는 그대로 왕복한다 — `formation` 과 좌표는 저장 payload 에 실린다).
  **별도 태스크이고, UI 착수 규칙에 따라 A·B·C 3안 대상이다.** 그때 `buildFormationPresets`
  (이번에 dead 로 삭제)를 다시 만든다 — D-17("서버 lineupConfig 가 유일한 출처, 화면에
  하드코딩 카탈로그 금지")은 그대로 유효하다.
