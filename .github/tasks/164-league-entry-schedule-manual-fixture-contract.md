...
- [ ] 결과 보장(D6)은 이미 구현된 계약(즉시 확정 + 이의 7일, 운영자 사후 입력 = `league-result-entry`)을 **그대로 유지**한다 — 수동 대진·신청 팀의 경기도 같은 경로를 탄다는 것이 이 태스크의 검증 항목이다.
...
- **BE-5 ④ contract.** `v1League.*` / `v1LeagueTeam.*` 호출 12 파일 → `V1Tournament(kind='regular_league')` / `V1TournamentRegistration` 으로 재배선. 역방향 dual-write 제거. **`git grep -n 'v1League\b\|v1LeagueTeam\b' -- apps/v1_api/src apps/v1_web/src → 0`** 이 된 뒤에만 drop 마이그레이션 PR 을 따로 연다. drop 은 idempotent(`DROP TABLE IF EXISTS`), alpha 실행 전 사용자 직접 승인.
...
- [ ] `git grep -c 'v1League\b\|v1LeagueTeam\b' -- apps/v1_api/src apps/v1_web/src` → 0, 그 뒤 drop 마이그레이션. 마이그레이션 replay + drift 게이트 green.
...
3. **수동 대진의 라운드 번호.** 선택 입력, 없으면 `null`(순위 계산에 라운드는 쓰이지 않는다 — `league-standings.ts` 확인 필요, BE-1 에서 실측 후 확정).
