...
## 최신 판정 — 기준선 UI 결함 3건(M-T/M-M/M-A) dev 머지·alpha 실화면 재검증 완료. 대회판 로스터 동기화 결함(신규 발견) 수정·머지 완료. 남은 것은 42흐름 중 A-M(관리자 결과정정) 우선 재검증 + 나머지 39흐름의 스코프 재확인
...
- **42흐름 재검증 스코프 재확인(리서치 결과):** `docs/scenarios/index.md`"41/42, 잔여 A-M"은 **9/15 M11 마이그레이션+alpha 데이터 전체 재생성 이전** 기준 — A-M(관리자 결과정정)의 유일한 19/19 PASS 근거는 alpha가 아니라 별도 복제 DB에서 나온 것이라 현재 alpha엔 적용된 적 없음. 그 외 39흐름은 "M11 이후 큰 로직 변경 없음"을 전제로 보류하되, 이 전제 자체는 명시적으로 미검증 상태로 남긴다. 사용자 지시(2026-09-15): 전체 재검증 대신 **영향받은 항목만 우선**(A-M + 위 3건 재검증) — 위 3건은 완료, **A-M은 관리자 결과 정정(officialize) 뮤테이션이 필요해 alpha 데이터 쓰기 승인을 별도로 받고 진행 예정.**
