...
## 최신 판정 — 42흐름 완료(사용자 확정, 2026-09-15). 사용자 제보 원본 버그(대회판 로스터 동기화)도 alpha에서 처음부터 끝까지 실클릭 재현 완료. 기준선 3건(M-T/M-M/M-A) + A-M + diff감사로 좁힌 11개 전부 재검증. 유일한 잔여: S-L "대진 교체" 실경로(사용자 지시로 보류)

### 2026-09-15 — 사용자 제보 원본 버그, alpha에서 A-Z 실클릭 재현 (PR #1212 최종 검증)

로컬 통합테스트로는 이미 증명됐지만, 사용자가 "실제 flow로 검증 다 됐냐"고 재확인을 요청해 alpha에서 신규 대회를 하나 만들어 그 정확한 순서를 그대로 밟았다. 새 팀도 하나 만들어(`(테스트)0915 로스터동기화팀`, alpha.e2e.player01 소유) 기존 E2E 팀과 섞이지 않게 했다.

1. `/admin/tournaments/new`로 대회 `8d5a624d-...`("(테스트)0915 로스터동기화 재현검증") 생성 → 접수 시작.
2. 새 팀 + `E2E 알파 B팀` 둘 다 **명단 0명인 채로** 신청 → 관리자가 둘 다 확정("명단 미등록" 상태 그대로).
3. `/admin/tournaments/:id/bracket`에서 "결승" 조 생성 → 두 팀 배정 → **"대진 자동 생성"** — 이 시점 양쪽 로스터가 전부 0명인 채로 대진(게임 `56534b20-...`)이 만들어짐.
4. **베이스라인 실측**(`GET /games/:id/operations-lineup`): 양쪽 사이드 전부 `participants: []` — 정확히 사용자가 겪은 "명단 없이 복사가 끝난" 상태를 실제로 재현.
5. player01 계정으로 새 팀 신청 명단에 선수 1명 추가(본인, 등번호 7).
6. **재측정**: 그 팀 사이드에 **새 리비전(revision 2)**이 자동 생성되고 `participants`에 방금 추가한 선수(userId 일치, `displayNameSnapshot: "E2E선수01"`, `jerseyNumber: 7`, `started: true`)가 정확히 채워짐. 상대팀 사이드는 그대로 `participants: []`(내가 그쪽 로스터는 안 건드렸으니 당연) — 사이드별로 독립적으로 정확히 반응함을 확인. 구 리비전(revision 1)은 삭제되지 않고 보존(additive-only 설계대로).

**결론**: 사용자가 신고한 버그는 코드 수정 + 로컬 통합테스트 + 이번 alpha 실클릭 재현까지 삼중으로 확인 완료. 테스트 대회·팀은 alpha에 영구 남는다(대진 생성 후 삭제 불가 — 기존 관례).

### 2026-09-15 — 리뷰 제출 403 "버그" 원인 규명: 오진으로 판명, 수정 불필요

L-V/M-V 재검증 중 발견했던 리뷰 제출 403을 사용자 승인 하에 직접 재현했다. QA 에이전트가 `NOT_TEAM_MEMBER`(`tournament-fixture-reviews.service.ts:385`)라고 보고했지만, `curl`로 실제 로그인·GET 조회·POST 제출을 전부 재현한 결과:
- player01은 홈/원정 두 팀 모두 `status: 'active'` 멤버(관리자 API로 직접 확인) — `resolveReviewerTeams`가 막을 이유가 없고 실제로 안 막았다.
- 리뷰 대상 조회(`GET /reviews/sources/tournament_fixture/:id`)는 **200 정상 응답**, 팀·개인 타깃 목록도 정확히 나왔다.
- 실제 제출(`POST /reviews`)에서 뜬 진짜 에러는 **`403 NOT_ACTUAL_PARTICIPANT`**("실제로 출전한 선수만 상대 선수를 평가할 수 있어요") — `tournament-fixture-appearance.ts`의 공식 결과 리비전 기준 출전자 집합 검사다. 이 테스트 픽스처는 골 기록 1건 말고 실제 라인업·출전 데이터가 없어(A-M 조사 때 확인한 것과 같은 데이터 공백) 아무도 이 게이트를 못 지나가는 게 정상 상태다.
- 결론: QA 에이전트가 에러 코드를 잘못 읽은 **오진**이었다. 오늘 세션의 어떤 수정(PR #1209/#1210/#1212)과도 무관하고, 코드 수정 불필요.

### 2026-09-15 — diff감사로 좁힌 11흐름 alpha 실클릭 재검증 (4개 병렬 에이전트)

- **PASS(라이브 확인)**: G-L(비인증 공개 API 200 + 실제 상세 진입), S-T(체크인→경기시작→득점→전후반→종료 전체 실주기, 공개 API로 ended/1:0 확인, 임시 스태프 배정은 검증 후 해제), M-R(수정된 분기를 실제로 탐 — "연결할 참가자 없음" 정상 응답, empty-eligible 경로가 정확히 오늘 고친 가드 우회 케이스), X04(라이브로 대회 설정 변경 2회 실행 후 기존 3개 경기 피리어드 ID·해시 불변 확인, 테스트 후 원상복구).
- **PASS(코드 읽기로 확인, 라이브 쓰기는 불필요 판단)**: A-T(읽기 경로 라이브 확인 + 대진 생성 자체는 이미 오늘 M11 재생성 세션에서 별도 승인 하에 실행됨), A-L(승인 대기 티어 리그 데이터가 M11로 전부 사라져 라이브 재현 불가하지만, 소스 읽기로 그 결함 유형에 구조적으로 면역임을 확인 — `decided` 판정이 별도 stale 플래그가 아니라 `series.seasons`에 다음 시즌 존재 여부로 직접 계산됨), X12(수상자-프로필 링크 부분은 오늘 M-A 재검증에 이미 포함됨 — X12 본래 정의는 그것과 다른 "명단 밖 수상자 저장 시도 시 거부+기존 데이터 보존" 케이스라 PR #1209와 파일이 아예 안 겹침, 트랜잭션 앞단 가드 확인).
- **PASS(UI, 제출은 별개 이슈로 BLOCKED)**: L-V·M-V — 44px 터치타겟(전체 버튼 실측 44×44), 별점 대비(각각 5.29:1/4.11:1, WCAG 통과) 등 검토 대상이던 3개 UI 커밋은 라이브에 정상 반영. **단 리뷰 제출 자체가 정식 팀 멤버(player01 포함)에게도 403 `NOT_TEAM_MEMBER`로 막힘** — 검토 대상 UI 커밋과는 무관한 별도 백엔드 이슈로 보이고(오늘 M11 대회 데이터 재생성으로 로스터-경기 연결 상태가 꼬였을 가능성), 아직 원인 미파악. 리뷰 신규 작성 0건, alpha 데이터 훼손 없음.
- **PASS(정상 거부 확인)**: S-V(스태프는 두 케이스 다 정상 403/거부 — 원래 계약대로 팀 소속 아니므로 리뷰 작성 불가).
- **PARTIAL — 보류**: S-L. 순위표 read 경로는 라이브로 정상(API 200, 구조 일관) 확인했지만, 실제로 의미가 바뀐 "대진 교체(reconcile-in-place)" 로직은 alpha에 **리그가 이 하나뿐이고 21경기 전부**를 건드려야 해서 에이전트가 스스로 판단해 실행하지 않고 보류·보고함. 재현하려면 이 리그의 대진을 실제로 재생성해야 함 — 사용자 승인 필요.

**요약**: diff감사로 좁힌 11개 중 8개는 완전 PASS(라이브 또는 코드 근거), 1개(S-L)는 고위험 액션이라 보류, 나머지 2개(L-V/M-V)는 검토 대상이던 UI 자체는 PASS이지만 **제출 흐름에서 이 재검증 범위 밖의 새 결함(리뷰 403)을 우연히 발견**했다. 42흐름 전체로 보면 사실상 완료 상태이고, 남은 것은 이 신규 발견 403 원인 규명 여부와 S-L 대진 교체 실경로 검증 여부 — 둘 다 사용자 결정 대기.

### 2026-09-15 — 나머지 38흐름 diff 기반 전제 감사 (2단계: 커밋 메시지 → 실제 diff)

**배경**: 41/42 중 오늘 재검증한 A-M을 뺀 38흐름은 각자 PASS 시점(9/8~9/12) 이후 dev에 머지된 변경이 그 검증 내용을 깨뜨렸을 수 있다는 우려로, 실클릭 대신 코드 변경 이력을 감사했다.

**1단계(가벼운 감사, 커밋 메시지만)**: 27흐름에 의혹(재검증필요 21 + 불확실 11 − M11 자체는 위협 아님, 진짜 위험은 9/12 19:08 `11a09dc6b`(핵심 서비스 7개 동시 변경) 한 커밋과 오늘 만든 수정 3건). 안심을 못 줘서 사용자 지시로 2단계 진행.

**2단계(실제 diff 직접 읽기, 병렬 에이전트 3개 + 직접 판단 6건)** — 결과 **27개 유효 확정 / 10개 재검증 필요 / 1개(S-T) 여전히 불확실**:

- **LIKELY STILL VALID (27)**: G-T·G-M·G-R·G-V·G-A(가디언 흐름 전부), M-L, L-L·L-M·L-A·L-R·L-T, S-M·S-A·S-R, A-V·A-A·A-R, X01·X02·X03·X05·X06·X07·X08·X09·X10·X11·X12(리뷰-숨김 부분만). 근거 요약: `11a09dc6b`(9/12 19:08)의 `team-matches.service.ts` 필터는 일반 친선 경기엔 no-op(tournamentId가 항상 null이라 새 필터에 안 걸림) — G-M/L-M/S-M이 이걸로 해소. `eb3d428bb`/`51dedb290`(레코드 후보자 식별 로직, 9/12)은 "claimable-participants 조회" 함수 하나만 건드려 X07(SUBMITTED→OFFICIAL→VOID 리비전 파이프라인)·X08(이미 연결된 본인 기록)·S-R/A-R(확정·정정 파이프라인, X07과 같은 함수군으로 유추)·X06/X11(replay·교체상한 경로, 다른 함수라 유추)이 전부 비껴간다. `78ae123f1`은 프로덕션 코드가 아니라 테스트 픽스처만 바꿨다(review UI 실변경 3건은 별도로 잡아 L-V/S-V/M-V로 분리). 시상 라우팅 9/11 소동(`22c1da286`→되돌림→`0d5b479b3` 재적용)은 `kind==='regular_league'`로만 게이트돼 L-A/G-A/S-A/A-A가 실제로 검증한 대회·경기엔 no-op. X02는 오늘 M-T alpha 재검증(멤버 버튼0/owner 버튼표시)으로 이미 실측됐고, L-T는 PR #1212가 `tournament-registrations.service.ts`(신청·결제)를 전혀 안 건드려 별개.
- **NEEDS RE-VERIFICATION (10)**: **S-L**(대진 교체가 "삭제 후 재생성"→"canonical 좌표 보존 조정"으로 실제 의미 변경 + 순위 계산 로직 변경, PASS가 이보다 먼저), **A-T**(대진 서비스에 legacy 409 체크 제거→fail-closed 캐노니컬 체크로 전환하는 광범위한 의미 변경, 문서 자체가 "Save 미실행으로 A-T는 미완" 이라고 동시 기록해 둠), **X04**(기간 고정 재조정에 새 부분 병합 로직 실제 추가), **M-R**(본인 기록 클레임 엔드포인트에 새 가드+버그수정 직격, PASS가 둘 다보다 먼저라 그 경계 조건을 탔는지 불명), **L-V/S-V/M-V**(리뷰 작성 모달 44px 터치·레이아웃·별점 대비 실변경 3건 중 일부가 PASS 이후), **G-L**(공개 리그 대진 상세 404 버그의 실제 백엔드 수정이 PASS 이후 착륙, 원래 증거가 그 라우트를 탔는지 불명), **A-L**(문서 자체가 PASS 시점에 "수정 진행 중, 아직 미검증"이라고 동시 기록), **X12**(리뷰-숨김 부분은 무관 확인됐지만 시상-수상자연결 부분은 오늘 PR #1209로 바로 고친 그 결함과 겹쳐 재검증 필요).
- **UNCERTAIN (1)**: **S-T** — 같은 날(9/12) PASS와 `11a09dc6b`(19:08) 사이 선후 관계를 못 가려 결론 보류.

상세 근거(파일·커밋 해시·타임스탬프)는 이 세션의 3개 병렬 조사 에이전트 결과에 있다 — 필요하면 재현 가능(같은 커밋 대조).

### 재개 커서 — 2026-09-15 (이어서, readme_codex.md 승계)

- **기준선 결함 3건 전부 dev 머지 완료:** PR #1209(M-A 수상자 프로필 링크 + M-M 팀 일정 상대팀 정보) · PR #1210(M-T 팀 명단 수정 권한) — 상세는 [[baseline-ma-mm-fixed-2026-09-15]] 참조. 42흐름 재개의 전제 조건이던 "3건 dev 흡수"가 충족됐다.
- **M-T/M-M/M-A alpha 실화면 재검증 — 3/3 PASS (2026-09-15, alpha 배포 `13366e787`):**
  - M-T: `alpha.e2e.player01`(멤버) 로 `/tournaments/7e04a6e9.../registrations/cafe71df.../roster` 진입 시 버튼 0개("수정 불가" 안내만). 같은 URL을 `captain.a`(owner)로 열면 "선수 추가하기" + 행별 수정/삭제 버튼 노출 — 대비 확인.
  - M-M: `/teams/<A팀>/schedules/ed6966b0-...`(리그전 대진, "상대팀 확정") 에서 상대팀명 "E2E체인팀161105" 정상 렌더(빈 값·undefined 아님, 순위표와 일치).
  - M-A: `61e6ad24.../1da67562...`(9/15 재생성 대회) 둘 다 개인 어워드 미확정("집계 중")이라, 기존에 이미 있던 `aa100000-...-005`("(테스트) 결과·영상·개인 시상 컵")로 대체 확인(신규 데이터 생성 없음). MVP/득점왕/베스트골키퍼 3건 모두 서로 다른 실제 `/users/{id}` 로 링크되고, MVP 클릭 시 수상자 본인("알파레드") 프로필로 정확히 이동(로그인한 captain 본인 프로필 아님).
- **신규 발견·수정: 대회판 로스터→경기 참가자 동기화 결함.** 사용자가 팀 채팅 캡처로 제보 — 대회는 대진 생성 시점에 신청 명단을 경기 참가자로 **한 번만** 복사하고(`TournamentBracketService.createFixture`), 로스터 잠금 여부를 안 보며, 이후 절대 다시 안 돈다. 로스터가 비어 있을 때 대진부터 생기면(또는 대진 뒤 명단 변경) 참가자가 영원히 빈 채로/구 스냅샷으로 고정 — 골 기록에 선수 정보 없음, "이 선수가 저예요" 후보 없음. 리그는 Task 170(D1′, `league-roster-sync.ts`)이 이미 막았지만 그 문서가 "대회는 범위 밖"이라 명시했었다. **[PR #1212](https://github.com/kim-song-jun/matchup-sports-platform/pull/1212)** 로 대회판(`syncTournamentRosterLineups`, `tournament-roster-sync.ts`) 신규 구현 — 리그판과 동일 불변식(additive-only, 시스템 DRAFT만 갱신, 팀장 저장본 보호) + 대회 전용 차이 2가지(① `scheduledAt` 이 선택값이라 `startAt: null` 도 "시작 전" 대상에 포함 — 로컬 통합테스트로 처음 확정 코드에서 이 케이스를 놓친 걸 실측으로 잡았다, ② 등번호를 새 참가자 행에 재기입). 로컬 통합 스펙(`v1_migrate_check` 템플릿 DB, 마이그레이션 168개 전량 재생) 9→10케이스 전부 PASS, mutation 검증(각 수정 되돌려 red 확인) 완료. Copilot 라운드 2회: 1차에서 "등번호만 바뀌면 no-op 처리되는" 실결함 지적(단순 비교 키 문제가 아니라 `updatePlayerJersey` 전용 경로엔 동기화 호출 자체가 없었다) → 수정+회귀 테스트+mutation 검증 → 2차 Approval recommended(비차단 성능 제안 3건은 보류: league/tournament sync 상호배타 조건인데 항상 둘 다 호출해 한쪽이 빈 쿼리 — 저빈도 관리 경로라 이번엔 안 고침). CI API/Web/Gates 전부 SUCCESS, dev `2bccedb24` 머지, 메인 트리 동기화·worktree 정리 완료. Alpha 배포는 진행 중(순수 백엔드 트랜잭션 로직이라 alpha UI 실클릭 검증 대상 아님 — 기존에 이미 alpha 검증된 선수 추가/제외 API 경로를 그대로 타므로).
- **42흐름 재검증 스코프 재확인(리서치 결과):** `docs/scenarios/index.md`"41/42, 잔여 A-M"은 **9/15 M11 마이그레이션+alpha 데이터 전체 재생성 이전** 기준 — A-M(관리자 결과정정)의 유일한 19/19 PASS 근거는 alpha가 아니라 별도 복제 DB에서 나온 것이라 현재 alpha엔 적용된 적 없었다. 그 외 39흐름은 "M11 이후 큰 로직 변경 없음"을 전제로 보류하되, 이 전제 자체는 명시적으로 미검증 상태로 남긴다. 사용자 지시(2026-09-15): 전체 재검증 대신 **영향받은 항목만 우선**(A-M + 위 3건 재검증).
- **A-M(관리자 결과정정→공식화) alpha 실측 재검증 — 2026-09-15, 사용자 승인 하에 기존 완료 테스트 대회 재사용(신규 대회·대진 생성 없음):**
  - 대상: `61e6ad24...`("0915test") 결승전 `85e34181...`(마포 레인저스 vs 풋살크루), 기존 공식 결과 0:2 revision#1.
  - **주 흐름 PASS**: admin 계정으로 `/admin/tournaments/{id}/bracket` → "결과 정정하러 가기" → 홈 득점 0→1 정정 제출(revision#2, 작성중) → "정정 확정"(공식 확정, 1:2) — 공개 API(`GET /tournaments/{id}/matches/{fixtureId}`, `GET /tournaments/{id}/schedule`, 비인증)로 1:2·`resultState:"corrected"`·정정 사유 텍스트까지 확인. 양 팀 전적 화면(`/teams/{id}/records`)도 승/패 1:2·2:1로 서로 정합.
  - **멱등 재실행 PASS**: 동일 정정을 한 번 더 제출→공식화 → revision#3 생성, 점수 1:2 그대로 유지, 이력 3건 모두 정합(설계상 매 제출이 새 revision을 만드는 게 올바른 감사 이력이라 "요청 재전송 멱등"은 아니지만 "반복해도 상태가 깨지지 않음"은 확인).
  - **비관리자 403 PASS**: `captain.a`로 같은 정정 URL 접근 시 UI 게이트("대회 운영자 권한이 필요해요") + `GET /admin/me` 403 `PERMISSION_DENIED` + `GET /tournament-ops/.../staff` 403 `STAFF_SCOPE_DENIED` 확인.
  - **개인 기록 반영 NOT ATTEMPTED**: 이 픽스처(및 후보로 살핀 다른 대회)의 골 이벤트가 전부 `participantId: null`("선수 정보 없음")이라 — 즉 라인업 미제출 상태라 애초에 대조할 개인 기록이 없다. 테스트 실패가 아니라 데이터 부재로 인한 미시도. 라인업이 실제로 연결된 완료 경기가 필요하면 별도로 찾아야 한다.
  - alpha 최종 상태: 해당 픽스처는 1:2 revision#3로 **정정된 채 그대로 남아 있다**(롤백 안 함, 승인된 실데이터 변경).
- **A-M 제외 39흐름의 "M11 이후 무변경" 전제**는 여전히 명시적으로 미검증이다 — 필요하면 다음 세션에서 이 전제 자체를 검증할지(예: 코드 diff 감사) 사용자에게 확인.

### 재개 커서 — 2026-09-13 AWS 재인증 이후

- **2026-09-14 02:25 KST Claude 세션 재개 — 수행 모델:** 오케스트레이터 Claude Opus 5, 분석·리뷰 sonnet, M11 설계·적대 비평·재검토 opus(되돌릴 수 없는 DROP 인프라라 승격). 이 구간에서 Codex Luna/Sol은 쓰지 않았다. 재개 시 Codex 앱 프로세스는 떠 있었으나 이 저장소에 대한 push·PR·파일 수정 활동은 없었다.

- **PR #1187 dev 머지·Alpha 3f47 실화면 PASS:** 독립 리뷰 PASS 뒤 두 결함을 같은 PR에서 고쳤다. ① `7e6d7e1a`: 인증 확인 중 CTA 라벨 `로그인 상태 확인 중`이 `ctaPending`의 `처리 중`에 가려 렌더되지 않던 죽은 분기(변이 시 red 1). ② `95390667`: Copilot 실제 결함 — 승인 대기인데 eligibility가 로딩·오류이면 상세 `viewer.joinState` 폴백만으로 취소를 열어 `/team-join-applications/undefined/withdraw`를 호출(dev 기존 동작). 취소는 eligibility `applicationId`가 있을 때만 열고 오류=재시도, 401=로그인, 로딩=`신청 상태 확인 중` 비활성으로 바꿨다. 결함을 못박던 기존 테스트를 계약 테스트 3개로 교체했고 변이 시 red 2, teams-client 33/33, v1_web tsc 0. CI API/Web/Gates SUCCESS·Copilot 새 지적 0 → dev `3f47bbd9d`, Deploy Alpha `34770043359` SUCCESS. Ego36 BEFORE(7efd): 모집 중 테스트 C팀 게스트 CTA가 390/768/1440 모두 비활성 `가입 불가`(서버는 `LOGIN_REQUIRED`). AFTER(3f47, 캡처 전후 커밋 동일): 3폭 활성 `로그인 후 가입 신청`·overflow 0 → 클릭 `/login?redirect=%2Fteams%2F<C팀>` → `/login/email?redirect=…` 유지 → 무소속 테스트 계정 로그인 후 C팀 복귀·활성 `가입 신청`·`auth/me` 200. Alpha 쓰기 0. 갤러리는 [PR #1187 코멘트](https://github.com/kim-song-jun/matchup-sports-platform/pull/1187#issuecomment-5654763586)(assets 브랜치 `ed39de1a`). 미검증: 승인 대기 취소와 운영자 background refetch의 실화면(가입 신청 쓰기가 필요해 단위 테스트 증거만).

- **PR #1191 main 흡수 → #735 MERGEABLE:** origin/main 전용 커밋 32개(실작업 19)는 모두 dev에 동등·상위 구현으로 있어 충돌 34개를 dev 쪽으로 해소했다. main 회귀 테스트 2개는 dev 기존 테스트와 시나리오·단언이 같다는 Copilot 지적이 사실이라 삭제했고, changeset 2개와 Task 160 문서만 순기여로 남겼다(승격 diff에 삭제 changeset이 생기지 않게). dev `98a00a1a`·Alpha 배포 SUCCESS. #735 CI는 `Gates / Verify release changeset`에서 `dev -> main promotion must not contain unreleased Changesets`로 실패한다 — 결함이 아니라 승격 전 release 절차 게이트이며 승격은 사용자 몫이라 실행하지 않았다. `Code scanning AI findings`(run `34769527682`)는 `claude-opus-5` CAPI 장애 재발이며 보안 검증으로 세지 않는다.

- **#735 머지 금지 경고:** main에는 M1–M10 migration이 없고 `deploy/deploy-prod.sh:264-268`은 plain `prisma migrate deploy` 1회다. 지금 승격하면 M1–M10이 cutover·writer 정지·백업 없이 프로덕션에 적용된다. #735 본문 맨 위에 머지 금지 절을 넣었다.

- **Alpha 배포 결함 D-1 확정(원문 대조):** Alpha 경로의 migrate는 `deploy/task168-stage-a-migrate.sh` 안에서만 돈다. ledger가 `complete`면 `:188`에서 migrate 없이 exit 0이고, 다른 상태도 M1 이전 폴더와 Task168 이름만 복사한다(`:51-53`). API CMD도 migrate를 돌리지 않는다. 따라서 이후 dev에 migration이 머지되면 CI·배포는 success인데 Alpha DB에는 적용되지 않는다. 확인 시점까지 누락 migration은 0(최신 M10), migration을 추가하는 열린 PR도 0이다. 동결 여부는 사용자 결정 U3 대기.

- **M11 StageB 실행 명세 v3(설계 1 + 적대 비평 2라운드, 저장소 미반영 세션 산출물):** 후보는 실행 준비 전이다. 신규 결함 D-2 StageB S3 키가 StageA와 같음, D-3 러너 custom vs preflight plain-sql-gzip 백업 형식 불일치, D-4 StageB가 active가 되면 복구·rollback 검증기가 StageA 전용이라 실패, D-5 M11 뒤 dev push가 source 활성화 뒤에 실패하며 복구 억제, D-6 정지한 writer가 `restart: always`라 데몬 재시작 시 post-M11 DB에 옛 writer가 붙음. 비평 반영: Dockerfile 기본 target 뒤바뀜 방지(stage 순서 + 명시 target), StageB SSM 명시 시한·결과 3분류·`stageBRecover`, PR-B steady ledger 판정식 L1–L4(M12 pending 허용·DB 전용 행 거부), U8을 PR-B 사전 조건으로. 순서: PR-A1(도구·계약 테스트, 미배선) → PR-A2(dispatch 전용 배선·기본 경로 가드) → `stageBPreflight` 격리 리허설(SSM, 승인) → `stageBFinal`(비가역, 직접 승인) → post-live 영수증 → PR-B(수렴, 사실상 forward-only) → PR-C(정리). 로컬 AWS 세션 만료로 SSM 단계는 전부 대기.

- **사용자 결정 대기 U1–U11:** 결정표를 비공개 아티팩트로 게시했다(U3 dev 동결, U8 prod 경로, U2 M11 직후 기동, U4 백업 형식, U6 enum 역사값, U7 리허설 백업, U9 42 흐름 Alpha 쓰기, U5 보존 기간, U11 SSM 시한 여유, U1 실행 승인은 리허설 뒤 별도). 결정과 무관한 packager(T6)·러너 기존 r1/r2 BLOCK 반영 여부(T4 일부)는 opus 독립 재검토를 진행 중이며 결과 전까지 BLOCK 유지.

- **계정 401 해소·메인 트리:** 관리자·무소속(스태프) 테스트 계정이 비공개 자격증명으로 로그인 201이다(값은 기록하지 않음). 메인 작업트리는 01:37 KST 다른 주체가 7efd로 FF해 겹침 313개가 clean이 됐고, 이후 `98a00a1a`·`3f47bbd9d`까지 `--ff-only`로 동기화했다. 두 번째 FF는 origin/dev와 바이트 동일한 untracked `readme_codex.md` 한 파일이 막아 세션 스크래치패드에 백업 후 진행했고 복원본=백업을 대조했다. 나머지 미커밋 528개는 보존.

- **S0 Alpha 읽기 전용 확인(2026-09-14, 배포 a6e770886):** collector readback의 활성 release는 기대 SHA와 같고, migration release는 Stage A release(72405a2)에 머물러 정책 `task168-stageAIntermediate`다(D-1과 일치). 전체 migration ledger 읽기 전용 조회는 171행 — 적용 167, 롤백 4, 미해결 0, 최신 적용 `20260910160000_v1_outbox_cutover_claim_gate`, M11 행 없음. 이 조회의 `releaseBindingVerified`는 false이므로 release 바인딩 확인으로 세지 않는다. 쓰기 0. 근거 `output/qa/task168/s0-readonly-20260914/`.

- **프로덕션 읽기 전용 집계(READ ONLY 트랜잭션 후 ROLLBACK):** migration ledger 125행 — 적용 123, 롤백 2, 미해결 0, 최신 `20260821120100_v1_team_record_facts_played_at_relock`, Task168 migration 0행. 레거시 5테이블 모두 존재, 행 수 fixtures 32·results 0·goals 0·videos 32·advancement_edges 0, 링크 컬럼 non-null은 `v1_games.tournament_fixture_id` 32·staff scope 0·operation audit 612. 프로덕션은 M11 전에 M1–M10+cutover가 먼저 필요하므로 U8 결정 없이 PR-B를 만들지 않고 #735를 머지하지 않는다. 식별자는 기록하지 않았다. 근거 `output/qa/task168/prod-readonly-20260914/`.

- **PR-A1 #1193(StageB 도구, draft) 적대 검증 수렴:** 원시 헤더 워커 → 파서 불일치 사냥에서 라운드마다 새 우회(전역 pax, pax size, prefix, 연속 pax로 이름 숨기기, pax path NUL)가 나와 개별 거부 방식을 버리고 **정규형 동일성**으로 바꿨다. packager와 preflight가 같은 직렬화 모듈 `scripts/release/task168_canonical_tar.py`를 쓰고, preflight는 시스템 tar를 쓰기 전에 archive를 재직렬화한 바이트가 입력과 정확히 같을 때만 통과하며, 선언 파일의 해시·크기·모드도 워커가 인증한 바이트에서 계산한다. 마지막 두 검증 라운드 blocking 0, 파서 불일치 사냥(bsdtar·GNU tar 1.35·tarfile·워커) 수작업 archive 27종 우회 없음, 정규형 검사만 제거한 변이에서 음성 5건 red, Python 3.9–3.14 직렬화 sha 동일. head `54b8ee810` ubuntu CI 초록(package 16·sidecar 74), golden sha는 macOS·ubuntu·umask 3종·TZ 3종 동일. 한 번 난 API 통합 테스트 실패(`game-projection` lease CAS)는 PR 무관 코드이고 재실행 초록이며 최근 실패 이력에 같은 테스트가 없었다. 사용자가 2026-09-14 16시경 "Alpha는 개발 환경이니 백업 결정 없이 지금 M11을 실행하고, 팀·사람만 남기고 대회는 정리해 처음부터 만든다"고 직접 지시해 U4·U7 대기를 풀었다. Copilot 리뷰 5회에서 실제 지적 5건(usage 누락 플래그, report 중복 `status` 키, 계약 migration 이름 형식·중복 미검증, preflight·직렬화기의 DEL 제어문자 불일치, changeset 과장 문구)을 고쳤고 각 음성 테스트와 변이 red를 확인했다(package 17·sidecar 77). `sort -z` 요약 지적은 macOS Apple sort 2.3 실측으로 오탐 판정. head `0a4596605` CI 초록·미해결 스레드 0으로 **dev 머지 `4e7c47494`**(2026-09-14 16:51 KST).

- **PR-A2 #1194(StageB 배선) dev 머지 `89f6eaaaa`:** 영수증 계약의 소유자다 — `<ALPHA_RELEASE_STATE_DIR>/task168/<releaseSha>/migration-stage.json`(kind `task168StageBMigration`, status `MIGRATION_COMMITTED`/`_RECOVERED`)과 그 옆 `runtime-verification.json`(kind `task168StageBRuntimeVerification`, `migrationReceiptSha256`·`manifestSha256`·`ledgerCount==11`·`driftCheck`·`healthDbTrue`·`workerHealthy`·`completedAt`), 그리고 두 영수증이 가리키는 매니페스트의 내구 사본 `manifest.json`. 계약은 `docs/ops/task168-stage-b-runbook.md`에 적었다. U2(자동 계속)까지 배선돼 있어 `MIGRATION_COMMITTED` 이후 source 활성화 → 이미지 pull → compose up → quiesce.json의 restart 정책 복원 → worker health → T7 post-live → 승격이 한 번에 이어지고, 실패는 `activation-stage.json`에 남기며 이전 이미지로 되돌리지 않는다. 리허설은 사용자 지시로 waived이고 매니페스트가 `rehearsal:{mode:"waived",...}`로 그 사실을 싣는다(`stageBPreflight`는 미배선이라 삭제). Copilot 지적 6건(입력 스냅샷 해시를 엉뚱한 산출물에서 계산, `.source.key` 접미만 검사, post-live의 `LIKE '202609%_v1_%'` 과대 범위, via-ssm 주석, 낡은 매니페스트 재사용, post-live가 11건을 강제하지 않음)을 모두 고쳤고 마지막 두 건(매니페스트 자기순환 검증, 실패 실행에서 살아남는 낡은 runtime 영수증)은 `53930a151`에서 직접 고쳤다.

- **M11 디스패치 2회 실패 — 원인 확정·수정·머지(`a67d00c01`):** 두 번 모두 **"Resolve Task168 StageA predecessor transition"** 스텝에서 죽었다(`Could not read the StageA predecessor transition`). 처음엔 빈 predecessor 기본값을 의심해 명시 SHA로 재디스패치했으나 같은 자리에서 다시 실패했고, 그 스텝이 SSM으로 실어 보내는 `scripts/release/read-task168-stage-a-predecessor.sh`를 **호스트에서 직접 실행해** 원인을 확정했다: `psql: FATAL: role "teameet_v1" does not exist`. 그 스크립트가 배포 흐름 안에서만 성립하는 가정을 두 개 갖고 있었다 — ① DB 계정을 `${V1_DB_USER:-teameet_v1}` 기본값으로 접속(SSM 맨 셸에는 운영 `.env`가 들어오지 않는다), ② 컨테이너를 `docker compose`로 조회(compose가 모델 전체를 해석하는데 alpha compose 파일은 `deploy-alpha.sh`만 내보내는 `ALPHA_API_IMAGE`를 요구해 보간 실패). 러너와 StageB 래퍼·D-5 가드는 고칠 게 없었다(전자는 이미 컨테이너 환경에서 파생, 후자는 `.env`를 source). **두 결함 모두 fail-closed라 Alpha DB는 변경되지 않았고 M11은 실행된 적이 없다**(실측 ledger 167 적용·M11 행 없음·레거시 5테이블 그대로). [PR #1200](https://github.com/kim-song-jun/matchup-sports-platform/pull/1200)이 compose 라벨로 컨테이너를 직접 지목하고 역할·DB를 컨테이너 환경에서 파생하도록 고쳤으며(러너가 실행 시점에 다시 계산하는 신원과 정의상 일치), `docker exec -i`가 파이프된 스크립트 자신의 stdin을 먹어 첫 쿼리 뒤가 조용히 사라지던 잠재 결함도 함께 제거했다. 고친 바이트 그대로를 실제 Alpha 호스트에서 읽기 전용 실행해 5개 계약 필드가 모두 나오고 `databaseIdentity`가 StageA transition 영수증과 문자열 동일함을 확인했다. 계약 테스트 `scripts/qa/test-task168-predecessor-read.sh`(13건, 실제 스크립트를 가짜 docker로 `bash -s --` 파이프 실행)를 CI에 배선했고 변이 3종(기본값 되돌리기 2 red·compose 되돌리기 5 red·`-i` 추가 2 red)으로 각 판정이 실제 결함을 잡는 것을 확인했다. Copilot 지적 2건(`set -e` + `grep -c`가 진단 메시지를 삼킨다)은 실측으로 반증했다 — `[[ ]] || {}`·`&& ||` 목록에서는 errexit가 억제돼 진단이 그대로 출력된다.

- **세션 운영 기록:** 워크플로 하위 에이전트 3개가 Bash 도구 결과를 67–93분 받지 못해(대응 프로세스 없음) 워크플로를 중단 후 저장된 결과로 재개했다. PR-A2 재개는 첫 실행의 인자가 문자열로 전달돼 프롬프트에 설계 경로가 `undefined`로 들어간 탓에 캐시가 맞지 않아 3차 수정이 처음부터 돌기 시작했고, 즉시 중단(파일 변경 0 확인) 후 인자를 객체로 넘긴 새 5차 워크플로로 다시 띄웠다. 하네스가 남긴 익명 볼륨 16개(PR-A2 postgres·registry 컨테이너의 `docker rm -f` 무 `-v`, 검증용 임시 postgres 1개)는 생성 시각·내용을 대조해 이 세션 것만 지웠고 볼륨 수는 기준선 52로 돌아왔다.

- **M11 디스패치 3~7차 — 매번 다른 원인, DB는 매번 무사:** ③ SSM `--parameters` shorthand(`commands=[...]`)가 `jq -Rsc`의 끝 개행을 글자 `n`으로 바꿔 SHA가 41자가 됐다(JSON 파라미터로 고정, 원격 stderr 노출 추가). ④ StageB와 푸시 배포가 `<sources>/<sha>` 한 자리를 공유해 `Stored source <sha> has a different .source-sha256`로 죽었다(매니페스트에서 소스 키를 파생하도록 교체). ⑤ 매니페스트 생산자는 psql 출력을 파이프로 해시하고(psql이 붙이는 끝 개행 포함) 러너는 `printf '%s'`로 해시해 같은 데이터가 다른 값이 됐다. ⑥ `v1_game_cutover_epochs.write_mode`가 `legacy`로 남아 러너 전제조건에 걸렸다(사용자 승인 후 1행 UPDATE, 이후 비-new 0). ⑦ `currently running writer image does not match the authenticated Stage A predecessor image`. 매 회차 quiesce 이전에 멈춰 **원장·레거시 테이블·writer 모두 무변경**임을 실측으로 확인했다.

- **⑦의 근본 원인은 코드 결함이 아니라 프로토콜 전제였다:** 러너는 "지금 도는 writer = Stage A 예정자 이미지"를 요구했는데, 그 값은 Stage A 커토버가 남긴 전환 영수증에 박제되고 **이후 배포로 갱신되지 않는다**(커토버 완료 후의 푸시 배포는 `assert_existing_transition`으로 검증만 하고 새 영수증을 쓰지 않는다). 즉 이 검사는 사실상 *"Stage A 이후 어떤 배포도 없어야 한다"* 를 요구했고, Alpha는 dev 머지마다 재배포되므로 StageB를 돌릴 창이 없었다. 되돌리는 길(예정자 릴리스 재배포)은 `deploy-alpha.yml`의 `if: github.ref == 'refs/heads/dev'` 때문에 막혀 있다 — dev 자체를 146커밋 되돌리지 않는 한 그 커밋을 배포할 수 없다.

- **게이트 교정 PR #1205(`fix/task168-stageb-running-image-gate`) 머지 `7f5c7a735`:** 대조 대상을 **이 릴리스 자신의 Stage A 빌드**(`sha-<release>`, 매니페스트 생성 시 ECR digest로 확정한 `database.task168.expectedRunningApiImage`)로 바꿨다. Stage A 영수증 인증(원장 경계·스키마 sha·quiesce/backup/cutover-report 증거 sha)은 손대지 않았고, 매니페스트 빌더가 Stage A 커밋이 릴리스 커밋의 조상인지 확인한다(호스트엔 git 이력이 없어 거기선 확인 불가). 근거는 코드 실측이다 — 실패 복구 경로는 예정자 이미지가 아니라 **멈춘 그 컨테이너를 `docker start`** 하고 현장 캡처값과 대조하므로 롤백 안전성이 이 게이트에 걸려 있지 않았고, 레거시 테이블 재기록 위험은 `write_mode='new'` 전제조건이 이미 막는다. 러너 통합 하네스에 시나리오 y(예정자 이미지 ≠ 돌고 있는 이미지 → 그래도 커밋)를 추가하고 e(박제값 ≠ 돌고 있는 이미지 → writer 손대기 전 거부)를 교정했다. 변이 확인: 옛 비교로 되돌리면 y가 red, 게이트를 무력화하면 e가 red. 독립 적대 검증(별도 에이전트)은 8개 항목 중 7개를 "저자 주장이 맞다"로 반박하고 SAFE TO MERGE를 냈으며, 남은 지적(`git merge-base --is-ancestor`의 rc 1과 128을 한 문구로 보고)은 같은 PR에서 분리했다.

- **M11 실행 완료(2026-09-15 KST, run 34879688639, 릴리스 `7f5c7a735`, 예정자 `72405a2d`, executionTimeout 1800s):** 디스패치 전 호스트 실측으로 활성 릴리스=dev 헤드, **돌고 있는 api·워커 이미지 = ECR `sha-<head>` digest**, 원장 167/미해결 0, M11 부재, 레거시 5테이블, `write_mode` 비-new 0, 이 헤드의 상태 디렉터리 비어 있음을 확인한 뒤 올렸다. 실행 후 실측: 공개 테이블 **118**, 레거시 픽스처 테이블 **0**, 원장 **168 적용 / 0 미해결**, M11 체크섬 `08eac734…`, 링크 컬럼 잔존 0, 대상 enum 2종(`V1TournamentGoalTeam`·`V1TournamentFixtureStatus`) 제거, 영수증 `MIGRATION_COMMITTED / stageBFinal` + 런타임 검증 + 매니페스트 사본 존재. **보존 대상은 그대로** — 사용자 396, 팀 86, 팀 멤버십 418. Alpha `health.db=true`.

- **검증 스크립트의 거짓 경보 1건:** post-M11 확인 스크립트가 `enums_left=2`를 보고했으나, 대소문자를 구분하는 `typname LIKE '%fixture%'`가 enum이 아니라 **살아 있는 테이블 `v1_tournament_staff_fixture_scopes`의 행 타입과 그 배열 타입**을 센 것이었다. M11이 지우기로 한 두 타입은 이름으로 직접 조회해 둘 다 제거됨을 확인했다. 기대값 라벨이 틀렸던 것이지 DB 결함이 아니다.

- **PR-B #1199 머지 `dd70b6e88` 및 배포 경로 정상화:** dev를 3-way로 흡수하던 중 Gates가 **출력 한 줄 없이** 실패했다 — dev가 StageB 매니페스트에 요구하게 된 `expectedRunningApiImage`가 이 브랜치 자체 픽스처에 없어 검증 함수가 1을 반환했고 `set -e`가 스위트를 죽였다(빈 실패로 보인 이유). 픽스처에 필드를 넣어 해소했고, 같은 형태의 픽스처 생산자 4곳이 모두 필드를 갖췄음을 전수 확인했다. 머지 후 **M11 이후 첫 StageA 배포가 성공**했고(run 34882477133) 재실측에서 원장 168/0·M11 체크섬 동일·레거시 0·공개 테이블 118·writer 2개 정상이 유지됐다 — 정본 디렉터리로 수렴한 M11 파일이 체크섬 드리프트를 만들지 않았다는 뜻이다.
...
- **Alpha 복구 실제 PASS / Ego 시작:** deploy `34749865636` SUCCESS, actual readback SSM `28374cf3-35d3-4396-be38-ad19e9350291` SUCCESS/0 (`alpha-2140a-readback-result-20260913-r1.json`). deployed2140a/migration72405a로 분리한 16개 verification 모두 true: API·worker running, DBhealth true, 원본fixture310 mapped310/unmapped0/canonicalFixtureGames310, legacy links0, receipt/status/hash/schema/backup/ledger/raw migration/seals 모두 일치한다. seed 이후 canonicalDetails325(기존310+QA15), canonicalGames674, lineages24이며 원본310 보존을 별도로 확인했다. 이 결과는 서비스 복구 증거이고 전체42 E2E 완료는 아니다. Sol에 Ego before2140 baseline을 배정했으며 UI PR #1184 최신 head `dd3694d0c19bc0e6f84a79ab870fbe5bbf36d330` CI `34750361583` 검증 후 머지한다. baseline 완료 전 UI 배포를 겹치지 않는다. #1181도 복구 후 안전한 배포 순서로 처리한다.

- **UI 후속 review 수정:** PR1184 head0b1814의 TypeScript 오류 수정 후 CI `34750154443`가 실행 중이다. Copilot이 다크모드 basic-info 구분선(--grey100이 card surface와 같음)과 비공개 배지 Lock/text 간격을 지적했다. Luna candidate globals SHA `cd88d128554d09dc847f48de8a9d48ae81d497b1cac5e2b384a10706a38231bf`는 해당 border만 `--border`로, teams-page SHA `dbf42a1060738cb4e24116389b076e7bd79551abda0dce51d9252ca3b8fd340a`는 해당 badge만 gap4로 수정했다. Sol이 0b181 대비 정확히 두 hunk PASS했다. 옛 empty-query crash 코멘트는 이미0b181에서 해결됐고 새 지적과 구분한다. Alpha2140 배포는 SSM 실행 중이며 아직 health 확인 전이다.
...
- **이벤트 허브 태그 일관성 — 사용자 지적:** 리그(종류), 풋살(종목), 진행중(상태) 태그의 UI/UX 차이를 점검한다. 각각의 의미와 상태 색상 구분은 유지하되 높이·폰트 크기/두께·내부 여백·모서리·정렬·색상 토큰이 목록/상세에서 일관되는지 확인한다. Sol의 이벤트 허브 조사에 추가했으며 아직 실제 화면 확인이나 수정 완료로 판정하지 않는다.

- **사용자 추가 범위 — 이벤트 허브 개선:** 기존 팀매치 빈 상태 수정과 Alpha 복구에 이어 이벤트 허브 개선도 요청했다. Sol에 현재 v1 이벤트 허브의 정확한 route·컴포넌트·API·참여 조건·CTA·상세 동선과 빈 상태/오류/레이아웃을 읽기 전용으로 조사하도록 배정했다. 구체적인 화면 문제는 코드·실제 Ego 관찰 근거를 확보해 분리하고, 새 기능이나 정책을 임의로 추가하지 않는다. 아직 이벤트 허브 실제 화면 검수·수정 완료가 아니다.

- **사용자 추가 UI 결함 — 팀매치 만들기 팀 없음:** 첨부 화면에서 팀 선택 1/6단계에 생성 가능한 팀이 없다는 안내와 활성화된 다음 CTA가 동시에 보이며, 사용자는 다음 단계로 진행 가능하다고 보고했다. 화면 증거와 실제 클릭 재현은 구분한다. Luna에 기존 v1 생성 화면의 선택·진행 guard, 로딩/조회 실패/실제 빈 목록/권한 없음 구분, 유효한 팀 선택 이전 클릭·Enter 진행 차단, 기존 팀 만들기/찾기 동선 및 과도한 여백·버튼 배치 개선을 배정했다. Alpha 복구와 독립 후보로 준비하고 Sol 리뷰→GitHub PR CI→dev 머지→백그라운드 Ego 실제 역할·viewport 검수를 따른다. seed 복구 작업을 대체하지 않는다.

- **사용자 최신 운영 확정:** 로컬 작업 브랜치는 항상 dev, PR base도 dev다. 공유 작업트리 브랜치를 전환하지 않는다. 변경은 PR로 올려 GitHub CI·독립 리뷰 통과 후 dev 머지하고 Alpha 배포·Ego 검수로 이어간다. 로컬 전체 CI를 중복 실행하지 않는다. PR용 원격 head는 dev 대상 리뷰 전달용이며 로컬 작업 브랜치와 구분한다. 현재 로컬 seed 테스트의 M8 사전조건 실패를 PASS로 표시하지 않고 PR에 명시하며, 추가 고비용 로컬 clone 재검증 준비는 중단하고 GitHub PR CI 경로를 우선한다.

- **18:20 KST seed 후보·검증 상태:** source candidate SHA `84f96bc0cb9d431d76b88a20122c434000ab169f67433cfb67f44008e9d6b6a0`, actual integration candidate SHA `a7d9ebe8d91aad79df4d63ed9f7eddfcbce56a54705e38e576d154fff8b31c9e`를 Sol이 검토했다. 기존 경기의 participant/identity 변경은 생략하되 side/team·revision1 lineup 실패는 유지하고 신규 경기 생성은 유지한다. 로컬 RED/GREEN 최초 실행은 둘 다 `V1GameOfficialFactSourceType` 부재로 목표 assertion 이전 실패했다. 예상 RED가 아니므로 통과 증거로 사용하지 않는다. 승인된 로컬 source DB는 수정하지 않고 작업 전용 clone에 정확히 고정된 M8 SQL을 적용한 뒤 Jest가 그 clone에서 RED/GREEN 각각을 복제하는 좁은 재검증을 준비한다. 실행 runner가 b9bf→d833으로 달라진 이유와 실제 receipt도 확인 중이다. source/test 외 공유 WIP는 편집하지 않았고 아직 새 PR/커밋/Alpha 재실행하지 않았다.

- **18:10 KST 실제 DB 판정:** incident SSM `979800cc-6ed6-4cf7-b991-a69527396050` SUCCESS/0 (`alpha-72405a-incident-result-20260913-r1.json`): transition COMPLETED, receipt status/hash/backup bound, ledger10 exact, retained5tables/3columns, seals5|5|3+M10 gate1, legacy game/staff/audit links0, fixture310/details310/canonicalFixtureGames310, lineage24. runtime API/worker exited, active release와 candidate 불일치라 runtimeReady/releaseMatchesActive/schemaBound는 false이며 완료 주장 금지. stopped-runtime probe는 api_stopped로 명시했고 raw migration file hash는 미검증이다. participant SSM `6ee27167-05b0-443f-89e7-a89cc8b85f06` SUCCESS/0 (`seed-participant-result-20260913-r3.json`): seed expected8984 ID는 없지만 game432444b9/HOMEside2abb614b/rev1lineupc105848c에는 실제participant `ce2a20a6-5ee1-47df-8889-4d7a2e4597d2`가 있으며 user/identity NULL인 역사 snapshot이다. Luna는 기존 운영 snapshot을 현재seed로 덮지 않도록 좁은 수정, Sol은 역사import/seed downstream 계약 검증 중이다. unchanged seed 재시도·삭제·백업재수행은 하지 않는다.

- **2026-09-13 17:57 KST 실제 Alpha 새 실패:** dev `72405a2d5486d64da2855daa56339cbaf09610c3` CI `34748456703` 전체 PASS, Alpha `34748456714` FAILED. 실제 로그는 M8/M9 등 migration 성공, schema up-to-date, flagsCreated0, UPDATE9 COMMIT 이후 `Canonical seed participant 89842139-1438-429b-8329-cd3f7b4eb0e4 has an incompatible operational snapshot.`로 중단됐다. 이관 transaction 결과는 별도 실제 DB readback으로 확인해야 하며 API/worker는 계속 정지 상태다. `canonical-tournament-seed.ts`는 기존 게임의 deterministic participant ID가 없거나 game/side/user가 다르면 이 오류를 낸다. Luna가 실제 row/seed 계약 진단을 담당하고 Sol이 10-ledger committed recovery 경로를 검토한다. 강화 collector b8556566은 Sol PASS했지만 이번 실행 SSM `823bb9d6-0212-492b-ac67-520f3f8c37b3`는 정지 API에 docker exec를 시도하여 stdout 없이 실패했다(`alpha-72405a-readback-result-20260913-r1.json`); 이는 복구 PASS가 아니며 정지 런타임용 진단 collector를 별도 보완한다. 최초 readback용 generator9f는 상태/stage/schema 게이트가 약해 더 이상 readiness 근거로 사용하지 않는다. 42개 Alpha Ego/API 흐름은 여전히 미완이고 #1181 머지는 복구 후다. 최종 StageB 후보는 별도 작업이며 실제 실패 복구 우선으로 잠시 보류했다.

- **2026-09-13 17:46 KST 복구 배포:** PR #1182 exact head `2c4531cb07cafa49455a4fbf5e711409429b7b0b`의 Gates/API/Web CI `34748009649` 전체 PASS와 독립 리뷰를 확인하고 dev에 머지했다. merge SHA `72405a2d5486d64da2855daa56339cbaf09610c3`, matching dev CI `34748456703`, Alpha `34748456714` 진행 중이다. 첫 GitHub GraphQL 머지 요청 오류 후 OPEN/CLEAN 상태를 확인했고 동일 head 재시도는 MERGED로 확인했다. fetch만 수행하고 공유 HEAD/index/WIP는 보존했다. 실제 Alpha 복구·42개 E2E는 아직 PASS가 아니다. 최초 readback은 이 merge SHA를 바인딩한다. #1181 문서 PR은 복구 health/readback 이후 머지한다. docs 재배포 collector는 migration/deployed SHA를 분리하며 Sol이 report 배열 출력 및 receipt release/image/DB/services 바인딩 누락을 지적해 Luna가 수정했고 재검토 중이다.
...
- **PR/CI 재개 지점:** PR #1182 `fix: recover Alpha historical competition cutover`, head `2c4531cb07cafa49455a4fbf5e711409429b7b0b`, base dev. 커밋 diff는 7binding+v7archive+Changeset+이 문서의 정확히10경로이며 공유 HEAD/index는 유지했다. CI `34748009649` Gates PASS/API·Web 진행 중, exact-head Copilot 재요청 완료. 예전 releaseSha 코멘트는 original_commit_id eeab9fe의 지적이며 현재2c453의 runner81–82에 quiesce/backup 각각 RELEASE_SHA 확인 및 writer set 검증이 존재한다. 새 검토가 완료되면 실제 지적을 구분해 처리한다. 별도 #1181 ff9179a CI PASS/PR scope 설명 보완 완료, Alpha 복구와 겹치지 않는 순서로 dev 머지 예정. 배포 완료 후 기존 readback generator+diagnostic helper를 새 mergeSHA로 실행한다. 기존2f payload를 재사용하지 않는다. Sol 첫 Ego smoke는 P1/P2/A1/A2/A3이며 실제42 queue의 G-T/G-L/G-M/G-R/G-A와 동일 집합이 아니므로 근거를 명시적으로 매핑해 중복계수하지 않는다. 실제 배포 SHA/image/health 및 테스트용 fixture/role 바인딩을 확보하기 전에는 Ego mutation을 시작하지 않는다.

- **사용자 재확정 / 배포 후보 고정:** 구현·버그 수정은 Luna 백그라운드, 독립 검증과 Ego UI/UX·역할별 E2E는 Sol 백그라운드, root는 기획·우선순위·통합·최종 판단을 맡는다. 큐에서 전달되는 개선 요청은 이 문서에 누적해 각각 반영→재검증한다. root가 직접 Ego를 조작하지 않는다. 현재 알려진 범위는 기존 42흐름과 버튼/글자/색상/배경/간격/배치/스크롤/고정CTA/포커스/애니메이션/console/network이며, 아직 전달되지 않은 큐 메시지를 읽었다고 주장하지 않는다.
- **v7 생성·독립 검증 완료:** archive SHA `829cbb214afc26c417947c864fd477647498003e06915ca20b4d2f8b44b80c4b`, manifest SHA `b270be3c365ad780a2988ccf16f4850c15807ebc3eb9eb763f2bcdd0418f2f74`, 596857bytes/130manifest files. 세 source가 리허설 후보와 byte-match하고 모든 hash/size/SHA256SUMS 및 7binding/mode를 Sol이 검증했다. `v7-binding-candidate-20260913/readiness.json` SHA `d5660ba7bb1e982379dcd06e7975e3960bc28cefd7bb4c2521eee54d7ead89ba`는 생성 직후 기준이며, root는 이후 Dockerfile 설명의 v6→v7 한 글자와 Changeset의 team-side 보존 설명만 보완했다. executable binding 로직은 동일하다. 이전 v6는 historical QA extractor의 사용 경로가 있어 유지한다. 다음 private-index 커밋/PR #1182 갱신은 새 archive+7bindings+Changeset+이 문서만 포함하며 공유 WIP는 포함하지 않는다. CI→dev merge→실제 Alpha image/transition/DB health 검증을 통과하기 전에는 복구 완료라고 하지 않는다. 최종 DROP과 Alpha42 Ego/API/DB 검증은 계속 미완이다.

- **r4 실제 전체 데이터 리허설 PASS:** SSM `7ee4c7fa-297e-458e-a307-f17ef31664b3`, `three-source-rehearsal-result-20260913-r1.json`에서 exitCode0/executionPass=true/cleanupComplete=true를 확인했다. 새 원본 백업 복원 후 세 reviewed source overlay로 fixtureCount310/detailsCount310/canonicalGameCount310, game/staff/audit legacy links0, historical-result game-less24, M8-before-cutover 및 M8/M9/M10 적용, legacy5tables/3columns 보존, seals5|5|3, M11 미적용을 검증했다. 로그 SHA `68bd41765076548ebad049696e42bdb40f9a1f86b37e60a52389ac16a80d9f59`, host params SHA `1b99af42f133aa554aa9bd03a28eafcb470e3ad7c1932999d67e1c6572eb025b`. **이는 세 파일 후보를 고정 v6 이미지에 overlay한 격리 DB 증거이며 v7 이미지/실제 Alpha 배포/사용자 E2E PASS가 아니다.** 다음은 packager 실패 시 temp 회수·검증 전 최종출력 방지 보완→v7 archive 생성·mode/hash 확인→PR #1182에 rawarchive+모든bindings/CIfixtures/Changeset/doc 반영→CI/Copilot→dev/Alpha다. Sol이 리허설 근거를 독립 확인 중이다. Alpha 원본 API/worker는 계속 중지 상태이며 전체0/42다.

- **TEAM_MISMATCH 근거 및 r4 진행:** read-only SSM `d64a96f8-8d6f-4c42-b6ea-16dfd53dcdf2`, `team-mismatch-read-result-20260913.json`에서 fixtures310/legacyGames286, 신청 NULL+game team 존재 홈3·원정3, 양쪽 nonnull 충돌0·0을 확인했다. Backfill은 game-side 팀을 보존하지만 source-cutover 검사는 nullable registration만 비교하는 계약 불일치였다. 세 번째 후보 `tournament-team-match-source-cutover.ts` SHA `664495d08b0d7db74965a8a9b1ea16c2d12af6f9250a1823e9c0e3c68d6a6184`는 동일 `resolveTournamentTeamIds`로 검증하며 실제 nonnull 충돌 거부와 alreadyCanonical 동작을 유지한다. Sol bounded PASS 후 r4 세 파일 overlay 리허설 SSM `7ee4c7fa-297e-458e-a307-f17ef31664b3` 진행 중. harness `376cbc857f54d7848e3ae92fd89530f1f99e40dc6b32e3797ffa85786c9e1a2d`, 압축 parameters `2e99c5236578527bc50f4164b8c187b8c5f83595840f91ac1876d2ef6887f288`, 42790bytes; explicit exit1을 적용했다. v7 packager `af8a7d59b328fc267e59864e7a10f8312e3249a24b0750bbb07b65a93434f299`는 git2f의 원본 tar/130파일을 검증하고 정확히 세 source만 바꾸는 방식으로 독립 검토 중, 아직 생성하지 않았다. 현재 열린 dev PR은 #1182 eeab9fe, #1181 ff9179a 두 건뿐이며 새 머지/Alpha 원본 변경은 없다.

- **최신 실패 지점 / r3 미통과:** R3 후보 overlay SSM `458259e9-e1c6-4010-8029-b0190018d95d`는 M8 이후 `TournamentTeamMatchSourceCutoverError:TEAM_MISMATCH`로 실패했다. 실제 실행 exitCode1/executionPass=false/cleanupComplete=true이며 SSM 자체 Success는 wrapper의 `test && ...` 뒤 출력으로 종료 코드가 가려진 버그다. PASS로 세지 않았고 다음 wrapper는 명시적 exit1을 추가했다. 원본 report SHA `21286b1a0e810daab3a6d8cc60280a68ad5c0f00ab8c69bd3bbb479de767eda9`, 실행 로그 SHA `0575a26aade7750448b11ab11e22aa41b42c851668df728d2d16825edb0e14d5`, 근거 `standings-rehearsal-failure-read-result-20260913-r3.json`. 최초 r3 전송은 SSM97KB 한도로 실행 전 거부되어 gzip 압축35482bytes/원본hash검증으로 해결했다. Luna/Sol은 TEAM_MISMATCH가 원본 데이터 문제인지 새 팀 미정 경기 import 문제인지 조사 중이며 재실행/삭제/계약 완화는 하지 않았다. v7 packager는 BSD tar 호환 Python deterministic archive 방식으로 준비만 됐고 아직 실행하지 않는다. 9행 영수증 releaseSha 누락을 추가 보완한 runner SHA `01571d404bdf0e787a49615decf8f3729bfb0529915ae5b1edf1847673086f2e`, Sol 반례 증거 `nine-row-release-binding-20260913/evidence.json` PASS. 이전 동일 릴리스 검증 PASS 설명은 잘못이었음을 정정한다. PR #1181 head ff9179a는 CI PASS와 범위 설명 보완 확인, 현재 Alpha 복구와 중첩 배포를 피하려고 미머지다. 전체 Alpha0/42 유지.

- **이관 충돌 원인 확정 / 다음 후보:** 진단 SSM `f3c045a3-e1cb-4c27-a991-9bb5a3d91be6` 성공은 **실패 원인 수집 성공**이며 cutover PASS가 아니다. `m8-rehearsal-diagnostic-result-20260913-r1.json`에 HTTP409 `CANONICAL_MATCH_REQUIRED`, `A tournament group contains a non-canonical TeamMatch.`, standings service266→historical importer258→full cutover161 스택을 확인했다. 실제 집계는 historical-result game-less24, affected groups3, 같은 조 game-less sibling을 가진 후보18이다. cleanupComplete=true. Luna 수정 후보는 batch import의 순위 계산을 전체 game 생성/source cutover 후로 옮기고 standalone의 기존 즉시 계산을 유지한다. source hashes `a33b76e49a5e7187ad04d2c47d681291a1f924f648e6449165ba998770f3064b` / `522071e50d4a8fe75864305ba20e7a66329a24b3e356bb2b488576d6ed3fadd8`, Sol source-contract PASS. 다음은 이 두 파일을 고정 v6 tool에 읽기 전용 overlay한 **후보 코드 리허설**→새 immutable v7 archive 및 hash binding→PR #1182 갱신/CI/리뷰→dev/Alpha다. overlay 리허설과 v7 이미지 배포 증거는 구분한다. PR #1181 최신 head `ff9179a8c8a79477b7182fbf19ff77f3cf3f064e`는 문서의 오래된 줄 번호와 PR 범위 설명까지 보완했고 CI/Copilot 대기다. Alpha 원본 runtime 복구·최종 DROP·42흐름은 여전히 미완이다.

- **최신 리허설 결과 — merge 보류:** 격리 SSM `30ea8c1b-be17-4508-bb4d-5c8a96295694`는 실제 백업 복원 및 M8 migration 성공(166 migrations up-to-date) 후 고정 v6 CLI의 `ConflictException`으로 실패했다. 임시 컨테이너/네트워크/볼륨 cleanupComplete=true이며 Alpha 원본 데이터는 변경하지 않았다. 실행 로그 SHA `7a344f2fad959d9333e43e00b7a393378907e2e0462a6d8039bb372e76c0eb53`, `m8-rehearsal-failure-read-result-20260913-r1.json`에 근거를 보존했다. raw CLI 보고서는 초안 cleanup에서 제거되어 원인 상세가 부족하므로 Sol이 다음 진단부터 mode600으로 보존하도록 보완하고 Luna가 ConflictException 발생 경로를 조사한다. 동일 명령 재시도·PR #1182 머지는 하지 않는다. Copilot 두 건 보완 후보 runner `f3347b8cf9b333d6f05d06e14b9f679b34ae5c4242dbb94dda4af2fd291e7f31` 및 enum 명칭 수정 Changeset `cfb4f76df6e000d97108e0351a47828faeaf4605441202a7f0fc2fb29af3474f`는 Sol 독립 PASS다. 이 작은 보완은 이관 충돌 해결을 의미하지 않는다.

- **17:04 KST 실행 커서:** 수정안 PR #1182 head `7e8392bc77dbf621c658153353424b2f17e5e9bb`, CI `34746515983` 진행 중. Copilot의 Changeset enum 명칭 오기 및 backup 존재 확인 순서 개선을 Luna가 보완한다. 실제 백업 격리 리허설 SSM `30ea8c1b-be17-4508-bb4d-5c8a96295694`가 진행 중이며 미완이다. Sol harness SHA `89197629820f3bccdef2e4c46b63e0111ec41f8aecf75e38379241816be0b523`, parameters SHA `55071a3312777b7a5ee76abae3d40a718ea1030b3d672e78691383717af777a9`. 고유 internal network/DB에서 실행하며 Alpha 원본은 변경하지 않는다. 실패한 Luna 초안은 폐기했고 Sol replacement는 `m8-precutover-rehearsal-20260913/sol/`에 있다. 호스트 사전 점검의 candidate.json 부재는 배포 실패 시 manifest를 failed/로 옮기는 정상 이력임을 확인해 정확한 failed manifest SHA `4df7e375a16d815d464ebc235166bd0aa67fa20c89cc5d386547af4380e5803c`로 해결했다. PR #1181 문서 보완은 Sol PASS 후 head `f6354241a5314620fe29774c4eec7924846a58cb`로 push했으며 CI/Copilot 대기, 아직 머지하지 않았다. Alpha 복구·최종 DROP·42흐름은 계속 미완이다.

- **최신 커서 2026-09-13 16:50 KST:** dev `2f04470b86c07d8217bf0fdebf29190600b10803`의 CI `34745135521`은 성공했으나 Alpha `34745135491`은 실패했다. 실제 PostgreSQL 오류는 `V1GameOfficialFactSourceType` 타입 부재다. 과거 결과 import가 이 타입을 사용하지만 M8이 cutover 뒤에 배치되어 있었다. 실제 DB 조회로 fixtures310, canonical details0, legacy game links286, retirement triggers0, M8/M9 미적용을 확인했다. cutover transaction은 rollback됐고 앞서 승인된 65fixture/25game 삭제는 유지된다. **API/worker 정지, 최근 health502, 새 Alpha E2E0/42**. 아래 진행 중 기록은 당시 이력이다.
- **수정 후보 / 검증 한계:** Luna 후보 `output/qa/task168/m8-precutover-fix-20260913/candidate/deploy/task168-stage-a-migrate.sh` SHA `855fd388857e2cddb3db88c935a46f51f308c185fa4a88298470f06bf0e7bba8`는 M8을 cutover 전에 실행하고 post에도 M8/M9/M10 전체 이력을 포함한다. 8행 재시도와 9행 미이관/이관커밋 상태를 구분한다. 실제 M9 SQL 실패로 invalid Prisma ledger가 남으면 자동 resolve하지 않고 거부한다. 9행 committed resume은 M9 기록 전 중단에 한정되며 모든 M9 실패 복구라고 주장하지 않는다. Sol 독립 검토와 실제 백업 격리 리허설 전에는 배포하지 않는다.
- **실제 데이터 리허설 보완:** `m8-precutover-rehearsal-20260913/` 초안은 사용 중인 deploy_default 요구, plain SQL gzip에 pg_restore 사용, Stage A에서 보존할 테이블/컬럼의 삭제 기대 때문에 반려했다. 고유 private network/DB에서 실제 310fixture 백업을 psql로 복원하고 M8→고정 v6 CLI→M9를 실행하도록 수정 중이다. production runner 전체 실행과 SQL/CLI 리허설 증거를 구분한다. Alpha 원본 DB와 기존 소유 컨테이너는 건드리지 않는다.
- **근거 및 남은 작업:** `alpha-cutover-p2010-result-20260913.json`, `alpha-cutover-postgres-error-result-20260913.json`, `alpha-cutover-type-catalog-result-20260913.json`에 실제 실패·타입·rollback 조회를 보존했다. 최신 백업 SHA `8a6e26294a38d1cab33c66c0cce691e99a907b3ac3a4cd720af2043a1b3b5911`, 3,920,102bytes. 다음은 후보 검증→새 dev PR/CI/리뷰→Alpha 재배포→실제 readback이다. PR #1181 문서 수정 후보는 준비됐지만 최종 검토/반영 전이며 복구 배포와 중첩시키지 않는다. Stage B 최종 DROP과 42개 역할별 Ego/API/DB 검증은 여전히 미완이다.

- **가장 최신 실행 커서 — #1180 dev 머지 완료:** CI `34744843149` Gates/API/Web 전체 PASS, exact head·base dev 확인 후 PR #1180을 머지했다. remote dev/merge `2f04470b86c07d8217bf0fdebf29190600b10803`, mergedAt `2026-09-13T07:24:02Z`. 새 Alpha run `34745135491`, matching dev CI `34745135521` 진행 중이다. fetch는 성공했지만 로컬 FF는 동일한 다른 세션 WIP 충돌로 거부되어 HEAD/index/파일을 강제로 바꾸지 않았다. AWS STS 재확인은 성공했다. **아직 새 이관·API 복구·42개 E2E PASS 아님.** 배포 성공 후 root는 `run-alpha-ssm-20260913.cjs`로 `alpha-recovered-stage-a-collector-20260913/parameters-2f04470b-diagnostic.json`을 읽기 전용 실행한다. 이 payload는 실제 파일 hash·DB 이력·상태·image/DB/link/mapping을 수집하며, false 검증값도 숨기지 않도록 최종 raw JSON을 보존한다. v6 report 전체 result는 대형 배열 때문에 SSM24KB를 초과할 수 있어 scalar `result.verification`만 반환한다. 원본 전체 보고서는 호스트에 보존되고 파일 hash로 연결된다. 이전 c4cc용 payload는 이번 릴리스에 사용하지 않는다.

- **즉시 재개할 PR:** [#1180](https://github.com/kim-song-jun/matchup-sports-platform/pull/1180), base dev, head `513a2bdd8ee05d06afc18a51719adaa89e66cbec`, CI `34744843149` Web/Gates PASS·API 진행 중. Copilot `5189976247`은 이 head 3/3파일 검토·새 comment0·승인 권고다. Sol `preflight-blocked-status-fix-20260913/independent-review.json` SHA `fc6b38bcc5fe69e72d0fd7ffb2b69f26ccdffe978643d98c672534a6519c4498`에서 실제 incident 기반 8/8 실행과 단일 줄 delta를 확인했다. 결과 SHA `40b96e924d9a4e3885e5f1e81c36dabb720a2f0222b7645a0c08f7a437937a2a`. 다음은 API CI 성공→exacthead/base검사→dev merge→matching dev CI/Alpha deploy다. 이전 #1179의 c4cc Alpha 실패를 성공으로 바꾸어 기록하지 않는다. 현재 API/worker정지, scoped삭제완료, 새로운 canonical Alpha0/42는 유지한다.
...
- **현재 최우선 실행 커서 — 삭제 R1 드라이런 이후:** SSM `e0244a6f-7720-4d81-b937-53b29aab6bb0`에서 구 API/worker를 정지하고 restart policy를 `no`로 바꿨다. 정책 원본·정지 영수증·새 백업은 호스트 `/home/ec2-user/.teameet-alpha-releases/task168/cleanup-20260913-r1/`에 보존된다. 새 백업 4,076,534 bytes / SHA `1263b6661495d758b803b25ac010c0e0073e474a80b8958636255ee820097f9b`. **현재 API/worker는 정지 상태이고 신규 배포·실제 삭제 커밋은 없다.** R1은 `v1_game_official_facts` 추가 append-only 트리거에서 실패해 연결 종료 시 전체 롤백됐다. 후속 실제 조회에서 대진375 / 기존 링크 경기311 / disabled trigger0을 확인했다. `alpha-cleanup-dryrun-result-20260913-r1.json`, `alpha-all-delete-guards-result-20260913.json` 참조. Sol이 관련 DELETE 트리거를 전수 확인해 공식 fact·result participant 보호를 추가 처리 중이다. 단순히 모든 트리거를 비활성화하거나 session_replication_role로 우회하지 않는다.
- **권한·비FK 종속 관계 최신 증거:** `alpha-semantic-read-result-20260913.json`에서 target outbox4(COMPLETED/GAME), watermark4(APPLIED), idempotency95(GAME84/FIXTURE8/FIXTURE_FIELD3), audit95(GAME92/FIXTURE3), result decision0을 확인해 SQL predicate와 건수를 고정했다. `alpha-staff-assignment-result-20260913.json`의 운영자 배정 `a6c73d86-5f82-4ded-bdeb-e019fa0090d4`는 FIELD_OPERATOR/활성/field없음, 삭제 scope1/남는 scope2이므로 권한 불변식을 유지할 수 있다. staff scope 제약 트리거는 유지한다.
- **다음 즉시 작업:** Sol의 수정 SQL 최종 SHA를 받으면 root가 delta 검토 후 R2 wrapper를 만든다. R1 백업/정지/재시작 정책과 파일 해시를 재검증하여 기존 백업을 재사용하되 R1 산출물을 덮어쓰지 않는다. R2도 `task168_commit=false`로 실제 삭제 후 ROLLBACK이다. 성공 시 동일 고정 SQL의 true 실행을 별도로 기록하고, 이후 canonical cutover까지 구 writer를 시작하지 않는다. `prepare-alpha-cleanup-dryrun-20260913.cjs`는 R1 생성용이므로 현재 SQL이 바뀌면 해시 가드 실패가 정상이다. 새 후보를 무검토로 pin하지 않는다.
...
- **정확한 차단 보고서 / 접근 상태:** `/home/ec2-user/.teameet-alpha-releases/task168/0d76e6b2d077e505eb746173154f9d2df5128af4/report/cutover-report.json`의`preflightReport`에실제차단사유가있다. 이실패경로에서는transition.json이생기지않으므로성공상태collector를쓰면원인확인전에실패한다. Sol이비밀값/환경/백업내용없이FAILED/PREFLIGHT_BLOCKED·비READYfixture finding·정지상태만읽는`alpha-host-read-candidate-20260913/incident-host-read-r2.sh`와parameters를준비했다(SHA `a60b7b26b6c1687b34502b37f512e4ec243ee5d8f81f9e7b3beb4659936c9a09`, `d50ae693e1061a4c1a4e33cea88cdaf24a2f7080b1dc643b8f5e1da7b610152d`). **실제finding은아직읽지못했다.** 로컬AWS로그인은재확인해도만료상태이며사용자`aws login`완료회신대기다. 기존문서상Alpha운영경로는SSM이고SSH설정에는prod별칭만있어그서버로접속하지않았다. 원격계정·키·역할변경이나진단을가장한배포workflow재실행은하지않았다. sanitized배포로그 `alpha-release-evidence-20260913/`,통합영수증 `pr1178-merge-release-20260913.json`에실패를분리기록했다.
...
- **자정 테스트 원인 / Copilot 반례 판정:** CI는 9월13일00:19KST에 실행됐고, 실패한4개테스트는9월12일을고정클릭했다. 제품의과거날짜차단이정상작동한테스트결함이며Luna가Date전용고정시계후보를준비한다. 원문test는이전78ae와6fe사이에동일하다. Sol은Copilot2건을실제경로상결함아님으로판정했다: 영상입력은저장전에parsed.url로정규화되고업로드실패정리는UploadsService의정규URL을쓰며, 대회운영대진은TeamMatch+Details+TEAM_MATCH Game을하나의transaction에서만든다. Game없는Details를정상집계에포함하는수정은이관불변조건을숨긴다. 직접DB수정으로만들수있는손상상태는잔여위험으로구분한다. 독립근거 `copilot-stage-a-claims-review-20260913.json` SHA `d502e07f3cde838d3bde5c94a7779ade77ea2cf03a6beff14d639931d7a8291d`. 이번hostedCI의Task9실제HTTP/DB도PASS이며artifact를같은CI증거폴더에내려받았다. API통합96묶음672PASS/3SKIP,단위269묶음3520PASS다. 읽기준비중잘못추정한vitest.config.ts경로는실제목록의vitest.config.mts로교정했으며제품오류나실행실패가아니다.

- **2026-09-13 최신 원격 CI / 미완 범위:** head `6fe21712d3edf52c43af9a8992d434788cec8180`, CI `34701824859`에서 Gates·API는 성공했으나 Web은 리그 일정 날짜 선택 테스트 4개가 실패했다(430개 파일 통과, 3562/3566 통과). 같은 웹 코드의 이전 CI 통과와 비교해 날짜 경계 원인을 Luna가 조사한다. 전체 CI 성공·dev 머지·이번 버전 Alpha 검증은 아직 아니다. 새 Alpha는 **0/42**, 과거 로컬 합성 42/42와 구분한다. Copilot `5186946334`의 영상 URL 정규화 및 대진 수 집계 지적 2개는 Sol이 실제 canonical 불변조건·호출 경로와 대조 중이다. 원본 로그 `output/qa/task168/pr1178-ci-34701824859/` 보존. 열린 dev PR 전수 조회 결과 #1178 하나이며 DRAFT를 유지한다. 실패한 CI를 무작정 재실행하지 않는다.

- **8f433fc9 실제 검증 r3 + Sol 독립 PASS:** exactGit1092/1092파일·후보3개byte일치,generatedClient와같은공식formatter의원문copy SHA `1351af0ffb59e43223887c8bb0a06725d6789c8722d1f77ae64c5860dd494b27` 전체일치,API build0,실제PG영상URL동시성1/1,Task9literalHTTP10/10 통과. 권한403/404두건/재요청200 replay=true/변경payload409,DB거절무변경·A확인/B해결version1·감사/멱등기록정확2·추가변경0 확인. 소유container/anonymousvolume/archive/DB/port/PID잔여0,Node64→64,지정cachedClient/메타20개전후불변. 근거 `task9-video-commit-proof-20260912-r3/`, 독립리뷰 `task9-video-runtime-independent-review-20260913.json` SHA `39231170b42fe90bc8b3219b40eee23f9100026eef12504a1276a823785b4322`. local syntheticCI재현이며hostedCI/Alpha판정과구분한다. 새원격CI/Copilot→dev머지→Alpha42개→별도finalDROP릴리스가남아있다.

- **8f433fc9 로컬커밋/실행준비 판정:** 정확히gate/Task9/videohelper/실제PGspec/이문서5경로를private index로커밋했고공유HEAD/index불변을확인했다. 첫시도는신규test등록의`update-index --add`누락으로커밋전에중단해보정했다. 실제실행r1/r2는소유Prisma generation만exit0 뒤검증helper에서중단됐고API/DB/HTTP미시작·PID잔여0·archive삭제·기존Client/cache불변이다. r1의rawschemaSHA검사와r2의orderedtoken검사는Prisma가V1TeamMatch/V1Tournament의@@unique를@@index앞으로재정렬해실패했다. 보존된r2schema를대조한결과23567tokens양쪽동일·block추가삭제0이며두block선언순서만다르다. 원문copy에같은공식Prisma formatter를적용하고generated파일과fullbyteSHA를대조하도록helper를보완중이다. 검증실패를제품버그나API테스트실행으로부풀리지않는다.

- **Task9·영상잠금 소스 독립 PASS:** Luna의후보는같은UUID의TeamMatch+Details와TEAM_MATCH Game을만드는준비데이터수정,공통64비트 `video-url:` 잠금,실제PG경합검증1개다. Sol이기존10개HTTP/DB/권한/멱등성/충돌/cleanup단언이유지됨을확인했다. 보고서 `task9-video-independent-review-20260912.json` SHA `359c7314985b6b3b834fb06f960df580c4537c326990b7a9c1122f2149c31505`. 실행helper도Client격리·소유processgroup TERM/KILL·컨테이너label/백업cache불변검사를검토했다. 공유캐시의generatedClient가최종DROP용인것을확인해그대로재사용하지않고이번커밋의active schema로소유Client를생성한다. 의존성전체의OS읽기전용이나전체hash검증을주장하지않으며,generatedClient디렉터리/지정메타의전후검사범위를명시한다. 실제build/PG/HTTP실행전판정이다.

- **이관 SQL 독립 검토 및 gate 후보 PASS:** Sol은 active10개 migration의 정확한SHA256·중지/백업/1–7+10→cutover→8/9 순서·비어 있지 않은기존DB 증거를 대조해 SQL 수정 없이31개 정확한 문장 등록을 승인했다. 이는Sol검토이며 사용자 개별SQL검토로 가장하지 않는다. 최종DROP승인·Alpha실행PASS가 아니다. root의r2후보는 기존검사알고리즘/93개항목을유지하고31개만추가해 self-test/실제dd4d..78ae gate가PASS,첫등록제거부정대조는예상exit1이다. `stage-a-migration-gate-independent-review-20260912.json` 및 `ci-migration-gate-repair-20260912-r2/receipt.json` 참조. r1생성helper의String.replace가SQL달러인용을변형한실패는replacement callback으로교정했고원본실패로그를보존했다. 실제migration SQL은변경없다.

- **78ae123f 원격 CI34699157675 확정:** API 단위269묶음3520/3520, 통합95묶음671PASS/3SKIP, API 빌드, Web431묶음3566/3566 및 빌드가 통과했다. 이전 Alpha release-state/복구 검사도 통과했다. 남은 실패는 (1) Task168 신규 SQL의 검토 등록 누락으로 expand-contract gate 거부, (2) Task9 실제 HTTP/DB QA 도구의 `undefined.createMany`다. Sol의 정확한 parser 재현은 거절31개이며 Luna의 의미별 목록33개와 혼동하지 않는다. 같은 diff 안의 함수 교체도 안전성 검토에 포함하되 등록은 실제 거절문만 대상으로 한다. 각각 Sol SQL 독립 검토와 Luna의 canonical harness 수리를 진행한다. 기존 SQL의 안전성 검토 없이 목록만 추가하거나 QA 단언을 삭제해 통과시키지 않는다. 원본 로그 `pr1178-ci-34699157675/` 보존. 아직 dev 미머지·새 Alpha0/42다.
- **dev PR 전수 재조회 / Copilot:** 열린 dev 대상 PR은 #1178 하나이며 head78ae123f/DRAFT다. 최신 Copilot은136/335파일 부분 검토에서 영상 URL 잠금의32비트 `hashtext`와 namespace 부재를 지적했다. root가 모든 등록/삭제/worker가 같은 helper를 사용하는 것을 확인했으며, 실제 경합 계약을 유지하는 수정 범위를 검토 중이다. 이전 부분 검토의 inline0을 전체 승인으로 해석하지 않는다.

- **API 후보 r6 독립 검토 PASS:** Sol이 exact a709 기준 1092개 경로를 독립 대조해 원본 1082개·수정 10개, 누락/범위 외 변경/해시 오류 0을 확인했다. manifest SHA `2aa7e086f668b00aedcfba5f17e1f8e1727f267bf4b4a04783e601b9688669e0`. 실행 판정은 9개 파일 162건과 마지막 후기 파일 53건을 합친 `PASS_COMPOSITE_TARGETED`이며 새 CI 전체 PASS와 구분한다. 리뷰 `ci-api-unit-contract-independent-review-20260912.json` SHA `4487d2bf30fb903fec26e0ad6f01de14b0a0301fbf0f885cc3e4c78085abfdda`. 현재 prod 변경은 닉네임 표시 수정 1개이며 기존 경기·권한·오류 계약을 바꾸지 않았다.
- **API 수정 최종 좁은 실행 PASS(합성 증거):** 후보r6의1092개 유효 파일(원본1082+수정10) 전체 해시를 잠근 뒤 root가 마지막 변경인reviews 단위 파일만 실행해53/53을 확인했다. 이전r6 실행에서 통과한 다른9묶음162건은현재 후보와byte-identical이므로 합쳐10묶음215건의계약을 증명한다. 한 번의215/215 전체실행PASS라고 쓰지 않는다. 근거 `ci-api-unit-contract-repair-20260912/{candidate-manifest-r6,candidate-integrity-r6-receipt,root-final-review-unit-receipt}.json`; PID36347 및자식group회수, 실행전후1092파일불변. 다음원격CI에서 전체API를다시검증한다.
- **반복 수정 오류의 처리:** Terra 후보에서중첩select 괄호누락,transaction delegate/lineup fixture누락,담당경기정렬기대오류를 발견했고 모든실패로그를유지했다. 마지막sportId transaction의findFirst 추가가동일모양의다른fixture에여러번적용되어 root가worker를중지하고소유권을회수했다. 중지직전정확한객체에들어간마지막한줄을직접확인해r6로고정했으며그뒤추가제품수정없이해당파일만재검증했다. 미완후보나오래된manifest를독립PASS/PR근거로사용하지않는다. 실제production변경은표준닉네임표시helper를적용한mock-seed1개,나머지9개는테스트계약보완이다.
- **독립 리뷰의 잘못된 추가 요구도 교정:** 필드 테스트의 옛 리그 거절을 정상 리그 허용으로 바꾸며 root/Sol이 대체unsupported-kind 부정 테스트를 제안했으나, 실제V1CompetitionKind는regular_tournament/regular_league 두 값뿐이고 둘 다 허용된다. root가 스키마와ALL_COMPETITION_KINDS를 대조해 가짜 세 번째enum fixture 요구를 철회했고 Sol도 수용했다. 실제로 가능한 없는/삭제된 대회404와미기록, CAS409와감사미기록을 검증한다. 리뷰 의견도 현재 코드·데이터 계약과 맞지 않으면 그대로 적용하지 않는다.
- **배포 상태 테스트 후보 독립 PASS:** Terra가 구expand-contract 명세를 만들던 Alpha 테스트2개를 현재StageA 명세로 맞췄고 기존 부정 테스트와wrong-stage 거부를 보존했다. Sol은 정확한 후보 해시·실행권한100755·생성기/검증기 정합을 확인했고 Linux Bash5.2에서 과거trap 결함 재현 및alpha/prod 복구 동작을 증명했다. macOS의Alpha 전체 테스트는 소유synthetic sudo passthrough로PASS이며 Linux 전체Alpha 실행은 캐시 이미지에rsync가 없어 새CI에서 확인한다. 소유 컨테이너 회수 완료, 실제deploy 코드0변경. 리뷰 `ci-alpha-release-state-independent-review-20260912.json` SHA `30c09218f500062405e38549ac5e6617e322aa161b279889ce3b022eddc712c5`.
- **API 단위 회귀 수정 진행:** exacta709의1092개 API/config blob을 확인한 소유 archive에서 원래9개와기존mock-seed까지10묶음213건 중30FAIL을 재현했다. 실제 코드 결함은 mock-seed의realName 직접복사1개로 분류하고 표준participantDisplayName/profile선택을 적용한다. 나머지는정본 소속/권한/transaction/current identity link/응답shape fixture를 맞추되 기존 보호조건은 유지한다. archive 준비 과정의 빈 목록·symlink 탐색·stale tar·없는 선택 파일·중복 manifest 오류는 보존하고 최종해시 확인 후에만 테스트했다. 수정 후 실행과독립 검토 전이다.
- **BEFORE 지적과 현재 후보 대조:** a709의 `result-edit-modal.tsx:817` 삭제 버튼은 이미red700,880의 골키퍼 label은minHeight44이며 `corrections-page-client.tsx:155–157`은 가시적 패널에focus-visible outline을 둔다. 이전 Alpha 화면 지적을 새 미수정 결함으로 중복 집계하지 않는다. 배포 후 실제 색 대비·체크박스 label 터치 영역·선택 행과 패널의 키보드 포커스 이동을 다시 측정해야 하며 소스 속성만으로 시각 PASS를 주장하지 않는다.
- **Alpha BEFORE 검수 완료(새 배포 PASS 아님):** Sol이 현재 존재하는 대회12dad010/경기7b818ffb에서 정정 모달·공개 일정/기록·공개 경기 CTA의3화면×390/768/1440을 실측하고22개 스크린샷을 보존했다. 가로 넘침0, 정정 모달 내부 끝의 제출 버튼 노출,44px 동작 버튼 및 공개 CTA 가림0을 확인했다. 삭제 글자 대비3.71:1, 선택 행의 outline/boxShadow 없음은 AFTER 재검수 항목이며, 선택 테두리와 별개인 키보드 포커스 표시를 비교해야 한다. 비활성 컨트롤4.19:1은 측정값으로 남기고 그 수치만으로 활성 컨트롤의 대비 위반으로 확대하지 않는다. 현재 정보 모달에 피리어드 편집 없음, 새 인라인은 미배포다. 명시적 API3건200만 증명하며 global console/network clean이나 effective release SHA는 미증명이다. space260/p1 종료·unmanaged0·해당 Ego 프로세스 잔여0. 근거 `alpha-before-stage-a-20260912/baseline.json`; 새 Alpha42개 계수는0/42 유지.
- **원격 a709b0f4 / CI34696640194 판정:** Web 작업은 테스트·빌드 PASS, API 통합95묶음671PASS/3SKIP가 통과했다. 이어 실행된 API 단위269묶음 중260PASS/9FAIL,3518건 중3488PASS/30FAIL이며, Gates는 앞선 release contract20/20 통과 뒤 `test-alpha-release-state.sh`에서 진단 없이 exit1로 실패했다. 두 실패 범위를 exact Git 후보에서 조사·수정 중이며 CI 전체 PASS나 dev 머지를 주장하지 않는다. 원본 로그는 `pr1178-ci-34696640194/`에 보존했다. Copilot의 a709 검토는136/328파일·추가 inline0건으로 부분 검토이고 배포 위험에 대한 추가 검토 의견이 있어, 독립 검증과 실제 전환 증거를 대신하지 않는다. 열린 dev PR 전수 재조회 결과 #1178 하나다.
- **Ego 알림 해석 교정:** Sol이 업데이트 notice를 필수 중단으로 해석해 BEFORE 검수 space258을 조기 종료했다. 실제 스킬455–458은 현재 브라우저 작업을 완료하고 업그레이드는 승인 후에만 실행하라는 규칙이며, 현재 버전의 검수 금지 규칙은 아니다. root가 원문 확인 후 현재 버전으로 같은 미완 baseline을 재개하도록 지시했다. 업데이트는 실행하지 않았다.258은 closedSpace=true/unmanaged0으로 회수됐고 초기 역사적 URL의 bracket404 관찰만 있으며3화면×3viewport 완료 또는 새 Alpha42개 통과로 계산하지 않는다.
- **수정 커밋3e9d53eb 실제 검증 PASS:** source2494파일을 exact commit과 대조한 archive에서 API surface/Web pattern, 실제PG16의167개 migration, CI와 같은 competition-config bootstrap 및 award dry-run, 문제 범위10integration파일76/76(27.347s), 관리자 단위71/71(7.362s)가 통과했다. 격리 clone은 테스트 종료 때 제거됐고, 소유 template DB만 남은 것을 확인한 뒤 컨테이너·볼륨·archive를 모두 제거했다. Node13→13. 근거 `ci-integration-commit-proof-20260912-r2/receipt.json`. r1은 root 실행기의 config bootstrap 누락으로 FK가 실패한 불완전 harness였으며 원본 실패 receipt와 별도 원인 JSON을 보존했다. 이를 제품 회귀나 통과 증거로 계산하지 않는다.
- **보호된 migration 복구 r2 실제 증명 PASS:** Sol은 실제PG16+Prisma CLI로 인식된 두 실패 서명의 rolled_back_at 기록을 확인했다. 첫 서명 인식/둘째 미인식 조합은 exit1이며 두 ledger 행 전체 before/after SHA `d5d1211a803281657112661ddb0e0dad121cb8c3ffda50b9959c80aca8e84986`가 동일하다. 두 대상을 먼저 모두 읽기 검증한 뒤에만 적용한다. 컨테이너/네트워크/포트55672 회수 완료. 증거 `protected-recovery-sol-proof-20260912/real-pg-proof.json` SHA `bb7eed901cf29a81a77cdaf6130dcc087954e8f1c793f75a40a5c323b8cbe0e2`. 이번에는 helper를 실제 DB로 증명했으며 전체 cutover/pg_dump는 반복하지 않았다. 인증·quiesce·backup의 선행 순서는 소스 검증, 전체 전환은 이전 정상/committed-error 리허설 증거로 구분한다.
- **스키마 snapshot 정합:** game-schema.fixture의 최종 삭제 스키마 pin을 현재 활성 Stage A schema의 실제 SHA `91222f64cf30dd15169a17cf5eb096c446861c5f578a31c51d44c92b3a321f3f`로 맞췄다. assertion copy인 schema.stage-a.prisma와 byte-identical이며 immutable migration pin은 그대로다. 정적 Sol 검토 및 실제 game-schema suite 통과를 확보했다. main/프로덕션·최종 DROP·새 배포본 Alpha42개는 계속 미완이다.
- **현재 CI 후속:** run34694939185의 Web431파일·3566테스트 및 실제109개 static page 빌드는 PASS다. API는95파일 중85PASS/10FAIL,625테스트 중606PASS/16FAIL/3SKIP이며, 전체 결과를 PASS로 보고하지 않는다. nullable 일정 테스트4개, canonical 소속 fixture3개, SQL check/enum fixture2개, 리그 순위 응답의 registrationId omission1개로 원인을 분류했다. 새 후보는1f6dc48d에서만 재구성했다. Copilot 추가 count 최적화·가시적 키보드 포커스 후보도 준비했다. 배포 복구는 검증→첫 resolve→다음 검증의 부분 변경 위험을 Sol이 차단해, 두 대상 모두 읽기 검증한 뒤에만 resolve하는 r2로 수정했으며 실제 PG 증명 전이다.
- **공유 경계 사고 및 회수:** Luna가 output-only 범위를 어기고 공유8개 파일을 수정했다. 즉시 중지하고 after snapshot을 격리한 뒤 해당 에이전트의 hunk만 제거했다. 7개 파일은 기록된 pre-edit Git blob과 동일, serializer1개는 유일한 한 줄 수정의 정확한 역변환, 다른3개 파일은 snapshot과 동일함을 검증했다. 기존 공유 WIP는 보존했고 이 후보와 manifest는 PR 증거에서 제외했다. 근거 `ci-integration-ownership-incident-20260912/{snapshot,recovery}.json`. root의 새 `ci-integration-contract-repair-r2-20260912`는10개 파일을 exact git base에서만 만들었다. 반복된 경계 위반 때문에 후속 제한 구현2건은 Terra로 배정하고 Sol 독립 검증을 유지했다.
- **Alpha 재검증 준비:** `alpha-stage-a-20260912/queue.json`에 정본의30역할+12경계 ID와 경로·행동·재조회·화면 크기·fixture 정리 조건을 정리했다. 새 Phase3 배포 기준42개 모두 pending이며 자격증명은 승인된 문서 경로/필드명만 참조하고 값은 복사하지 않았다.
- **PR 최신 원격1f6dc48d:** 23개 파일의 후속 수정(+147/-60)을 기존 PR #1178에 정상 fast-forward push했다. 공유 dev HEAD는 dd4d 그대로이고 main/프로덕션 변경은 없다. Sol 최종16파일 hash 검토 PASS, review SHA256 `2ea5977f816ed41003caa118cfbbc06cb2b6fd257c685366102736649e5dc535`. 새 CI `34694939185`는 API surface와 Web tsc/pattern을 통과한 뒤 API migration replay 및 Web unit을 실행 중이다. Gates는 기존 nullable-goalkeeper/played-at 복구 순서를 검사하는 `versioning.contract.test.mjs`의 `recovery < deploy`2건에서 실패해 실제 Stage A 실행 순서와 대조 중이다. Copilot 새 head 재검토 요청도 전송했다. 이 상태에서 머지하지 않는다.
- **CI/Copilot 수정 검증:** exact 후보0b7a1612의1090개 API 파일 hash를 검증한 별도 archive에서 surface 검사와 리그 일정·관리자 집계2개 단위 파일79/79가 통과했다. archive를 제거했고 Node 수14→14를 확인했다. Sol이 API/Web16개 Copilot·패턴 수정 파일을 검토했고, 후보에서 누락된 기존 `teamMatchesRepointed: 0` 기대값3개는 복원한 뒤 테스트했다. 원격 push 전 root가 schedule radius10→16 변경을 추가 발견하여 canonical token 지침의10→12(control),6→8(chip)로 정정했다. 글자 토큰14/12px은 기존 크기와 같지만 radius는 동일 픽셀이 아닌 지침에 따른 정규화이므로 Alpha에서 실제 화면을 재검수한다. 근거 `ci-api-commit-tests-20260912-r1.json`, `copilot-ci-independent-review-20260912.json` 및 Web 후보 receipt. CI 전체와 Alpha 검증은 여전히 미완이다.
...
- **2026-09-15 KST — M11 이후 대회·정규 리그 처음부터 재생성, 전 구간 실사용자 E2E 1회 통과:** 사용자가 기존 팀·사람은 보존하고 대회 데이터만 처음부터 다시 만들라고 직접 지시했다(백업 결정 필요 없음). PR-B #1199 머지로 배포 경로가 정상화된 뒤, Alpha에서 정규 대회 1건·정규 리그 1건(제목 "(테스트)" 접두)을 관리자 화면으로 새로 만들고 실제 계정으로 신청 → 선수 명단 등록(등번호+이름) → 운영 확정 → 조 편성("직접 입력" 안에 숨은 팀 검색 → "담은 팀" → "N팀 배정" 커밋 버튼 — 상태 표시가 아니라 실제 배정 트리거임을 DB 대조로 확인) → 대진 자동 생성 → 운영 콘솔로 경기 시작 → 득점 기록 → 전후반 진행 → 경기 종료 → 결과 확인·확정 → 공개 대진표 반영까지, 그리고 리그는 라운드로빈 대진 생성까지 전 구간을 1회 통과시켰다. 각 단계는 DB 직접 대조로 검증했다(`v1_tournament_group_teams` 행 수, 신청 상태, 명단 행 등). 위 단계 순서가 재현에 필요한 전부다 — 세부 화면 좌표·스크린샷 등 부가 자료만 저장소 밖 개인 메모리(`alpha-competition-rebuild-recipe`)에 추가로 남겼다. 이 통과는 M11 이후 스키마에서 대회 생성부터 결과 확정까지의 핵심 경로가 실제로 동작함을 증명하지만, **42개 역할별 흐름 + 12개 경계 상황의 전수 재검증은 아니다** — 아래 두 항목 때문에 의도적으로 보류했다.
- **2026-09-15 KST — 레퍼런스 벤치마크 P0 중 사용자가 선택한 1건 구현·머지:** 2026-09-12 벤치마크 감사(10사이트 실측, 전체 아티팩트는 저장소 밖 메모리 `reference-site-benchmark-2026-09-12`)의 P0 3건 중 사용자가 "② 비활성 CTA 사유 표준 컴포넌트"만 선택했다(①참가비/환불 정책 노출, ③네이버 카페 GTM은 이번 라운드에 미선택). `apps/v1_web/src/components/v1-ui/blocked-action.tsx` 신설 — 비활성 버튼의 사유를 `aria-label`에 욱여넣던 기존 패턴(WCAG 2.5.3 Label in Name 위반: 접근성 이름이 화면 라벨과 달라짐)을 버리고, 보이는 라벨은 그대로 두고 사유는 `aria-describedby`로 연결된 별도 텍스트로 뺐다. 대회 상세의 신청 마감 CTA를 이 컴포넌트로 이전(PR #1207, dev `6cb09059b`). 같은 라운드에서 이벤트 허브 문구("이벤트 허브"→"팀밋이 여는 대회")와 리그 상세의 실제 편성과 불일치하던 문구("서로 한 번씩 맞붙어요"→"맞붙는 횟수는 시즌 주차 수에 따라 달라져요")도 함께 고쳤다(PR #1206). Alpha 배포(`6cb09059b` 서빙 확인) 후 세 문구 변경 모두 실화면에서 신규 문구 존재·구 문구 부재를 확인했다. `BlockedAction`은 alpha에 신청 마감 CTA가 노출되는 대회가 현재 없어(status=open 대회는 전부 마감일 경과 → 이미 다른 비활성 분기) 실화면 검증은 못했고, 영구 삭제 불가능한 테스트 픽스처를 새로 만들면서까지 강행하지 않았다(단위 테스트 2건으로 aria-describedby 배선만 검증).
- **2026-09-15 KST — 기준선 UI 감사 결과와 42흐름 재개 조건:** M11 적용 직후 alpha 새 데이터로 전 흐름 UI/UX 재검수를 했고, critical 3건(M-T 팀 신청 명단에서 member 권한에게도 수정 CRUD가 활성 노출 / M-A 시상 화면에 수상자 프로필 링크 0건 / M-M 팀 일정 상세에 상대팀·장소 정보 누락)과 major/minor 다수를 발견했다. **critical 3건 중 최소 M-T는 이미 다른 세션이 메인 작업트리에서 직접 수정 중임을 diff로 확인했다**(`tournament-roster-client.tsx`에 `canManageRoster`/`useV1TeamDetail` 기반 owner/manager 게이트가 미커밋 상태로 존재). 중복 작업을 피하기 위해 이 세션은 그 파일들을 건드리지 않았다. **42흐름 전수 재검증은 이 critical 3건이 dev에 머지된 뒤로 미룬다** — 지금 돌리면 이미 알려진 결함을 다시 발견만 반복하고, 그 결함들의 수정이 다른 흐름에 준 영향(예: 명단 권한 게이트가 팀장 흐름에 준 회귀 여부)을 놓친다.
- **main 승격 PR #735 갱신:** M11이 dev에 들어오면서 승격 위험 범위가 "M1–M10 없음"에서 "M1–M11(DROP 포함) 없음"으로 넓어졌음을 반영해 경고 문구·현재 상태·남은 작업 절을 갱신했다(머지는 여전히 사용자 전용, 에이전트는 실행하지 않음).
