...
- [x] SafeImage 컴포넌트를 `next/image` 기반으로 리팩토링
- [x] `next/image`의 `priority` prop이 `<link rel="preload">` 를 자동 주입하도록 동작
- [x] SafeImage의 기존 인터페이스(src, fallbackSrc, alt, onError, className 등) 호환 유지
- [x] `next/image` 필수 속성(width/height 또는 fill) 적절히 처리
- [x] 12개 파일, 23개 SafeImage 사용처 전부 정상 렌더링 확인
- [x] 42개 파일의 모든 `<input>`, `<select>`, `<textarea>`에 `<label htmlFor>` + `id` 연결
- [x] 시각적으로 label을 숨겨야 하는 경우 `sr-only` 클래스 사용
- [x] `tsc --noEmit` 통과
- [x] 기존 테스트 전체 통과 (`pnpm test`) — 24 suites, 247 tests
