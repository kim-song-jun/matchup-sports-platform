# Task 77 — Test Infra Upgrade (Vitest 1→3, jsdom 24→26, @swc/cli 0.7→0.9)
...
| `@swc/cli` | 0.7.10 | 0.9.x |

현재 전체 테스트 스위트 43 파일 / 352 테스트가 통과 중이며 알려진 장애는 없다. 이번 업그레이드는 **순수 chore** 로 사용자 기능 변경 없음. 개별 패키지를 점진적으로 올리면 패키지 간 호환성 추적이 어려우므로 한 PR 에서 조율된 업그레이드를 단행한다.

---

## Goal

프론트엔드 테스트 인프라를 현재 안정 버전으로 올리고, 기존 352개 웹 테스트와 638개 API 테스트 + 42개 통합 테스트를 모두 그린으로 유지한다. DX 회귀(HMR 속도 저하, watch 반응성 악화)를 발생시키지 않으며, CI 소요 시간은 기준선 대비 10% 이내를 유지한다.
...
- [ ] **C6** API `@swc/cli` 0.7.10 → 0.9.x
- [ ] **C7** 웹 352 + API 638 + 통합 42 테스트 전량 그린
...
### S1 — 개발자 단위 테스트 실행
개발자가 `cd apps/web && pnpm test` 실행 → 352개 테스트 통과, 기존 대비 수행 시간 동일하거나 단축.
...
### Happy path
- C1~C3: 업그레이드 후 기존 352개 테스트 그대로 통과, assertion API 변화 없음
- C6: `nest build` 가 `@swc/cli` 0.9 로 정상 컴파일 (`dist/` 출력 동일)
...
### Error cases
- `@swc/cli` 0.9 가 기존 NestJS CLI (`nest build`) 와 충돌 시: `nest build --watch` 로 전환하거나 `@swc/core` pin
...
**Track C — @swc/cli 0.9 API build** (backend dev):
...
1. `vitest@3.x`, `jsdom@26.x`, `@swc/cli@0.9.x` 가 lockfile 에 반영됨
2. `cd apps/web && pnpm test` 352개 통과
...
| R5 | `@swc/cli` 0.9 와 `@swc/core` 1.x 버전 불일치 | Low | `@swc/core` 같이 latest 로 올리거나 peer dependency 확인 |
