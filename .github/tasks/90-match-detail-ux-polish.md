...
- [x] 팀 매치는 신청 마감, raw `closed`, 상대 팀 확정(`matched`) 상태여도 경기 시작 전까지 일반 목록에 신청마감으로 노출한다.
- [x] 개인·팀 매치는 경기 시작 시각 이후 일반 탐색 목록에서 제외한다.
- [x] 개인·팀 추천순과 홈 추천은 신청 마감이 지나지 않은 raw `recruiting` 매치만 노출한다.

## 2026-09-07 Application Availability Ordering Hotfix

- [x] 개인 매치 목록은 신청 가능한 카드를 먼저, 신청마감 카드를 아래에 노출한다.
- [x] 팀 매치 목록도 동일하게 신청 가능한 카드를 먼저, 신청마감 카드를 아래에 노출한다.
- [x] 각 상태 그룹 안에서는 API가 제공한 최신순·마감임박순을 유지한다.
- [x] cursor `더 보기`로 페이지를 누적한 뒤에도 로드된 전체 목록을 다시 정렬한다.

## 2026-09-07 Team-match CTA State Hotfix

- [x] 팀매치 신청 CTA의 라벨과 동작은 원시 `status`가 아니라 서버가 마감 시각까지 반영한 `displayState`를 따른다.
- [x] `status=recruiting`, `displayState=closed`인 마감 매치에서 로그인·팀 생성·신규 신청으로 이동하지 않는다.
- [x] 기존 신청자는 신청서가 살아 있으면 마감 이후에도 자신의 신청을 철회할 수 있다.
- [x] 모집 중인 경우에만 비로그인 로그인 유도, 무팀 사용자 팀 생성 유도, 신청 가능 팀의 신규 신청을 제공한다.
...
|---|---|
| 호스트 팀 운영진 | 매치 관리 |
| 신청 대기(`requested`) | 해당 신청 철회 |
| 승인 완료(`approved`) | 상태 표시, 신규 신청 없음 |
| 신청 마감·상대 확정·취소·종료 | `신청 불가`, 이동·신규 신청 없음 |
| 모집 중 + 비로그인 | 로그인 후 현재 상세로 복귀 |
| 모집 중 + 소속 팀 없음 | 팀 생성으로 이동 |
| 모집 중 + 신청 가능 팀 | 선택된 팀으로 신청 |
| 모집 중 + 부적격 팀 | 사유 표시, 신규 신청 없음 |

### Progress Snapshot

- 2026-09-07: 운영 상세에서 `status=recruiting`, `displayState=closed`인데 `신청 불가` 클릭 시 `/teams/new`로 이동하는 회귀를 재현했다. dev의 표시 상태 게이트를 운영 데이터 조합으로 회귀 테스트하고 main에 최소 핫픽스로 승격한다.
