...
- [x] Added additive `v1_user_profiles.real_name` schema field and migration that preserves every non-blank legacy `display_name`, including values equal to `nickname`.
...
- 2026-07-17 production preflight: all three target migrations are unapplied and no target-column drift exists. All 72 production profiles have a preservable legacy `display_name`; the `real_name` migration now backfills all 72, including the 44 values equal to `nickname`.
- 2026-07-17 isolated PostgreSQL rehearsal: all non-blank legacy names, including same-as-nickname and `k_` fixtures, were copied to `real_name`; blank names stayed null and legacy name/nickname values were unchanged.
