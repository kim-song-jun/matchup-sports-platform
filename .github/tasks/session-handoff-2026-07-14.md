...
- [x] 기존 dev DB는 migration history 47개에서 누락된 canonical SQL 5개를 한 transaction으로 적용했다. reset/seed/db push는 실행하지 않았고, 현재 history는 이전 extra migration을 포함해 52개이며 tournament row는 2개로 유지됐다. 이전 disposable production-clone 검증은 historical evidence일 뿐 현재 runtime 기준이 아니다.
...
- [ ] Codex Security standard 전체 scan `ac898bd4-fc5c-4024-9360-9ef52bb9162e`은 workspace `1a52d3a8-1d57-434b-881a-9430e0460095`에서 reporting 단계다. 152/152 파일을 검토했고, validation 44/44와 attack-path 44/44를 완료해 최종 reportable finding 21건(Critical 1, Medium 11, Low 9)으로 정리했다. 취약점별 distributable write-up과 hardening portfolio, sealed final report가 남았다.
...
- 캠페인 관리자 API는 기존 `V1AuthGuard`를 상속한다. production은 기본적으로 header auth를 fail-closed하지만, `V1_ALLOW_HEADER_AUTH=true`를 켜면 client-controlled `x-v1-user-*` 헤더로 관리자 가장이 가능하다. signed server session/JWT 전환 전에는 캠페인 관리자 기능을 production-ready로 승인하지 않는다.
- 업로드 저장소는 현재 public unguessable URL만 제공하고 ownership/attachment row, delete/retention/orphan collector, malware scanner가 없다. MIME spoof 차단은 적용했지만 private-media 또는 production-ready upload system으로 승인할 수는 없다.
...
2. **Visual QA**: 서버 불안정으로 Playwright 라이브 캡처 미실시. 다음 세션에서 서버 안정 후 390/768/1440 QA 실시 필요.
3. **성별 쿼터(Phase 2)**: `genderCategory`/`genderMinMale`/`genderMaxMale`/`genderMinFemale`/`genderMaxFemale` 필드가 아직 backend에 없음. `feat/v1-tournament-gender-quota` 브랜치에서 재조정 후 dev 머지 필요.
