...
**공개 캠페인 소개 모션:**
- 히어로의 제목과 주요 CTA는 첫 페인트부터 읽을 수 있어야 하며 scroll reveal 뒤에 숨기지 않는다
- 히어로 이미지는 최초 진입 때 `transform`만 사용하는 800ms 이내의 단일 settle 모션을 허용한다
- 본문 섹션은 기본 상태에서 항상 보이게 두고, 지원 브라우저에서만 view timeline 기반 `opacity + translateY(8px 이하)` 진입을 240ms 이내로 적용한다
- `content-visibility`로 캠페인 본문을 숨기거나 스크롤 중 뒤늦게 materialize하지 않는다
- `prefers-reduced-motion: reduce`에서는 모든 캠페인 transform·stagger를 제거하고 최종 상태를 즉시 표시한다
