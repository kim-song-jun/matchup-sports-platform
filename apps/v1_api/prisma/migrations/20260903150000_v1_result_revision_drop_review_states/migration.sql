...
-- `change_requested` 하나로 묶어 보여주고 있었다(`league-result-stage.ts`).
--
-- **그래서 "보는 값이 안 달라진다" 는 얼린 갈래에만 맞다** — 정확히는 이렇다:
--   · `CHANGE_REQUESTED` 로 얼린 행 → 단계가 그대로 `change_requested` 다(셋이 원래 여기로
--     묶였으므로 화면에 변화가 없다).
--   · `SUBMITTED` 로 되살린 행 → 단계가 `awaiting_approval` 로 **바뀐다.** 그게 의도다:
--     아직 확정 안 된 그 경기는 이제 어드민 확인 대기 목록에 다시 올라와야 한다(정본 §4 의
--     "보내기 → 확인" 한 단계). 그 목록에 안 뜨면 아무도 그 경기를 처리하지 않는다.
--
