  ...
  async getTeamMatch(user: V1AuthUser, teamMatchId: string) {
    ...
    const row = await this.prisma.v1TeamMatch.findUnique({
      ...
      select: {
        ...
        sport: { select: { name: true, code: true } },
        minSportLevel: { select: { name: true, code: true } },
        maxSportLevel: { select: { name: true, code: true } },
        region: { select: { name: true } },
    ...
    return {
      ...
      minLevelCode: row.minSportLevel?.code ?? null,
      maxLevelCode: row.maxSportLevel?.code ?? null,
  ...
  async listTeamMatches(user: V1AuthUser, query: AdminTeamMatchListQueryDto) {
    ...
    const [rows, statusGroups] = await Promise.all([this.prisma.v1TeamMatch.findMany({
      ...
      select: {
        ...
        league: { select: { id: true, title: true } },
        tournament: { select: { id: true, title: true } },
        _count: { select: { applications: { where: { status: 'requested' } } } },
      },
    }), this.prisma.v1TeamMatch.groupBy({
      by: ['status'],
      where: statusFacetWhere,
