 ...
 * `OfficialRevisionRowRaw` shape from
 * `v1_game_result_revisions` joined through its canonical TeamMatch, Details,
...
export function officialRevisionRowSelect(): Prisma.Sql {
  return Prisma.sql`
    SELECT
      ...
      COALESCE(team_match.start_at, revision.official_at) AS "playedAt",
      ...
      tournament_details.team_match_id AS "tournamentTeamMatchId",
      team_match.league_id AS "leagueId",
      team_match.tournament_id AS "teamMatchTournamentId",
      tournament_details.tournament_id AS "detailsTournamentId",
    ...
    LEFT JOIN v1_tournament_match_details tournament_details
