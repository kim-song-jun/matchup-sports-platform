 ...
 */
/**
 * 승부차기를 **저장해도 되는가**를 판정하는 단일 관문. `end`·정정 두 레인이 모두 이 함수를
 * 통과한다.
 *
 * 왜 하나로 합쳤나 — 게이트가 레인마다 따로 있으면 한쪽에만 붙은 규칙이 다른 쪽의 우회로가
 * 된다. 실제로 그렇게 됐다: 킥 수 필수 가드가 `end` 레인 서비스 메서드 안에만 있어서
 * **같은 값이 한 레인에선 422, 다른 레인에선 201로 저장**됐다(2026-08-18 알파 교차 측정).
 ...
 * 전에 저장된 리비전의 정정·복구가 영구히 막힌다. 그래서 "승부차기를 새로 쓰는가"
 * (요구함) 와 "이미 저장된 값을 그대로 옮기는가"(면제) 를 호출부가 구분해 넘긴다.
 */
