 ...
 *     src/games/migration/goal-event-backfill.cli.ts [--dry-run]
 *
 * `--dry-run` reports what WOULD be created/quarantined without writing
 ...
 * (game-result-backfill.ts/.cli.ts) and fixture-game-backfill.ts/.cli.ts
 * already established in this directory.
 */
import { PrismaService } from '../../prisma/prisma.service';
import { runGoalEventBackfill } from './goal-event-backfill';
...
async function main(): Promise<void> {
  ...
  try {
    const result = await runGoalEventBackfill(prisma, { mode: dryRun ? 'dry-run' : 'apply' });
