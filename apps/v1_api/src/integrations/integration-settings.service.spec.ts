 ...
 * priority, masked read shape (no raw key ever leaves getMasked()), and PATCH semantics
 * (undefined = untouched, "" = clear back to env fallback, non-empty = set).
 */
...
describe('IntegrationSettingsService', () => {
  ...
  let prisma: { v1IntegrationSettings: { findUnique: jest.Mock; upsert: jest.Mock } };
  const originalEnv = { ...process.env };

  beforeEach(() => {
    prisma = { v1IntegrationSettings: { findUnique: jest.fn(), upsert: jest.fn() } };
    service = new IntegrationSettingsService(prisma as unknown as PrismaService);
