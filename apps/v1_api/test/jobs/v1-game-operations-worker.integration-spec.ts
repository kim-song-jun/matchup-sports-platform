...
describe('V1GameOperationsWorkerService database lease contract', () => {
  ...
  it('reports built-in handler readiness and poisoned queue health without leaking owner identity', async () => {
    ...
    // GAME_RESULT_CHANGE_REQUESTED, LEAGUE_RESULT_ENTRY_REMINDER (리그 대진 결과
    // 미입력 +24시간 운영자 리마인더), IDENTITY_LINK_EXPIRY (신원 연결 요청 +24시간 만료
    ...
    // 재시도 후 POISONED 로 가고, 나중에 켜는 순간 그 행들이 이미 죽어 있다) -- 10 total.
    //
