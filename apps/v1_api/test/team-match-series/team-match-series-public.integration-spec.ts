...
const recordsOwnerUserId = `t4-series-records-owner-${suiteId}`;

describe('GET /team-match-series/:seriesId/player-records', () => {
  let app: INestApplication;
  let cleanup: (() => Promise<void>) | undefined;
  let prisma: PrismaService;
  let sportId: string;
  ...
  beforeAll(async () => {
    ...
    sportId = sport.id;
    const region = await prisma.v1Region.create({
      data: { code: `t4-series-records-region-${suiteId}`, name: 'T4 개인기록 테스트 지역', level: 2 },
    });
    regionId = region.id;
  });
  ...
  it('공식 결과에 반영된 어시스트가 도움 순위에 실제로 집계되고, 동의 없는 참가자는 제외된다', async () => {
    ...
    // 득점왕 유저: 골 2, 어시스트 1.
    const scorerUserId = `t4-series-records-scorer-${suiteId}`;
    // 도움왕 유저: 골 0, 어시스트 3 -- 골 순위엔 안 뜨지만 도움 순위엔 1위로 떠야 한다.
    const assisterUserId = `t4-series-records-assister-${suiteId}`;
    ...
    const res = await request(app.getHttpServer()).get(`/api/v1/team-match-series/${seriesId}/player-records`);
