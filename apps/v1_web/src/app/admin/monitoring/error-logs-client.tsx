  ...
  const columns: AdminTableColumn<V1AdminErrorLogListItem>[] = [
    ...
    },
    {
      ...
      render: (row) => (
        <span
          className={[
            'tm-on-tint inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[length:var(--font-size-caption)] font-semibold tabular-nums whitespace-nowrap',
      ...
      <AdminDataTable<V1AdminErrorLogListItem>
        ...
        renderActions={(row) => (
          <button
            type="button"
            onClick={() => setSelectedId(row.id)}
            className="inline-flex items-center justify-center min-h-[44px] px-3 rounded-lg text-[length:var(--font-size-label)] font-medium text-[var(--blue700)] bg-[var(--blue50)] hover:bg-[var(--blue100)] transition-colors focus-visible:outline-2 focus-visible:outline-blue-500 focus-visible:outline-offset-2"
