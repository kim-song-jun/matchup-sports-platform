...
export function tournamentCampaignStatusBadgeClass(status: V1TournamentCampaignStatus): string {
  switch (status) {
    case 'published': return 'rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-600';
    ...
    case 'draft': return 'rounded-full border border-[var(--border-strong)] bg-[var(--surface-soft)] px-3 py-1 text-xs font-semibold text-[var(--text-muted)]';
    case 'archived': return 'rounded-full bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-700';
  }
}
