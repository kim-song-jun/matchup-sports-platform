...
import { EVENT_CAMPAIGN_PAGE_SIZE, type EventCampaignSeed } from '@/lib/public-list-seed';
...
/** Suspense 경계가 없는 이유는 tournaments-list-client.tsx 의 TournamentsListPageClient 와 같다. */
export function EventsPageClient({ seed }: { readonly seed?: EventCampaignSeed }) {
  ...
  const {
    ...
    limit: EVENT_CAMPAIGN_PAGE_SIZE,
    // 서버 seed 는 종목 없는 목록이다 — 종목이 걸린 주소·선택에는 쓰지 않는다.
    seed: requestedSportCode === null && effectiveSportCode === undefined ? seed : undefined,
  });
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.listSection}>
        {masterSportResolutionPending || isPending ? (
          <EventListSkeleton />
        ) : isError && !data ? (
          <ErrorState
