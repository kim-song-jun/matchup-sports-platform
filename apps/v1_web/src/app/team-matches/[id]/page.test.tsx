...
vi.mock('@/lib/structured-data', () => ({ buildTeamMatchEventLd: vi.fn(() => ({ '@type': 'SportsEvent' })) }));
...
describe('TeamMatchDetailPage (server) — 첫 표시값 전달', () => {
  ...
  it('서버에서 받은 팀매치를 클라이언트에 seed 로 넘긴다', async () => {
    ...
    expect(findProps(element, 'teamMatchId')).toMatchObject({ teamMatchId: 'tm-1', seed: teamMatch });
    expect(vi.mocked(buildTeamMatchEventLd)).toHaveBeenCalledWith(teamMatch, 'tm-1');
