...
import { Clapperboard, ExternalLink, Film, Link2, Trash2, Upload } from 'lucide-react';
...
function AddVideoForm({
  ...
  const isPending = createLink.isPending || upload.isPending;
  ...
  function handleSubmit(event: React.FormEvent) {
    ...
    if (mode === 'link') {
      createLink.mutate(
        { fixtureId: fixture.fixtureId, url: url.trim(), ...(trimmedTitle ? { title: trimmedTitle } : {}) },
        {
          onSuccess: () => {
            reset();
            onDone('영상을 등록했어요.');
          },
          onError: (error) => onError(extractErrorMessage(error, '영상을 등록하지 못했어요.')),
        },
      );
      return;
    }
    ...
    upload.mutate(
      { fixtureId: fixture.fixtureId, file, ...(trimmedTitle ? { title: trimmedTitle } : {}) },
      {
        onSuccess: () => {
          reset();
          onDone('영상을 업로드했어요.');
        },
        onError: (error) => onError(extractErrorMessage(error, '영상을 업로드하지 못했어요.')),
      },
    );
  }

  const linkInputId = `video-url-${fixture.fixtureId}`;
  const fileInputId = `video-file-${fixture.fixtureId}`;
  const titleInputId = `video-title-${fixture.fixtureId}`;
