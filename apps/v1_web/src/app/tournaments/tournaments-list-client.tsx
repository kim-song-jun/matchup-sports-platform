 ...
 *
 * Suspense 경계가 없다 — page.tsx 가 요청 시점 렌더라 useSearchParams 가 suspend 하지 않는다.
 * 정적 프리렌더로 되돌리면 빌드가 missing-suspense 로 실패한다.
 */
export function TournamentsListPageClient({ seed }: { readonly seed?: TournamentListSeed }) {
  ...
  const seedMatches = seed !== undefined
    ...
    && querySportId === undefined
    && knownStatus === undefined
    && knownGenderCategory === undefined
    && page === 1
    && cursor === undefined;
  ...
  return (
    <div className="tm-tournament-list">
      ...
      <TournamentPromoCarousel
        ...
        loading={promoTournaments.isPending}
