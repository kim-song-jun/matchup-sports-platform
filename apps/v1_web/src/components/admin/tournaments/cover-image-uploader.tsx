    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="grid gap-3 sm:grid-cols-[minmax(0,240px)_1fr] sm:items-center">
        ...
        <div className="relative aspect-square overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--grey50)]">
          {/* 커버 예시 사진과 "정사각형으로 표시" 오버레이는 커버(cover) 전용 안내다 —
              contain(로고) 모드의 빈 상태에서 그대로 노출되면 엉뚱한 예시·설명이 된다. */}
          {value || previewFit === 'cover' ? (
            <Image
              src={publicAssetPath(value ?? COVER_EXAMPLE)}
              alt={value ? (previewAlt ?? '선택한 대회 커버 미리보기') : '대회 커버 이미지 예시'}
              fill
        ...
        <div className="flex flex-col items-start gap-2">
          ...
          {value ? (
            <button
              ...
              onClick={onClear}
              disabled={locked}
              className="inline-flex min-h-[44px] items-center justify-center gap-2 rounded-xl px-4 text-sm font-semibold text-[var(--text-caption)] transition-colors hover:bg-[var(--red50)] hover:text-[var(--red700)] disabled:opacity-50"
            >
