  ...
  return (
    <AppChrome title={edit ? '매치 수정' : '매치 만들기'} activeTab="matches" bottomNav={false} backHref={edit ? (model.matchId ? `/matches/${model.matchId}` : '/matches') : '/matches'}>
      ...
      <div className="tm-fixed-cta tm-create-fixed-cta">
        ...
        {edit && model.form?.recruitingToggle ? (
          <>
            <button
              ...
              type="button"
              style={{ marginTop: 8 }}
              disabled={model.form.submitting || model.form.recruitingToggle.pending}
        ...
        {edit && model.form?.onCancel ? <button className="tm-btn tm-btn-md tm-btn-danger tm-btn-block" type="button" style={{ marginTop: 8 }} disabled={model.form.submitting} onClick={model.form.onCancel}>매치 취소</button> : null}
      </div>
    </AppChrome>
  );
}
 ...
 *
 * 목록에는 마감된 매치도 경기 시작 전까지 남는다(matches.service.ts list()) — 그래서
...
function MatchCardItem({ match }: { match: MatchCardModel }) {
  ...
  return (
    <Link className={`tm-match-list-card tm-pressable${closedLabel ? ' tm-card-closed' : ''}`} href={`/matches/${match.id}`}>
      ...
      <div className="tm-match-list-card-body">
        ...
        <div className="tm-match-card-headline" style={{ marginTop: 8 }}>
