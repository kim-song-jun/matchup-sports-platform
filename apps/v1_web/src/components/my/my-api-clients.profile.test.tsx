...
vi.mock('@/components/auth/phone-verification/phone-verification-card', () => ({
  PhoneVerificationCard: ({ onVerified }: { onVerified: (proofToken?: string) => void }) => (
    <button type="button" onClick={() => onVerified('proof-token-abc')}>
      stub-인증완료
    </button>
  ),
}));
...
describe('ProfileEditPageClient 번호 변경 본인인증 게이트', () => {
  ...
  it('번호를 바꾸면 인증 카드가 나타나고, 인증 없이 저장하면 서버로 보내지 않는다', () => {
    ...
    expect(screen.queryByRole('button', { name: 'stub-인증완료' })).not.toBeInTheDocument();

    changePhoneTo('010-3333-4444');
    expect(screen.getByRole('button', { name: 'stub-인증완료' })).toBeInTheDocument();
  ...
  it('인증을 마치면 저장되고 증명 토큰이 함께 전송된다', () => {
    ...
    changePhoneTo('010-3333-4444');
    fireEvent.click(screen.getByRole('button', { name: 'stub-인증완료' }));
    fireEvent.click(screen.getByRole('button', { name: '프로필 저장' }));
