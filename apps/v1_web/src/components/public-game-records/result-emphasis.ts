 ...
 *
 * 색 선택 근거:
 * - 승/패처럼 강조색을 쓰는 칩은 `-700`(텍스트) + `-50`(배경) 쌍만 쓴다. `globals.css` 다크
 *   섹션 주석의 실측대로 500계열은 자기 틴트 배경 위에서 3.7:1로 AA 미달이고, 700계열은
 *   라이트/다크 각각 카드 표면과 틴트 배경 양쪽에 4.5:1 이상을 확보하도록 토큰이 재정의돼
 *   있다. 무승부·미확정은 애초에 강조색이 아니라 중립 톤(`--text-caption` + `--surface-soft`)
 *   이라 이 쌍의 대상이 아니다.
