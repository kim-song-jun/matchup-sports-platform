...
export function SearchExperience({ initialQuery = '', state = 'results' }: SearchExperienceProps) {
  const router = useRouter();
  const hydrationSafeInitialQuery = state === 'new' ? '' : initialQuery;
  const [query, setQuery] = useState(hydrationSafeInitialQuery);
  const [submittedQuery, setSubmittedQuery] = useState(hydrationSafeInitialQuery);
  ...
  return (
    <AppChrome title="검색" activeTab="home" topBar={false} bottomNav={false}>
      <div className="tm-search-frame tm-search-open-design" data-testid="search-open-design">
      <form onSubmit={submit} className="tm-search-topbar">
        <button type="button" aria-label="back" onClick={goBack} className="tm-search-back">
          <ChevronLeft size={20} />
        </button>
        <div className={`tm-search-input-shell ${viewState === 'error' ? 'tm-search-input-shell-error' : query ? 'tm-search-input-shell-active' : ''}`}>
          <input
            aria-label="검색어"
            value={query}
            onChange={(event) => setQuery(event.target.value)}
            placeholder="검색어를 입력해 주세요"
            className="tm-text-body tm-search-input"
            autoFocus
          />
          {query ? (
            <button type="button" aria-label="clear search" onClick={clear} className="tm-search-clear">
              <span className="tm-search-clear-icon">
                <X size={13} />
              </span>
            </button>
          ) : null}
          <button type="submit" aria-label="submit search" className={`tm-search-submit ${viewState === 'error' ? 'tm-search-submit-error' : ''}`}>
            <Search size={19} />
          </button>
        </div>
      </form>

      <div className="tm-search-scroll">
        <div className="tm-text-label">최근 검색</div>
        <div className="tm-search-chip-wrap">
        ...
        <div className="tm-text-label tm-search-section-label">빠른 조건</div>
        <div className="tm-search-quick-grid">
        ...
        <div className="tm-search-divider" />
        <div>
        ...
        {effectiveViewState === 'results' ? (
          <div className="tm-search-result-list">
            {results.map((item) => (
        ...
        {effectiveViewState === 'new' ? (
          <div className="tm-search-state">
            <div className="tm-search-state-icon">
              <Search size={22} />
            </div>
        ...
        {effectiveShowStateMessage ? (
          <div className="tm-search-state">
            <div className={`tm-search-state-icon ${effectiveViewState === 'error' ? 'tm-search-state-icon-error' : ''}`}>
