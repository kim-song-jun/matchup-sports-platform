...
export function getStatus(match: V1TeamMatch): V1TeamMatchApiStatus {
  ...
  // 개인 매치 쪽(matches-client.tsx getStatus)과 같은 마감 폴백. 서버가 이미 displayState 로
  // 같은 판정을 내려주지만, 그 필드를 싣지 않는 응답이 섞여도(실제로 GET /me/team-matches 가
  // 그랬다) 카드가 마감된 팀매치를 '모집 중'으로 그리지 않게 한다 — 목록과 상세가 서로 다른
  // 상태를 말하던 결함(2026-09-07 제보)의 프론트 쪽 방어선이다.
  if (base === 'recruiting') {
