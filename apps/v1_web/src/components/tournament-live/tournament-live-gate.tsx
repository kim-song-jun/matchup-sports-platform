...
import type { V1MyTournamentStaffResponse, V1TournamentStaffRole } from '@/types/api';
 ...
 * platform admin 여부는 `/me/tournament-staff`의 `platformRole` 응답을 사용한다. 전역
 * staff 목록에 내 배정 행이 없는 것만으로 platform admin을 추측하지 않는다.
 */
...
function deriveRole(
  ...
  if (myActiveRows.length === 0) return null;
  return myActiveRows.reduce((best, item) =>
    STAFF_ROLE_PRIORITY[item.role] < STAFF_ROLE_PRIORITY[best.role] ? item : best,
  ).role;
}

function deriveMyAssignmentRole(
  response: V1MyTournamentStaffResponse | undefined,
  tournamentId: string,
): V1TournamentStaffRole | null {
  const group = findMyTournamentGroup(response, tournamentId);
  if (group === null || group.assignments.length === 0) return null;
  const activeAssignments = group.assignments.filter(
    (assignment) => assignment.expiresAt === null || new Date(assignment.expiresAt).getTime() > Date.now(),
  );
  if (activeAssignments.length === 0) return null;
  return activeAssignments.reduce((best, assignment) =>
    STAFF_ROLE_PRIORITY[assignment.role] < STAFF_ROLE_PRIORITY[best] ? assignment.role : best,
    activeAssignments[0].role,
  );
}

function hasActiveFieldFixtureCoverage(
  response: V1MyTournamentStaffResponse | undefined,
  tournamentId: string,
  fixtureId: string,
): boolean {
  ...
  return group.assignments.some((assignment) => {
    ...
    return assignment.fixtureIds.includes(fixtureId) || (assignment.fixtureIds.length === 0 && assignment.fieldId !== null);
  });
}
...
function deriveMyRole(
  ...
  if (response?.platformRole === 'PLATFORM_OPS') return 'PLATFORM_OPS';
  return deriveMyAssignmentRole(response, tournamentId);
}
 ...
 * 꺼낸다. 그 외 경로면 null.
 *
 * field operator는 배정에 fixture/field 스코프가 붙어 대회 전역 read가 거부되므로,
 * 자기 배정 응답으로만 딥링크를 판정하고 전역 staff 조회를 시도하지 않는다.
 */
// ── 화면 ──────────────────────────────────────────────────────────────────
...
function AccessDenied({ scopeNotYetSupported, fixtureScope = false }: { scopeNotYetSupported: boolean; fixtureScope?: boolean }) {
  return (
    <div className="min-h-screen bg-[var(--surface-soft)] flex items-center justify-center px-4">
      <div className="flex flex-col items-center gap-3 text-center max-w-[340px]">
        ...
        <h1 className="text-[length:var(--font-size-subhead)] font-bold text-[var(--text-strong)]">
          {fixtureScope ? '담당 경기 범위 밖의 화면이에요' : scopeNotYetSupported ? '담당 범위 밖의 화면이에요' : '대회 운영자 권한이 필요해요'}
        </h1>
        ...
        <p className="text-[length:var(--font-size-body-sm)] text-[var(--text-muted)] leading-relaxed">
          {fixtureScope
            ? '이 경기는 담당 범위에 포함되어 있지 않아요. 담당 경기 목록에서 배정된 경기를 확인해 주세요.'
...
export function TournamentLiveGate({ children, tournamentId }: TournamentLiveGateProps) {
  ...
  // 자기 배정 응답은 모든 경로에서 플랫폼 권한을 판정한다.
  // 경기 콘솔은 이 응답만 사용하고, 운영 보드는 전역 staff 조회도 수행한다.
  const fixtureId = fixtureIdFromConsolePath(pathname, tournamentId);
  const myAssignments = useV1MyTournamentStaffAssignments({ enabled: true });
  const ownAssignmentRole = deriveMyRole(myAssignments.data, tournamentId);
  const ownConsoleAccess =
    fixtureId !== null &&
    ownAssignmentRole !== null &&
    (ownAssignmentRole !== 'FIELD_OPERATOR' || hasActiveFieldFixtureCoverage(myAssignments.data, tournamentId, fixtureId));
  // 경기 범위의 진입 판정에는 전역 staff 목록이 필요하지 않다.
  const shouldLoadGlobalStaff = fixtureId === null;
  const staff = useV1TournamentStaffAssignments(tournamentId, { enabled: shouldLoadGlobalStaff });
  ...
  if (
    authMe.isPending ||
    (fixtureId === null && staff.isPending) ||
    myAssignments.isPending
  ) {
  ...
  if (fixtureId !== null && ownConsoleAccess && ownAssignmentRole !== null) {
    if (ownAssignmentRole === 'FIELD_OPERATOR') {
      return (
    ...
    return (
      <TournamentOpsRoleProvider role={ownAssignmentRole}>
        <TournamentOpsShell
          tournamentId={tournamentId}
          role={ownAssignmentRole}
          tournamentTitle={tournament.data?.title}
  ...
  if (fixtureId !== null) {
    return <AccessDenied scopeNotYetSupported={false} fixtureScope />;
  }
  ...
  const role = myAssignments.data?.platformRole === 'PLATFORM_OPS'
    ? 'PLATFORM_OPS'
    : deriveRole(staff.data.items, authMe.data.user.id);

  if (role === null) {
    return <AccessDenied scopeNotYetSupported={false} />;
  }
