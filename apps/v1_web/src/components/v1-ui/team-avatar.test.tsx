...
describe('TeamAvatar', () => {
  ...
  it('hides the identicon svg once the logo image successfully loads', async () => {
    const { container } = render(<TeamAvatar seed="team-1" name="성수 러너스" logoUrl="/uploads/logo.png" />);
    ...
    // next/image는 우리가 넘긴 onLoad를 네이티브 'load' 이벤트에서 바로 호출하지 않고
    // `img.decode()`(Promise) 완료 후 호출한다(image-component.js `handleLoading`) — 실제
    // 브라우저에서도 마이크로태스크 한 틱 뒤에 반영되는 진짜 비동기 동작이라 waitFor로 기다린다.
    fireEvent.load(img);
    await waitFor(() => expect(svg.style.display).toBe('none'));
  });

  it('recovers visibility when logoUrl changes from a broken URL to a valid one', async () => {
    const { container, rerender } = render(<TeamAvatar seed="team-1" name="성수 러너스" logoUrl="/uploads/broken.png" />);
    const img = container.querySelector('img') as HTMLImageElement;
    fireEvent.error(img); // 첫 로고가 깨져서 onError가 display:none을 건다 (onError는 동기)
    expect(img.style.display).toBe('none');

    // 재업로드 등으로 유효한 로고로 교체됨 — 같은 <img> 엘리먼트가 재사용된다.
    rerender(<TeamAvatar seed="team-1" name="성수 러너스" logoUrl="/uploads/fixed.png" />);
    fireEvent.load(img);
    // onLoad는 img.decode() 이후 비동기로 호출된다(위 테스트와 동일한 이유).
    await waitFor(() => expect(img.style.display).not.toBe('none'));
