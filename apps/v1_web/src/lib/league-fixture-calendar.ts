 ...
 *
 * ## 팀 일정 캘린더(`team-schedules.view-model.ts`)와 합치지 않는다
 * 거기에도 6주 고정 월간 격자가 있지만 **기준이 다르다** — 그쪽은 `getFullYear()`·
 * `getMonth()`·`getDate()` 로 **브라우저 로컬 날짜**를 키로 쓴다(팀 일정은 보는 사람의
 * 달력에 맞추는 게 맞다). 리그 대진 날짜는 **KST 달력 날짜**여야 한다 — 서버 계약이 그렇고,
 * 로컬 기준으로 만들면 해외에서 접속한 운영자가 **다른 날을 보내게 된다.**
 * 합치면 이 모듈이 피하려는 바로 그 버그가 들어온다.
 *
 * ## 날짜는 KST 달력 날짜다
