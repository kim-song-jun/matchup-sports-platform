...
export interface V1LeagueFixture {
   ...
   * 있고 공개만 안 될 뿐이다. 공개 상세와 일정 목록 양쪽에서 채워지며, **확정된 결과가
...
export interface V1LeaguePlayerRecordsResponse {
  ...
  /**
   * 기록은 입력됐지만 **공개 자격**(신원 연동 + 기록 공개 동의)을 못 갖춰 집계에서 빠진
   * 행이 있는가. 둘 중 어느 쪽이 빠졌든 true 라서 이름이 consent 가 아니라 eligibility 다.
