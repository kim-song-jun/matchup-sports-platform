  ...
  return (
    <div className="pt-[var(--safe-area-top)] lg:pt-0 pb-32">
          ...
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 animate-fade-in">
            ...
            <div className="rounded-xl bg-gray-50 p-4 mb-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-[14px] text-gray-700">{formatCurrency(mockPayment.amount)}</span>
              </div>
      ...
      <div className="hidden lg:block mb-6">
        ...
        <h2 className="text-[24px] font-bold text-gray-900">환불 요청</h2>
      </div>
      ...
      <div className="px-5 lg:px-0 max-w-lg mx-auto lg:mx-0 space-y-4 mt-4 lg:mt-0">
        ...
        <div className="rounded-2xl bg-white border border-gray-100 p-5">
          ...
          <div className="rounded-xl bg-gray-50 p-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[14px] font-medium text-gray-900">{mockPayment.match.name}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-[13px] text-gray-500">결제 금액</span>
              <span className="text-[14px] font-semibold text-gray-900">{formatCurrency(mockPayment.amount)}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-[13px] text-gray-500">결제 수단</span>
              <span className="text-[13px] text-gray-600">{mockPayment.method}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-[13px] text-gray-500">결제일</span>
              <span className="text-[13px] text-gray-600">{formatDateTime(mockPayment.paidAt)}</span>
            </div>
          </div>
        ...
        <div className="rounded-2xl bg-white border border-gray-100 p-5">
          ...
          <div className="space-y-2">
            <div className={`flex items-center justify-between rounded-xl px-4 py-3 ${refundInfo.percentage === 100 ? 'bg-green-50 ring-2 ring-green-200' : 'bg-gray-50'}`}>
              <div className="flex items-center gap-2">
                <Clock size={14} className={refundInfo.percentage === 100 ? 'text-green-500' : 'text-gray-400'} />
                <span className={`text-[13px] font-medium ${refundInfo.percentage === 100 ? 'text-green-700' : 'text-gray-600'}`}>경기 24시간 전</span>
              </div>
              <span className={`text-[13px] font-bold ${refundInfo.percentage === 100 ? 'text-green-600' : 'text-gray-500'}`}>전액 환불</span>
            </div>
            <div className={`flex items-center justify-between rounded-xl px-4 py-3 ${refundInfo.percentage === 50 ? 'bg-amber-50 ring-2 ring-amber-200' : 'bg-gray-50'}`}>
              <div className="flex items-center gap-2">
                <Clock size={14} className={refundInfo.percentage === 50 ? 'text-amber-500' : 'text-gray-400'} />
                <span className={`text-[13px] font-medium ${refundInfo.percentage === 50 ? 'text-amber-700' : 'text-gray-600'}`}>경기 1~24시간 전</span>
              </div>
              <span className={`text-[13px] font-bold ${refundInfo.percentage === 50 ? 'text-amber-600' : 'text-gray-500'}`}>50% 환불</span>
            </div>
            <div className={`flex items-center justify-between rounded-xl px-4 py-3 ${refundInfo.percentage === 0 ? 'bg-red-50 ring-2 ring-red-200' : 'bg-gray-50'}`}>
              <div className="flex items-center gap-2">
                <Clock size={14} className={refundInfo.percentage === 0 ? 'text-red-500' : 'text-gray-400'} />
                <span className={`text-[13px] font-medium ${refundInfo.percentage === 0 ? 'text-red-700' : 'text-gray-600'}`}>경기 1시간 이내</span>
              </div>
        ...
        <div className="rounded-2xl bg-white border border-gray-100 p-5">
          ...
          <textarea
            ...
            rows={3}
            className="w-full rounded-xl border border-gray-200 px-4 py-3 text-[14px] text-gray-900 placeholder:text-gray-300 resize-none focus:outline-none focus:border-blue-500 transition-colors"
          />
        </div>
      </div>
