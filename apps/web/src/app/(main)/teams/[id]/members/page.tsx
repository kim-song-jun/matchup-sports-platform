...
import { ArrowLeft, Shield, User, MoreVertical, AlertTriangle, Crown, LogOut, MessageCircle, ExternalLink } from 'lucide-react';
...
// These hooks will be provided by frontend-data-dev (Wave 1C).
// Import signatures match task §6.1.1C.
import {
  useTeamApplications,
...
type ActiveTab = 'members' | 'applicants';
...
export default function TeamMembersPage() {
  ...
  // Support deep-link from notifications: /teams/:id/members?tab=applicants
  const tabParam = searchParams.get('tab');
  const resolvedTab: ActiveTab = tabParam === 'applicants' ? 'applicants' : 'members';
  const [activeTab, setActiveTab] = useState<ActiveTab>(resolvedTab);
  ...
  const isManagerOrOwner = myMembership?.role === 'owner' || myMembership?.role === 'manager';
  const membersPageTitle = isManagerOrOwner ? '멤버 관리' : '멤버 목록';
  ...
  const acceptMutation = useAcceptTeamApplication();
  const rejectMutation = useRejectTeamApplication();
  const startDirectChatMutation = useStartDirectChat();

  // Track which applicant is currently being actioned (for disabled state)
  const [pendingApplicantId, setPendingApplicantId] = useState<string | null>(null);

  function handleAccept(applicantUserId: string) {
    setPendingApplicantId(applicantUserId);
    acceptMutation.mutate(
      { teamId, applicantUserId },
      {
        ...
        onError: (err) => {
          toast('error', extractErrorMessage(err, '수락에 실패했어요'));
          setPendingApplicantId(null);
        },
      },
    );
  ...
  function handleReject(applicantUserId: string) {
    ...
    rejectMutation.mutate(
      { teamId, applicantUserId },
      {
        ...
        onError: (err) => {
          toast('error', extractErrorMessage(err, '거절에 실패했어요'));
          setPendingApplicantId(null);
        },
      },
    );
  ...
  function handleStartChat(applicantUserId: string) {
    startDirectChatMutation.mutate(
      { withUserId: applicantUserId },
      {
        onSuccess: (chatRoom) => {
  ...
  return (
    <div className="pt-[var(--safe-area-top)] @3xl:pt-0 animate-fade-in">
      ...
      {/* Tab switcher — only show "신청자" tab for manager+ */}
      {isManagerOrOwner && (
        <div className="px-5 @3xl:px-0 pt-3 pb-1">
          <div className="flex gap-1 rounded-xl bg-gray-100 dark:bg-gray-700 p-1">
            <button
              ...
              className={`flex-1 min-h-[44px] rounded-lg py-2.5 text-sm font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 ${
                activeTab === 'members'
            ...
            >
              ...
              {applications.length > 0 && (
                <span className="ml-1.5 inline-flex items-center justify-center h-4 min-w-4 rounded-full bg-blue-500 text-white text-2xs font-bold px-1">
                  {applications.length}
                </span>
              )}
            </button>
      ...
      {/* Members tab */}
      {activeTab === 'members' && (
        <div className="px-5 @3xl:px-0 pb-8 space-y-2 mt-3">
          {isError ? (
                ...
                >
                  ...
                  {canManage && (
                    <div className="relative" ref={(node) => setMenuRef(member.id, node)}>
                      <button
                        ...
                        className="flex items-center justify-center min-h-11 min-w-11 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
                      >
                        ...
                        <div role="menu" className="absolute right-0 top-full mt-1 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-600 shadow-lg z-10 overflow-hidden min-w-[160px]">
                          {member.role === 'member' ? (
                            <button
                              ...
                              className="w-full text-left px-4 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 flex items-center gap-2 min-h-[44px] disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
                            >
                          ...
                          <button
                            ...
                            className="w-full text-left px-4 py-2.5 text-sm font-medium text-amber-600 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-900/10 flex items-center gap-2 min-h-[44px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
                          >
                          ...
                          <button
                            ...
                            className="w-full text-left px-4 py-2.5 text-sm font-medium text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 flex items-center gap-2 border-t border-gray-100 dark:border-gray-700 min-h-[44px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
                          >
                  ...
                  {isSelf && !isOwner && (
                    <button
                      ...
                      className="flex items-center gap-1.5 rounded-xl bg-red-50 dark:bg-red-900/20 px-3 py-2 text-sm font-medium text-red-500 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors min-h-[44px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
                    >
      ...
      {/* Applicants tab — only visible to manager+ */}
      {activeTab === 'applicants' && isManagerOrOwner && (
        <div className="px-5 @3xl:px-0 pb-8 space-y-2 mt-3">
          {applicationsError ? (
            <ErrorState onRetry={applicationsRefetch} />
          ) : applicationsLoading ? (
            <>
              {[1, 2, 3].map((i) => (
            ...
            <EmptyState
              icon={User}
              title="신청자가 아직 없어요"
              description="팀 참여 신청이 들어오면 여기에 표시돼요"
              size="sm"
            />
          ) : (
            applications.map((applicant) => {
              const isActioning = pendingApplicantId === applicant.userId;

              return (
                <div
                  key={applicant.id}
                  data-testid={`applicant-row-${applicant.userId}`}
                ...
                >
                  <div className="flex items-center gap-2">
                    <UserCard
                      user={{
                        id: applicant.userId,
                        nickname: applicant.user?.nickname ?? '알 수 없음',
                        profileImageUrl: applicant.user?.profileImageUrl,
                        mannerScore: applicant.user?.mannerScore,
                      }}
                      onProfileClick={() => router.push(`/users/${applicant.userId}`)}
                    />
                  </div>
                  ...
                  <div className="flex items-center gap-2 mt-3">
                    ...
                    <button
                      type="button"
                      onClick={() => router.push(`/users/${applicant.userId}`)}
                      aria-label={`${applicant.user?.nickname ?? '신청자'} 프로필 보기`}
                      data-testid={`applicant-profile-${applicant.userId}`}
                      className="flex items-center gap-1.5 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-xs font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors min-h-[44px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => handleStartChat(applicant.userId)}
                      aria-label={`${applicant.user?.nickname ?? '신청자'}에게 채팅 보내기`}
                      data-testid={`applicant-chat-${applicant.userId}`}
                    ...
                    <button
                      ...
                      aria-label={`${applicant.user?.nickname ?? '신청자'} 거부`}
                      data-testid={`applicant-reject-${applicant.userId}`}
                      disabled={isActioning}
                      className="flex items-center justify-center rounded-lg border border-red-200 dark:border-red-800 bg-white dark:bg-gray-700 px-4 py-2 text-xs font-semibold text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors min-h-[44px] disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
                    >
                      거부
                    </button>
                    ...
                    <button
                      ...
                      aria-label={`${applicant.user?.nickname ?? '신청자'} 수락`}
                      data-testid={`applicant-accept-${applicant.userId}`}
                      disabled={isActioning}
                      className="flex items-center justify-center rounded-lg bg-blue-500 hover:bg-blue-600 px-4 py-2 text-xs font-semibold text-white transition-colors min-h-[44px] disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
                    >
                      수락
                    </button>
                  </div>
        ...
        <Modal isOpen={!!kickTarget} onClose={() => setKickTarget(null)} title="멤버 강퇴">
          ...
          <div className="mt-6 flex gap-3">
            <button
              ...
              className="flex-1 rounded-xl bg-gray-100 dark:bg-gray-700 py-3 text-base font-semibold text-gray-700 dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
            >
              돌아가기
            </button>
            <button
              ...
              className="flex-1 rounded-xl bg-red-500 py-3 text-base font-semibold text-white hover:bg-red-600 disabled:opacity-50 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
            >
      ...
      <Modal isOpen={showLeaveModal} onClose={() => setShowLeaveModal(false)} title="팀 탈퇴">
        <div className="flex items-center justify-center w-12 h-12 rounded-full bg-red-50 dark:bg-red-900/20 mx-auto mb-4">
          <LogOut size={24} className="text-red-500" aria-hidden="true" />
        </div>
