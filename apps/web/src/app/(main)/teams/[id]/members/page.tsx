...
const roleConfig: Record<string, { label: string; style: string }> = {
  ...
  admin: { label: '운영자', style: 'bg-blue-50 text-blue-600' },
  ...
  return (
    <div className="pt-[var(--safe-area-top)] lg:pt-0 animate-fade-in">
      ...
      <div className="hidden lg:flex lg:items-center lg:justify-between mb-6 px-5 lg:px-0 pt-4">
        <div>
          <h2 className="text-[24px] font-bold text-gray-900">멤버 관리</h2>
          <p className="text-[14px] text-gray-400 mt-1">팀 멤버 {members.length}명</p>
        </div>
        <button
          onClick={() => setShowInviteModal(true)}
      ...
      <div className="px-5 lg:px-0 pb-8 space-y-2">
        {members.map((member) => {
          ...
          return (
            <div key={member.id} className="rounded-2xl bg-white border border-gray-100 p-4 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-[15px] font-semibold text-gray-900">{member.nickname}</span>
              ...
              {member.role !== 'owner' && (
                <div className="relative">
                  ...
                  {menuOpen === member.id && (
                    <div className="absolute right-0 top-full mt-1 bg-white rounded-xl border border-gray-200 shadow-lg z-10 overflow-hidden min-w-[140px]">
                      {member.role === 'member' ? (
                        <button
                      ...
                      <button
                        ...
                        className="w-full text-left px-4 py-2.5 text-[13px] font-medium text-red-500 hover:bg-red-50 flex items-center gap-2 border-t border-gray-100"
                      >
