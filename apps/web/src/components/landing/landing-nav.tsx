      ...
      >
        <div className="max-w-[1100px] mx-auto px-5 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Link
              href="/login"
              className="text-base font-medium text-gray-500 hover:text-gray-900 active:scale-[0.97] transition-colors px-3 py-2.5 rounded-lg hidden sm:block focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-400"
            >
              로그인
            </Link>
            <Link
        ...
        {/* Mobile dropdown — focus trap dialog when open */}
        <div
          ref={dropdownRef}
          role={mobileOpen ? 'dialog' : undefined}
          aria-modal={mobileOpen ? 'true' : undefined}
          aria-label={mobileOpen ? '모바일 메뉴' : undefined}
          className={`md:hidden border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 transition-[transform,opacity] duration-300 ease-out ${
            ...
            {NAV_LINKS.map((link) => {
              ...
              return (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`block text-md font-medium px-4 py-3 rounded-xl transition-colors ${
                    isActive
                      ? 'text-blue-500 bg-blue-50 dark:bg-blue-900/30'
