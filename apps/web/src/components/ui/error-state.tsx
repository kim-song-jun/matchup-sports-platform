...
export function ErrorState({ message = '앗, 잠시 문제가 생겼어요', onRetry }: ErrorStateProps) {
  return (
    <div className="solid-panel rounded-[28px] py-10 text-center">
      ...
      {onRetry && (
        <button onClick={onRetry}
          className="mt-4 inline-flex min-h-[44px] items-center gap-1.5 rounded-full bg-gray-100 dark:bg-gray-700 px-4 py-2 text-xs font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors">
          <RefreshCw size={12} />
          다시 불러오기
        </button>
      )}
