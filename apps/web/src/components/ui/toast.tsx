...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <ToastContext.Provider value={{ toast: addToast }}>
      {children}
      {/* Toast container */}
      <div className="fixed bottom-24 lg:bottom-8 left-1/2 -translate-x-1/2 z-[100] flex flex-col gap-2 w-full max-w-sm px-4">
        {toasts.map((t) => (
          <div
            key={t.id}
          ...
          >
            ...
            <button onClick={() => removeToast(t.id)} aria-label="알림 닫기" className="flex items-center justify-center min-w-11 min-h-[44px] -mr-2 text-gray-500 hover:text-white">
              <X size={16} />
            </button>
          </div>
        ))}
