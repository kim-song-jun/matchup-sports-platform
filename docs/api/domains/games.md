...
|---|---|---|---|
| `GET` | `/api/v1/games/:gameId/visibility` | optional auth; public visibility policy | Returns the serialized `live`, `status_only` or `official_only` projection. A missing game or policy is `404`; `PUBLIC_LIVE=off` demotes `live` to `official_only` (confirmed results stay public). |
