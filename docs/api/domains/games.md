...
|---|---|---|---|
| `GET` | `/api/v1/games/:gameId/visibility` | optional auth; public visibility policy | Returns the serialized `live`, `status_only` or `official_only` projection. A missing game or policy is `404`; `PUBLIC_LIVE=off` demotes `live` to `official_only` (confirmed results stay public). **Known gap (pre-existing, not introduced here)**: this endpoint's caller collapses the stored policy to `status_only` or `live` only (`games.service.ts`, `mode: … === STATUS_ONLY ? 'status_only' : 'live'`), so a stored `OFFICIAL_ONLY` or `HIDDEN` policy is served as `live` and `official_only` output is reachable only via the kill-switch demotion above. The public-records lanes (`docs/api/domains/public-records.md`) resolve all four modes correctly; only this probe does not. |
