...
No change was needed to `apps/v1_api/src/games/core/revision-state-machine.ts`: the void flow's only invariant (`base.state` must be `OFFICIAL`) is checked inline in `voidResultRevision` rather than by adding a `'VOID'` member to the shared `RevisionSupersessionPurpose` union, since void revisions are created directly in the terminal `VOID` state (no draft step) and the existing `CORRECTION`/`TOURNAMENT_RESUBMISSION` purposes already cover every other supersession this lane performs unmodified.
...
| `POST /api/v1/games/:gameId/corrections` | `CreateGameResultCorrectionDto {expectedVersion,clientCommandId,baseRevisionId,reason,changes:{score,actualParticipants,eventsHash,mvpParticipantId?}}` | creates a same-game superseding `DRAFT`; creation alone never swaps the pointer | `tournament_director`/`platform_ops` (not flag-gated) |
