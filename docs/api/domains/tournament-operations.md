...
All routes below are under `/api/v1`, require `V1AuthGuard` (authenticated user), and return the global `{status,data,timestamp}` envelope. `expectedVersion` is always a non-negative integer compared by optimistic CAS. Fixture lineup save uses the target side's latest lineup revision (`0` when absent), and lineup submit uses the addressed draft lineup's revision; other commands use their documented aggregate/resource version.
...
2. **`PUT .../fixtures/:fixtureId/lineup/:sideId` body shape is `SaveGameLineupDto`, not the registry's `{expectedVersion,sideId,formation,starters,bench}`.** `sideId` is a path segment, not a body field, and the body is `{expectedVersion,clientCommandId,formation?,participants:[{participantId?,displayNameSnapshot,jerseyNumber?,position?,started}]}` — the same shape `PUT /games/:gameId/lineups/:sideId` already uses. `expectedVersion` is that side's latest lineup revision, not `V1Game.version`. There is no separate `starters`/`bench` split; `participants[].started` carries that distinction per participant.
