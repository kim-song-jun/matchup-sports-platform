...
- `reputation.highlight`: the tag on the most revealed reviews (`tagCode`, `label`, `rate` 0-1, `reviewCount`) from the same reviews as `mannerScore`; `null` below three revealed reviews so a rate never points at one or two reviewers. `GET /teams/:teamId/reviews` returns the same `highlight` for a team.
