...
> **상태**: 🟢 골격 결정 + IA 재정의 확정(2026-06-13). 버그픽스 먼저 / 신규 Tournament 도메인 / PG 포함 / realName 대회 신청 시 / 알림 전체확장 / **navbar 5탭(홈·매치·대회·팀·마이) · 매치=개인+팀 병존 · 대회=상금+구조**(§G 참고). **선출여부=자기신고+어드민검토 · 결제=PG+계좌이체 택1 확정(2026-06-14, §D #6·#7)**. **Wave1+1.5 버그픽스 커밋 완료(bd5d6377)** · **Wave1.6 IA 재정의 구현·검증 완료** · PR #21 Copilot clean(머지 보류). **Wave2 진행 중**: ① Prisma 스키마 10모델 db push 완료 · ② V1Tournament 어드민 CRUD(jest 12 + 스모크 19) · ③ V1TournamentRegistration 신청 상태머신(팀 manager+ 게이트, draft→submit→awaiting_payment +결제레코드(PG/계좌 택1), cancel-request, 동의·입금자명 검증; jest 15 + 라이브 스모크 12) 완료 — 다음: V1TournamentPlayer 명단(#11) 또는 어드민 입금확인·확정(#12).
