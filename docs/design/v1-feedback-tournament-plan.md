...
> **상태**: 🟢 골격 결정 + IA 재정의 확정(2026-06-13). 버그픽스 먼저 / 신규 Tournament 도메인 / PG 포함 / realName 대회 신청 시 / 알림 전체확장 / **navbar 5탭(홈·매치·대회·팀·마이) · 매치=개인+팀 병존 · 대회=상금+구조**(§G 참고). **선출여부=자기신고+어드민검토 · 결제=PG+계좌이체 택1 확정(2026-06-14, §D #6·#7)**. **Wave1+1.5 버그픽스 커밋 완료(bd5d6377)** · **Wave1.6 IA 재정의 구현·검증 완료** · PR #21 Copilot clean(머지 보류). **Wave2 진행 중**: ① Prisma 스키마 10모델 db push 완료 · ② V1Tournament 어드민 CRUD(jest 12 + 스모크 19) · ③ V1TournamentRegistration 신청 상태머신 · ④ **나머지 백엔드 전부**(ultracode 4-클러스터 병렬 생성→통합): 어드민 신청확정(confirm-payment→confirmed/waitlisted·취소·roster-lock) + 결제(PG prepare/confirm 스텁·계좌 어드민확인) + 대진(조/픽스처/결과/순위 자동재계산) + 명단(선수 6~10·realName·선출여부·CSV) + 소비자 조회(목록/상세 조별순위 중첩) + 공지 — **완료**. 전체 jest 25 suites/268 통과, 라이브 E2E 스모크 23(신청 bank+PG→확정→명단→CSV→대진→결과 3-1→순위 검증→소비자→공지). **백엔드 도메인 완성** · ⑤ **프런트엔드 완성**: 데이터 레이어(타입+훅 30) · 소비자(목록/상세/참가신청 플로우/명단/내신청) · 어드민(목록/생성/상세 3탭: 신청확정·대진·공지) · 홈 오늘의추천 대회 실데이터. 전 화면 라이브 스크린샷 검증(production fidelity, 콘솔/네트워크 에러 0). **대회 기능 end-to-end 완성**. 잔여 폴리시(비차단): 어드민 대진 탭 픽스처 팀명 표시(현재 regId), 어드민 공지 탭 기존 목록 표시(생성 후 새로고침 필요). v1_api jest 268 + v1_web 7 통과.
