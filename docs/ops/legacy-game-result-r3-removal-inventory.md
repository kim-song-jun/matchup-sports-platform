...
>
> **후속 주의(Task 10 컷오버 플래그 정리, 이 메모 작성 시점 이후).** §4의 1·2단계가 참조하는
> `GAME_READ` 운영 플래그·`CompareGameReadAuthorityService`·`task10-runtime-manifest.cli.ts`·
> `scripts/qa/verify-game-result-cutover.mjs`는 **전부 삭제됐다** — 컷오버가 이미 완료·영구
> 확정됐다고 판단해(alpha가 `GAME_READ=new`로 롤백 없이 충분히 오래 안정적으로 서빙 중이었음)
> 그 플래그와 부속 도구 일체를 걷어냈다. 즉 1·2단계가 확인하려던 "발산 0건 관찰 기간"이라는
> **사실 자체는 이미 충족된 상태로 컷오버가 끝난 것**으로 보이지만, 그 사실을 확인하던
> **메커니즘(§4 1·2단계에 적힌 그 절차)은 더 이상 실행할 수 없다.** R3를 실제로 착수하는
> 사람은 1·2단계를 문자 그대로 따르지 말고, 다른 방법으로(예: 컷오버 완료 시점의 운영 기록·
> 배포 이력 확인) 그 전제조건이 이미 성립했는지부터 별도로 재확인해야 한다. §1·§2·§3(실제
> 호환 리더 인벤토리)과 §4의 3단계 이후(리더 교체 절차)는 이 정리로 인한 영향을 받지 않는다 —
> 이 메모는 §4 1·2단계에만 해당한다.
