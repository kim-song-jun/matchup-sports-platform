...
| 자리 | 안 막혔을 때 무슨 일이 나나 | 마감 |
|---|---|---|
| `league-series-admin` 시리즈 최초 생성 | **거울이 아예 없다** → 그 리그가 read-swap 뒤 **화면에서 사라진다** | **필수.** 못 막으면 `--apply` 를 미룬다 |
| `league-completion-projection` | 거울 status 가 `in_progress` 로 남는다 → 끝난 리그가 **진행 중으로 보인다** | **선택.** 못 막으면 **승인 요청에 이름과 실패 모습을 그대로 적고** 진행한다 |

**가르는 축은 "행이 없는가" vs "값이 틀린가"다.** 행이 없으면 화면에서 사라지고 운영자는
"안 보인다"밖에 말할 수 없다. 값이 틀리면 보이긴 하므로 발견되고, `--apply` 의 거울 수
불변식과도 무관하다(개수는 맞다).
