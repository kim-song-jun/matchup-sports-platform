...
대상 독자: 서비스 기획·디자인·운영
검증 환경: local `dev` Web `3013` / API `8121` 및 `https://alpha.teameet.co.kr`
상태: `dev` CI·alpha 자동 배포·실사용자 E2E·최신 DPR1 시각 검증 완료. 보안 승격 차단 항목은 별도 조치 필요

## 0. 최신 alpha 확정 결과 (2026-07-18)

| 항목 | 확정 근거 |
|---|---|
| 배포 source | `9bf8e5d3812766d0f52a186900f1846ea1302a1a` |
| alpha version | `0.1.0-alpha.20260718.g9bf8e5d38127` |
| CI | GitHub Actions run `29641356982` 성공, 3분 38초 |
| alpha 배포 | GitHub Actions run `29641356985` 성공, 10분 19초 |
| 공개 응답 | `X-Teameet-Commit`이 전체 source SHA와 일치하고 `X-Teameet-Release`가 alpha version과 일치 |
| API health | `/api/v1/health` HTTP 200, `service=v1_api`, `checks.db=true` |
| 실제 E2E | 회원가입, 프로필 이미지 업로드·재표시, 팀 생성, 대회 참가 신청, 이벤트 lifecycle, 진행/완료 대회, 영상, 개인 어워드, 관리자 대진표 확인 PASS |
| 권한 E2E | 대회 팝업의 owner 편집 권한과 support 읽기 전용 권한을 각각 실제 화면에서 확인한 뒤 임시 관리자 권한을 정리하는 운영 절차로 인계 |

CI는 Changeset/SemVer, v1 lint·typecheck, 빈 DB migration replay, API/Web unit, API/Web build를 커밋본 기준으로 모두 통과했다. 로컬에서는 호스트 부하와 중복 검증 비용을 고려해 전체 Web unit suite를 다시 실행하지 않았고, GitHub CI 결과를 최종 자동 검증 근거로 사용했다.

### 프로덕션 승격 전 보안 차단 항목

아래 항목은 이번 기능 수정에 포함하지 않았으며, 해결 또는 명시적 위험 수용 전에는 alpha를 안전한 공개 QA 환경이나 프로덕션 승격 근거로 간주하면 안 된다.

- **CRITICAL — 배포 대상 fail-closed 부재:** migration·비식별화 전에 대상이 실제 alpha DB/호스트인지 강제로 증명하는 차단 장치가 충분하지 않다. 잘못된 target이면 데이터 변경 전에 즉시 종료해야 한다.
- **CRITICAL — 불충분한 데이터 비식별화:** alpha에는 대부분 비식별화되지 않은 production DB clone과 uploads가 남을 수 있다. 이메일·전화번호·password hash·OAuth 식별자·개인정보·관리자 권한이 포함될 수 있으며, production credential이 alpha에서도 유효할 가능성을 배제하지 못했다.
- **CRITICAL — production SSH host 검증 해제:** production 배포 경로의 `StrictHostKeyChecking no`는 중간자 공격과 잘못된 호스트 접속 위험을 만든다.
- **HIGH — 업로드 전체 용량 방어 부재:** per-user quota만으로는 다수 계정 생성에 의한 디스크 소진을 막지 못한다. 서비스·호스트 단위 quota와 관측·차단이 필요하다.
- **HIGH — 과거 대진표 자동 공개:** historical bracket backfill이 명시적 관리자 승인 없이 공개 상태를 만들 수 있다.
- **MEDIUM — 브라우저/공급망 노출:** 임의 원격 이미지 URL의 tracking·origin 노출, CSP의 `unsafe-inline`, mutable GitHub Action·container image tag가 남아 있다.
- **LOW — 업로드 한도 불일치:** Nginx 약 55MB와 API 약 200MB 제한이 달라 사용자가 API에 도달하기 전에 다른 오류를 받을 수 있다.

AWS 계정의 CloudTrail, IAM, S3 encryption/public-access block, EBS encryption, security group, SSM session logging 같은 외부 제어는 이번 브라우저·코드 검증만으로 증명하지 않았다.
...
### 5.1 최신 alpha DPR1 증거

최신 alpha의 canonical raw evidence 경로:

`output/playwright/visual-audit/2026-07-18-alpha-f2f1d720/`

09~15번은 DPR2/crop 문제로 최종 증거에서 제외한다. 16~25번만 DPR1 화면 증거로 사용하며, 18번 어워드 화면 뒤에 animation 제거가 반영되었으므로 어워드 최종 판정은 22번으로 대체한다.

| # | 화면·상태 | 뷰포트 | 판정 |
|---:|---|---|---|
| 16 | 공개 캠페인 상세 | 390×844 | PASS |
| 17 | 이벤트 허브 lifecycle 목록 | 390×844 | PASS |
| 18 | 개인 어워드 화면, remediation 이전 비교 증거 | 390×844 | REPLACED BY 22 |
| 19 | 대회 참가 신청 | 768×1024 | PASS |
| 20 | 이벤트 허브 | 1440×900 | PASS |
| 21 | 관리자 대진표·경기 결과 | 1440×900 | PASS |
| 22 | `22-awards-mobile-390-latest.jpg` — animation 제거 후 개인 어워드 | 390×844 | PASS |
| 23 | `23-team-sole-owner-mobile-390-latest.jpg` — 유일 소유자 팀 탈퇴 차단 | 390×844 | PASS |
| 24 | `24-admin-popup-owner-wide-1440-latest.jpg` — owner 팝업 생성·편집 권한 | 1440×900 | PASS |
| 25 | `25-admin-popup-support-wide-1440-latest.jpg` — support 읽기 전용, 추가·편집·삭제 없음 | 1440×900 | PASS |

최신 캡처는 가로 overflow 없이 실제 alpha 응답을 렌더링했다. 완료 대회의 WebM 영상은 브라우저에서 `readyState=4`, 약 3초 재생 길이로 확인했고, 개인 어워드의 장식 animation은 최종 화면에서 `animation-name: none`, `0s`로 확인했다. owner/support 권한 비교는 같은 대회 팝업에서 수행해 역할 외 조건을 고정했다.

배포 seed는 고정 alpha lifecycle 데이터를 재구성하므로 QA 중 생성한 일시적 참가 신청 row가 다음 배포에서 초기화될 수 있다. 이 동작을 사용자 데이터 영속성 PASS로 해석하지 않으며, 참가 신청 API·화면 계약 검증과 배포 후 seed 상태 검증을 분리한다.
...
DB 복원과 local event seed의 기존 22개 화면 검증에 더해, source SHA `9bf8e5d3…`의 CI와 alpha 배포, 회원가입부터 관리자 대진표까지의 실제 E2E, 최신 DPR1 16~25번 증거가 PASS다. 공개 header와 API health/DB 상태도 같은 SHA에서 일치한다.

기능·배포 동작에 대한 QA 판정은 PASS지만 보안 판정은 **BLOCKED**다. 특히 잘못된 배포 대상 차단, production clone 전체 비식별화, production SSH host key 검증은 프로덕션 승격 전에 해결해야 한다. 또한 GitHub Actions의 Node 20 action runtime이 Node 24로 강제 전환된다는 경고가 있어 action major/version compatibility를 후속 확인해야 하며, alpha SSM·순차 Docker build가 포함된 배포는 현재 10분 19초로 CI 3분 38초보다 길다. 무거운 테스트를 추가하기보다 SSM 대기·artifact 전달·API/Web image build step timing을 분리해 병목을 줄이는 것이 다음 운영 최적화 대상이다.
