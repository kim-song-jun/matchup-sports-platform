...
`05 · 레슨 Academy` includes:

- `lessons-academy-hierarchy`: Academy Hub is the canonical lesson root IA, with course, coach, trial, ticket, workspace, and exception layers.
- `lessons-state-edge`: empty, loading, offline error, sold out, deadline, pending coach review, disabled ticket, success states.
- `lessons-ticket-lifecycle`: active, remaining zero, expired, refund window, pending payment, coach-hold states.
- `lessons-schedule-exceptions`: class cancelled, reschedule request, sold out, duplicate trial, no-show history, coach unavailable states.
- `lessons-control-interactions`: schedule chips, pass selection, amount-changing sticky CTA, bottom sheet, disabled coupon state.
- `lessons-motion-contract`: hub push, schedule chip, ticket sheet, payment submit, coach reschedule, refund request transitions.
- `lessons-responsive`: mobile Academy Hub, tablet hub/detail split, desktop filter/result/action rail comparison.
- `lessons-dark-mode`: light/dark lesson comparison for ticket state, urgency, disabled, progress, sticky CTA contrast.
