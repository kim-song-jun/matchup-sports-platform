...
## 8.5 Personalization & Variant System

The prototype directions marked `A`, `B`, and `C` in the Remix design assets are not throwaway explorations. They are productized as **user-selectable interaction presets** that can be changed in Settings and persisted per user.

### Variant Families

| Surface | Variant A | Variant B | Variant C | Product Rule |
|------|------|------|------|------|
| Home / Check-in | Slide-to-clock-in | Big tap button | Card stack | Same attendance actions, different interaction shell |
| Team Status | Avatar grid | Team-grouped list | Timeline | Same status data, different scanning pattern |
| Widgets | Compact / summary-first | Balanced | dashboard-like | OS size and user choice decide layout |

### Behavior Rules

- Variants are stored as user preferences, not organization-wide experiments.
- The underlying domain actions must remain identical across variants.
- A user can choose their preferred check-in style and team view style in `Settings > 화면 꾸미기`.
- Organization policy may set the default variant, but users can override unless policy locks the choice.
- Accessibility settings such as font size, theme mode, and language must work consistently across all variants.

### Design Constraint

- Variants may change gesture and information density, but must not change the meaning of states, approval semantics, or location confirmation rules.
- All variants must preserve the same visual language: Toss-like restraint, calm white surfaces, sparse accent usage, and low cognitive load.

### Mobile Admin Design

Manager/admin functionality should also be available on mobile, but in a more compressed form than desktop admin.

- Prefer summary cards, compact inbox rows, and bottom-sheet decisions over dense tables.
- Approval actions must be reachable with one thumb in the lower half of the screen.
- Risk-heavy content such as expiring leave and pending approvals should surface as top-level cards.
- Policy editing on mobile should be limited to safe quick actions; full matrix editing remains more appropriate on larger screens.
