...
4. **[5.1.1(v)] 삭제가 "요청 → 운영자 확인" 2단계이고 처리 기한 안내가 없다** — Apple 은 수동 절차를 허용하되
   ...
   **보류(2026-09-26)**: 약관·개인정보처리방침에는 일 단위 기한이 없고 "지체 없이"뿐이라(`terms-client.tsx` 8절)
   화면 문구의 근거가 없다. 기한(유예기간)은 미결 결정이다(`docs/superpowers/specs/2026-08-17-privacy-legal-compliance-design.md` Q2).
   기한을 정해 방침에 먼저 적은 뒤 탈퇴 화면·이 메모에 같은 값을 반영한다.
