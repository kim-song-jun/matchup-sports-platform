...
- [ ] 호스트가 `나도 참가해요`를 끄고 매치를 생성해 본인이 현재 인원에서 제외되는지 확인한다.
- [ ] 같은 매치를 수정해 주최자 참가를 켜고/끄며 현재 인원과 정원 검증이 함께 바뀌는지 확인한다.
- [x] 호스트가 본인 매치 상세에서 모집 마감 후 다시 모집중으로 되돌린다.
- [x] 호스트가 수정 페이지로 진입해 제목, 설명, 모집 인원을 수정한다.
- [ ] 수정 페이지에서 종목, 지역, 제목, 설명, 이미지, 인원, 레벨, 성별, 규칙, 장소/주소, 경기 일시, 신청 마감을 모두 변경하고 상세에 반영되는지 확인한다.
- [x] 호스트 상세에서 `매치 수정`과 `신청자 관리`가 별도 CTA로 노출되고 각각의 실제 경로로 이동한다.
- [x] 호스트가 취소 또는 완료 처리를 수행한다.
- [x] 상세, 참가자 view, 내 기록에서 변경 결과를 확인한다.

### Expected

- [ ] 주최자 참가를 꺼도 호스트의 수정·신청자 관리·마감·완료 권한은 유지된다.
- [ ] 참가하지 않는 호스트는 경기 완료 시 참가자로 다시 생성되거나 완료 인원에 포함되지 않는다.
...
- [ ] 팀매치 수정에서 이미지, 지역, 장소/주소, 경기조건, 비용, 경기 일시, 신청 마감을 변경하고 목록·상세·재수정 화면에 동일하게 반영되는지 확인한다.
