...
- [ ] [15-focused-full-flow-test-matrix.md](./15-focused-full-flow-test-matrix.md) - in progress: Task 122가 alpha 프로필·대회 persona E2E를 canonical v1 route와 6개 상태 fixture로 좁혔고 API/Web readiness는 확인했으며, Codex 재시작 후 live profile/upload/permission/media/admin 검증이 남음
...
| `15-focused-full-flow-test-matrix.md` | Manual route/API matrix + Task 122 alpha persona execution | In progress: canonical `/my`·`/my/profile/edit`와 6개 alpha 대회 상태를 고정했고 API/Web readiness를 확인했다. Live profile/upload/role/media/admin evidence and narrow v1 Playwright coverage remain host-gated. |
