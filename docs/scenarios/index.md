...
- [ ] [10-profile-settings-admin.md](./10-profile-settings-admin.md) - partial: Task 103 customer `/admin` ERP verified; Task 104 `/ops` internal console routes/API passed backend/frontend coverage and final protected-route browser QA evidence; Task 105 focuses `/admin` + `/ops` shared bright operations template while preserving customer/operator flows; broader profile/onboarding verification remains pending
...
- [x] [13-v1-open-design-recovery-from-zero.md](./13-v1-open-design-recovery-from-zero.md) - Verified: Open Design Recovery From Zero completed for 109-page export; current v1 route matrix is 98 routes / 177 rows after Task 104 `/ops` addition. Historical 87/92-route sweeps remain reference evidence, and Task 104 live admin/ops QA evidence is `output/playwright/visual-audit/task104-admin-ops-split/summary-final.json`
...
| 2026-06-08 | Task 104 admin/ops split | Passed with final browser evidence | `/admin` customer ERP contract remains in Task 103, and internal reports/disputes/payments/refunds/settlements/payout/audit moved to `/ops`. Backend Prisma/API/Toss boundary, frontend `/ops` shell, MSW fixtures, docs, and final route-aware browser evidence passed 59/59 captures. | Keep Task 104 verification fresh after payment/webhook hardening changes; broader profile/onboarding items still keep scenario 10 partial. |
| 2026-06-08 | Task 105 admin/ops template redesign | Passed | `/admin` and `/ops` now share one bright Teameet operations template while preserving customer admin links and internal ops guard/action/provider-failure flows. Focused frontend tests, typecheck, build, route matrix, and final browser QA passed. Evidence: `output/playwright/task105-admin-ops-template-redesign/final/summary.json` (`59/59`). | Broader profile/onboarding items still keep scenario 10 partial; Task 105 scope is complete. |
