...
- [ ] [12-v1-sm-new-e2e-scenarios.md](./12-v1-sm-new-e2e-scenarios.md) - drafted: SM New v1 1차 디자인 완료 + API/DB 설계 기준 E2E 시나리오 매트릭스. Task 135에서 V1-14-008 진행 중 대회 fixture 리뷰의 역할별 대상·개인화 CTA·승부차기 표시 계약과 390/768/1440 시각 증거를 갱신 중
...
Todo 26 of `.omo/plans/teameet-team-tournament-operations-v1.md` requires this index to name all seven v1 E2E scenario IDs Tasks 12-24 introduced, with a pointer to their home suite. All seven live under `e2e/v1-tests/` (config `e2e/v1.config.ts`), never under the legacy `e2e/tests/` suite. As of this revision (2026-08-04), **none of the seven IDs has any implementation** in `e2e/v1-tests/*.spec.ts` — confirmed by grepping the entire `e2e/`, `docs/`, and `.omo/` trees for each literal ID string (zero hits anywhere except this table and the domain scenario files below). Do not read a row below as `Verified`; every row is `Unimplemented` until a later pass adds the actual Playwright assertions to its primary spec.

| ID | Covers | Primary spec (home suite) | Owning scenario doc |
|---|---|---|---|
| `E2E-TEAM-01` | Opponent lineup-change authorization on a team match (`POST /team-matches/:teamMatchId/lineup/change-request` reachable only by the approved opponent). | `e2e/v1-tests/team-match.spec.ts` | [05-team-match-flows.md](./05-team-match-flows.md) |
| `E2E-TEAM-02` | Host result submit → opponent decision round trip via the Game result-revision routes (`POST /games/:gameId/result-revisions[/:revisionId/submit\|/decision]`), including the `matched → completed` transition. | `e2e/v1-tests/team-match.spec.ts` | [05-team-match-flows.md](./05-team-match-flows.md) |
| `E2E-TOUR-01` | Result review flow: rejected/supplement-requested → supersede-and-submit, wrong-base rejection, atomic successor rollback (Task 22 `tournament-result-review` routes). | `e2e/v1-tests/tournament.spec.ts` | not yet a dedicated section — see `docs/api/domains/tournament-operations.md#result-review-officialize-correction-and-void-task-22` for the backend contract |
| `E2E-TOUR-02` | Headed, controlled-network live tournament commands: end→submitted atomicity/recovery, clock drift, duplicate/late events, 5-minute offline token reacquire/rebase, takeover/revoke (Task 20/21 live-ops surface). Per the plan (line ~550), this is executed later by V26 as a headed, controlled-network run — it is not a standard CI-parallel spec. | `e2e/v1-tests/tournament.spec.ts` (headed/controlled-network variant, not the default CI project) | not yet a dedicated section — see `docs/api/domains/games.md` (event/command routes) and `docs/api/domains/tournament-operations.md` (operations board) |
| `E2E-CORR-01` | `platform_ops` void plus `DIRECTOR_OFFICIALIZE` flag off→on→off UI/API behavior (Task 22 correction/officialize/void routes). | `e2e/v1-tests/tournament.spec.ts` | not yet a dedicated section — see `docs/api/domains/tournament-operations.md#result-review-officialize-correction-and-void-task-22` |
| `E2E-AUTH-01` | Tournament staff actor-authorization matrix (`public`/`authenticated_user`/`member`/`team_manager`/`team_owner`/`opponent_manager`/`platform_ops`/`tournament_director`/`field_operator`/`support_readonly`) per `docs/api/domains/tournament-operations-auth.md`. Named by Todo 26 itself — no other todo in the plan defines its content. | `e2e/v1-tests/tournament.spec.ts` | [17-tournament-gender-wizard.md](./17-tournament-gender-wizard.md) (`## Related` section) |
| `E2E-PUBLIC-01` | Public visibility matrix (`hidden`/`status_only`/`live`/`official_only`) plus consent revoke/regrant on the Task 24 public-records routes (`GET /tournaments/:id/schedule`, `.../matches/:fixtureId`, `GET /teams/:id/records`, `GET /users/:id/records`). Per the plan, headed 390×844/1440×900 evidence is also owned and executed later by V26. | `e2e/v1-tests/tournament.spec.ts` | [11-team-and-venue-hubs.md](./11-team-and-venue-hubs.md) |
...
| 2026-08-04 | Todo 26 scenario doc reconciliation (04/05/11/17 + index) | Updated | `04-team-and-membership.md`, `05-team-match-flows.md`, `11-team-and-venue-hubs.md` described only the legacy `apps/api`/`apps/web` stack (team hub, `/team-matches/:id/result`+`check-in`+`evaluate`, `/teams/:id/hub`+`/venues/:id/hub`) with no v1-stack section at all — none of that legacy prose was found false for the stack it describes, but reading it as v1 coverage would have been wrong (v1 has a different `TeamsController`/`TeamMatchesController`, a Game-aggregate result model per Task 16, and no hub/venue domain at all). Added a `## v1 stack (Tasks 12-24)` section to each of the three files, verified directly against the live controllers (`apps/v1_api/src/teams/teams.controller.ts`, `apps/v1_api/src/team-matches/team-matches.controller.ts`) and `docs/api/domains/{team-schedules,games,tournament-operations,public-records}.md`, plus a `## Related` cross-link in `17-tournament-gender-wizard.md` to `E2E-AUTH-01`. Added the `## v1 Team & Tournament Operations Scenario Ledger` section naming all 7 v1 E2E IDs (`E2E-TEAM-01/02`, `E2E-TOUR-01/02`, `E2E-CORR-01`, `E2E-AUTH-01`, `E2E-PUBLIC-01`) with a pointer to `e2e/v1-tests/` and their owning scenario doc. | None of the 7 E2E IDs has Playwright coverage yet (grepped `e2e/v1-tests/*.spec.ts` for each literal ID, zero hits) — implementing them, and resolving the 18-screen-ID ledger (also confirmed absent from the plan file and `docs/`), remain follow-up work this doc-reconciliation pass does not itself close. |
