...
flowchart TB
    ...
    subgraph Edge["nginx — deploy/nginx.conf"]
        ...
        E3["rate limit /v1/api → 60r/m · M1"]
        E4["rate limit /v1/api/auth → 10r/m · M1"]
        E1 --> E2 --> E3 --> E4
    end
...
| 계층 | 파일 | 이 PR의 강화 |
|------|------|--------------|
| Edge (nginx) | `deploy/nginx.conf` | TLS 프로토콜/cipher 고정(M3), HSTS·CSP(M2), `/v1/api` 및 `/v1/api/auth` rate limit(M1) |
