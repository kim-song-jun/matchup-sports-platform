...
- 상태: **구현 준비 완료 — §14 미결 3건 2026-08-18 사용자 확정** (4항목 개별 채점 방향에 이어 공개 임계값·레거시 payload·이상 탐지 자동화 수준까지 확정)
...
**권고: 현행 유지.** 두 옵션 모두 §1.2에서 검증된 익명성 보호 수준을 그대로 유지하므로 보안·프라이버시 트레이드오프는 없다 — 순수하게 "공개 속도 vs 문서 문구 일치" 트레이드오프다.

**확정(2026-08-18 사용자): 현행 유지.** 따라서 이 스펙의 어떤 태스크도 공개 게이트를 바꾸지 않는다 — `review-visibility.ts`의 상호제출/72h 판정은 그대로 두고, 문서01의 "3건"은 이미 존재하는 `trustState` 등급 배지가 담당하는 것으로 정리한다(§14).
...
## 14. 미결 — **2026-08-18 사용자 확정**

아래 3건은 사용자가 결정했다(결정 아티팩트로 옵션·장단점을 비교한 뒤 회신). 네 번째
항목은 코드 확인으로 해소했고, 다섯 번째는 데이터가 쌓인 뒤의 재검토 트리거라 착수를
막지 않는다.

| # | 항목 | **확정** | 근거 |
|---|---|---|---|
| D-3 | 후기 공개 임계값 | **현행 유지 — 상호제출 OR 72h(쌍 단위)** | §3.1 권고안과 동일. 팀 2~3개 대회가 영구 "집계 중"에 갇히는 회귀를 피하고, 3건 기준이 `trustState` 배지와 이중으로 존재하는 혼란을 만들지 않는다 |
| — | 레거시 payload | **동시 배포 — 이중 지원 없음** | 권고안(짧은 이중 지원)과 **다른 선택**. 이중 경로가 남기는 기술부채를 피하는 쪽을 택했다(원칙 1). 배포 순서 리스크는 아래 "실행 조건"으로 제거한다 |
| — | 이상 탐지 자동화 | **관찰 모드로 시작 — riskScore만 기록, FLAGGED 전이는 수동** | 임계값이 아직 근거 데이터 없는 추정치라, 오탐이 정상 후기를 숨기고 신뢰점수를 흔드는 사고를 먼저 막는다. 실데이터로 튜닝한 뒤 자동 전이를 켠다 |

**레거시 payload "동시 배포"의 실행 조건 (필수).** 이 선택은 프론트·백엔드가 서로 다른
배포에 실리는 순간 기존 클라이언트가 400을 받는다 — 이 저장소는 dev 머지가 곧 alpha
실배포라 두 PR의 타이밍을 사람이 맞출 수 없다. 따라서 **`POST /reviews`의 스키마 변경과
그 호출부 프론트 변경을 같은 PR·같은 커밋 범위에 넣어 원자적으로 배포한다**(별도 PR로
쪼개지 않는다). 이 조건을 지키지 않으면 선택 B는 그대로 장애가 된다.

**해소됨 — `review-summary` 전용 엔드포인트는 존재하지 않는다.** `apps/v1_api/src`
전체에 `review-summary` 문자열이 없다(2026-08-18 확인). §9.3은 "응답 셰이프 확장"이
아니라 **신규 엔드포인트 추가**로 진행한다.

**남은 재검토 트리거(착수 비차단)**: 프로덕션 리뷰 실제 건수 — §8.2. 데이터가 쌓인 뒤
이상 탐지 임계값을 튜닝하고 자동 전이 전환 여부를 다시 판단한다.
