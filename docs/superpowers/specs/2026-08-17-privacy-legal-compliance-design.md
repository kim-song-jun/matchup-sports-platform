...
- **리비전 노트 — 2026-08-18 정책 변경**: 사용자가 대회 기록 참가자 이름의 **2026-08-13 상시 실명 공개 결정을 번복**하고 **닉네임 기본 표시 + 프로필 실명 표시 토글**로 확정(§4 P-9). 이에 따라 §5.2를 "실명 공개 정합 — 동의 목적 추가 vs 게이팅 활성화"에서 "대회 기록 이름 표시 정책"으로 전면 재작성, §10.1-L1(실명 공개 소급 법무 질문)을 해소됨으로 표시, §10.2-Q1(실명 공개 방향 선택)을 §4 확정 표(P-9)로 이관했다. 같은 결정으로 직접 모순되던 §3 목표/비목표 문구, §8.2 PR 분할표의 "법무 게이트" 서술, §9 리스크 표의 "동의서 개정 소급" 행도 함께 정정했다. **파기·hold 관련 설계(§5.1, §5.1.2~5.1.5, §7, §8.1, §8.3의 파기 롤백, §9의 파기 리스크 행들)는 이번 리비전의 대상이 아니다 — 그대로다.** §1.2(노출 표면 실태 표의 "롤백 스위치" 행)·§8.3(실명 공개 롤백 스위치 문구)은 구 A/B 프레이밍을 그대로 남긴 **후속 정리 필요 항목**이다(이 태스크의 배정 범위 밖 — §5.2의 "남는 스위치" 참고 문단 참조).
...
2. **대회 기록 이름 표시 정합**: 공개 대회 기록의 참가자 이름을 **닉네임 기본 + 프로필 실명 표시 토글**로 전환해, 동의 근거 없는 실명 공개라는 문제 자체를 없앤다 (2026-08-18 정책 변경 — 2026-08-13 결정 대체, §5.2, §4 P-9).
...
- ~~공개 기록 실명 정책 자체의 번복 (오너 결정 존중 — 정합만 맞춘다)~~ — **2026-08-18 사용자 결정으로 이 항목 자체가 목표로 전환됐다**(위 목표 2). 이번 스코프에 여전히 없는 것: 프로필 실명 표시 토글 UI 컴포넌트·문구, 대회 기록 조회 경로의 조인 로직 구현 — 이건 프론트·백엔드 구현 트랙 소관이고 이 스펙(정책 정합)의 범위가 아니다
...
> 이 표에는 **이 스펙이 확정하는 결정만** 넣는다. 법무·사용자 게이트가 필요한 항목(3년 만료 시 realName 처분, 제재 이력 보유자의 파기 유예 방식)은 **전부 §10 미결**에 있다 — 미확정을 확정 표에 넣지 않는다. (**대회 기록 참가자 이름 표시 방향**은 2026-08-18 사용자 결정으로 확정되어 §10.2-Q1에서 이 표의 P-9로 이관됐다 — §5.2. **탈퇴 유예기간**은 2026-09-26 사용자 결정(30일)으로 §10.2-Q2에서 P-10으로 이관됐다 — L2 법무 확인은 남아 있다)
...
| P-9 | 대회 기록 참가자 이름 표시 정책 | **닉네임 기본 표시 + 프로필 실명 표시 토글**(개별 온오프, 대회 신청 시 별도 동의 절차 없음). `V1GameParticipant.userId` 있음+토글 ON → 실명 / `userId` 있음+토글 OFF(기본값) → 닉네임(`displayName ?? nickname`) / `userId` 없음(게스트·미연동) → 기존 `displayNameSnapshot` 그대로. **2026-08-13 결정(상시 실명 공개)을 대체** — 상세 §5.2 | 사용자 확정 (2026-08-18). 구 §5.2 v2의 "동의서 개정(A) vs 게이팅 스위치 활성화(B)" 저울질을 대체 — 기본값을 닉네임으로 바꾸면 두 갈래가 풀려던 "동의 근거 없는 실명 공개" 문제 자체가 없어진다 |
| P-10 | 탈퇴 유예기간 | **30일.** 탈퇴 요청(`withdrawal_pending`) 후 30일 동안 계정을 보관하고, 그 사이에는 고객센터를 통해 어드민이 `active` 로 복구할 수 있다. 지난 뒤 계정과 개인정보를 파기(§5.1.2 잡 A의 유예 만료 기준). 개인정보처리방침 v1.4 7절·탈퇴 화면 안내에 같은 값을 반영 | 사용자 확정 (2026-09-26). §10.2-Q2에서 이관. **L2 법무 확인(유예 운영 가능 여부·약관 반영 방식)은 여전히 남아 있다** — 이 결정은 L2 결과에 따라 문구·기간이 바뀔 수 있다 |
...
### 5.2 대회 기록 이름 표시 정책 (2026-08-18 정책 변경)

**정책(사용자 확정, 2026-08-18)**: 대회 경기 기록(라인업/득점자/MVP)의 참가자 표시 이름을 **닉네임 기본**으로 바꾼다. **실명 표시는 각 사용자가 자기 프로필에서 개별적으로 켜고 끄는 토글**로 관리하며, 대회 신청 시점마다 별도 동의를 묻지 않는다 — 프로필에서 한 번 켜면 계속 적용되고 언제든 끌 수 있다.

| 대상 | 표시 |
|---|---|
| `V1GameParticipant.userId` 있음 + 프로필 실명 표시 토글 ON | 실명 |
| `userId` 있음 + 토글 OFF (**기본값**) | 닉네임(`displayName ?? nickname`) |
| `userId` 없음 (게스트·미연동) | 기존 `displayNameSnapshot` 그대로 |

**구현 전제 (기존 실태 재확인)**: `displayNameSnapshot`은 브라켓 생성 시점에 `V1TournamentPlayer.realName`이 그대로 찍히는 불변 스냅샷이며(`tournament-bracket.service.ts:479-488`), 현재 공개 기록 조회 경로에는 `V1User`로의 라이브 조인이 없다(`public-tournament-records.service.ts:829-831` 주석 — "`V1User`로의 라이브 조인이 아예 없다"). 토글이 즉시 반영되려면 표시 시점에 프로필을 조인해야 한다 — 조인 키로 쓸 `userId`는 참가자 생성 시 스냅샷과 나란히 이미 저장되고 있으므로(`schema.prisma:2671`, `tournament-bracket.service.ts:479-487` — 각 참가자 객체가 `userId`와 `displayNameSnapshot`을 함께 채운다) 신규 컬럼 없이 조인이 가능하다. 스냅샷 문자열 자체는 `userId`가 없거나 조인이 실패하는 경우의 fallback으로만 남는다. **토글 필드의 정확한 스키마 위치·엔드포인트 설계는 이 스펙(법무 정합 스펙)의 범위가 아니다** — 구현 트랙에서 별도 확정한다.

#### 왜 동의서 개정도 게이팅 스위치 활성화도 더 이상 필요 없는가

이 절의 이전 버전(v2)은 "A. 동의서 v1.2 개정(목적 추가) vs B. 롤백 스위치(`V1_TOURNAMENT_PARTICIPANT_NAMES_CONSENT_GATE=true`)로 게이팅 복귀"를 놓고 저울질했다. 둘 다 전제가 같았다 — **"실명을 계속 공개하되, 그 공개의 법적 근거를 어떻게 확보·회복할지"**. A는 동의 목적을 늘려 근거를 새로 만들려 했고, B는 근거 없는 사람을 화면에서 빼려 했다. 그리고 둘 다 한계가 있었다:
- A(동의서 개정): **소급 문제** — 이미 구 버전에 동의한 참가자·이미 종료된 대회 기록에는 새 목적이 자동 적용되지 않는다. 대회별 동의는 신청 시점 1회라, 가입 약관처럼 재동의 게이트(`terms-reconsent-access.ts`)가 자동 적용되지 않는다 — 다음 신청부터만 신 버전이 적용된다.
- B(게이팅 복귀): 오너 결정(상시 공개) 자체를 번복한다. 게스트(동의 수단이 없는 참가자)는 영구 비노출되어 "등록해 뛴 것 자체가 공개"라는 정책 의도와 다시 충돌한다.

**새 정책은 이 저울질 자체를 없앤다.** 기본 표시를 실명에서 닉네임으로 바꾸면, "동의 근거 없이 실명이 노출되는 사람"이라는 문제의 대상 집합이 구조적으로 사라진다 — 실명은 그 사람이 프로필에서 직접, 언제든 철회 가능한 형태로 켠 경우에만 보인다. 이건 "목적 외 이용 문제를 해소할 별도 법적 근거(동의서 개정)를 마련"한 게 아니라, **애초에 목적 외 이용이 발생할 조건 자체가 생기지 않도록 기본값을 설계**한 것이다. 대회 신청 시점 동의를 요구하지 않는 이유도 같은 논리다 — 신청 시점 동의는 "그 대회에서 공개할지"를 묻는 것인데, 새 정책에서 공개 여부는 대회 신청과 무관하게 프로필에서 상시·개별적으로 결정되므로 신청 시점 동의가 규율할 대상이 남지 않는다. **소급 문제도 발생하지 않는다** — 토글은 등록 시점의 동의 버전을 참조하지 않고 표시 시점의 현재 값을 그대로 조인하므로, "구 버전에 동의한 과거 참가자"라는 구분 자체가 성립하지 않는다.

#### 실명 수집은 계속한다 — 수집과 공개를 분리

이 정책은 **실명 수집을 중단하지 않는다.** `V1TournamentPlayer.realName`은 여전히 필수 not-null 컬럼이고(`schema.prisma:2218-2226`), §1.3이 나열한 대회 PII 이용 목적 10개 중 ③참가 자격 검토 ④선출·비선출 구분에 실제로 쓰인다 — 팀 manager+·어드민의 로스터 조회(§5.4)에서 실명은 그대로 노출되며, 이 경로는 이번 변경의 대상이 아니다. 바뀌는 것은 **비로그인으로 접근 가능한 공개 대회 기록 API가 무엇을 내보내는가**뿐이다. "수집 목적"과 "공개 게시"를 사실상 동일시했던 것이 2026-08-13 결정의 암묵적 전제였는데, 이번 결정은 그 둘을 명시적으로 분리한다 — 실명은 계속 수집·내부 이용되지만, 공개 게시라는 별도 처리는 이제 데이터 주체 본인의 상시 철회 가능한 선택을 거친다.

#### 2026-08-13 결정을 뒤집는다는 사실

이 정책은 2026-08-13 결정(`public-tournament-records.service.ts:820-828` 주석 — "대회 참가자 이름 공개 정책... 계정 연동·동의(Task 24 consent) 여부와 무관하게 항상 공개된다")을 **번복**한다. 그 결정의 원래 근거는 주석에 그대로 남아 있다: "'대회에 선수로 등록해 실제로 뛰었다'는 사실 자체가 공개 활동이라는 전제" — 참가라는 행위 자체를 실명 공개에 대한 사실상의 동의로 간주했다. 새 정책은 이 전제를 더 이상 유지하지 않는다: **"경기가 열렸다·누가 득점했다"는 참가·기록 사실의 공개**와 **그 기록에 붙는 이름이 실명인지 닉네임인지**는 별개 문제이며, 후자는 각 개인이 자기 프로필에서 결정할 문제로 옮긴다. 참가 사실·기록 자체(득점·MVP·순위·경기 결과)는 정책 변경과 무관하게 계속 공개된다 — 바뀌는 것은 그 기록에 붙는 이름의 형태뿐이다.

> **참고— 남는 스위치**: 롤백 스위치 `V1_TOURNAMENT_PARTICIPANT_NAMES_CONSENT_GATE`(Task 24 동의 게이팅으로 복귀, `public-tournament-records.service.ts:837-839`, §1.2)는 이번 결정으로 목적을 잃은 별개의 구 메커니즘이다. 이 스위치의 존치·정리 여부는 이번 태스크의 범위 밖이며, §1.2·§8.3·§9가 참조하는 구 A/B 프레이밍과 함께 별도 후속 정리가 필요하다(이 문서 하단 리비전 노트 참조).
...
머지 후 §7.3 롤아웃. 잡 B(§5.1.3) LIVE 전환은 법무 게이트(§10.1-L3·L5·L8) 뒤 별도 PR. **대회 기록 이름 표시(§5.2, P-9)는 사용자 결정으로 이미 확정되어 이 법무 게이트 대상이 아니다** — 구현 PR은 §5.2의 프론트·백엔드 소관 항목(토글 UI, 조인 로직) 완성 시점에 별도로 진행한다.
...
| ~~동의서 개정이 소급 적용되지 않는 기간의 노출 지속~~ — **2026-08-18 정책 변경으로 소멸**: 닉네임 기본 표시로 전환해 "동의 없는 실명 노출" 자체가 없어졌다(§5.2, §10.1-L1 해소) | (해당 없음) |
...
| ID | 항목 | 취지 |
|---|---|---|
| ~~L1~~ | ~~실명 공개 게시가 현행 동의 목적 범위를 벗어난 목적 외 이용에 해당하는지, 동의서 개정 시 소급(기존 참가자·종료 대회) 처리 방법~~ — **해소됨 (2026-08-18, §5.2 P-9)**. 이 질문은 "실명이 동의 근거 없이 강제 공개된다"는 전제 위에서만 성립했다. 새 정책은 기본 표시를 닉네임으로 바꾸고 실명 노출을 데이터 주체 본인의 상시 철회 가능한 개별 토글로만 발생시키므로, 목적 외 이용·소급 동의라는 문제가 성립할 전제(동의 없는 강제 공개) 자체가 없어졌다 — 토글은 등록 시점 동의 버전을 참조하지 않고 표시 시점 현재 값을 조인하므로 "구 버전에 동의한 과거 참가자"라는 소급 대상 구분도 발생하지 않는다. **남는 항목**: 토글을 켤 때 "이 설정을 켜면 참가한 대회의 공개 기록에 실명이 노출된다"는 결과를 이용자가 명확히 알 수 있는 고지 문구·UI가 필요하다는 요건은 남지만, 이는 외부 법무 자문이 필요한 목적 제한·동의 원칙 판단이 아니라 구현 단계의 카피·UX 품질 문제로 판단해 법무 검토 항목에서 제외한다. | (해소 — 아래 사유 참조) |
...
| ID | 질문 | 선택지 |
|---|---|---|
| ~~Q1~~ | ~~실명 공개 정합 방향~~ | **결정 완료 — §4 P-9로 이관 (2026-08-18)**. 닉네임 기본 표시 + 프로필 실명 표시 토글로 확정, 구 A(동의서 개정)/B(게이팅 스위치)/C(현상 유지) 선택지는 폐기 — §5.2, §10.1-L1(해소됨) |
| ~~Q2~~ | ~~탈퇴 유예기간~~ | **결정 완료 — 2026-09-26 사용자 결정: 30일. §4 P-10으로 이관**. 개인정보처리방침 v1.4(7절)와 탈퇴 화면에 반영됐다. L2 법무 확인은 여전히 남아 있다 |
