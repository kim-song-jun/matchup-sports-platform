...
| 4 | 종료(거절/철회/만료)된 방 | ~~active 유지 + 입력 잠금~~ → **2026-09-03 개정(사용자 A안 선택)**: 거절·철회 시 응답 트랜잭션에서, 만료 시 lazy-flip 경로에서 방을 `archived` 로 바꾼다. 기본 목록에서 숨고, 팀컨택 필터의 "종료된 컨택 보기" 토글로 조회한다. 딥링크·상세·메시지 조회는 계속 열리고(읽기 전용) 입력은 잠긴다 |
...
- ~~종료된 컨택 방 자동 보관/정리 cron~~ → 2026-09-03 개정: cron 없이 응답·lazy-flip 경로 + 채팅 목록 조회 전 sweep 으로 보관(결정 4). 기존 데이터는 `20260903000000_v1_team_contact_rooms_archive_ended` 가 1회 정리
