...
test.describe('[owner] 팀 관리 플로우', () => {
  ...
  test('/my/teams/[id] — owner는 멤버 관리 운영 메뉴를 볼 수 있다', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/my\/teams\/[a-f0-9-]{8,}/);
  ...
  test('/my/teams/[id]/members — 멤버 탭·가입신청 탭이 렌더된다', async ({ page }) => {
    ...
    // 멤버 관리 링크 클릭
    const memberMgmtLink = page.locator('a[href*="/members"]');
    await expect(memberMgmtLink.first()).toBeVisible();
    await memberMgmtLink.first().click();

    await expect(page).toHaveURL(/\/my\/teams\/[a-f0-9-]{8,}\/members/);
