...
- 열린 dev PR은 [#1187](https://github.com/kim-song-jun/matchup-sports-platform/pull/1187). 기존 head `ec8d1b4`의 CI 실패를 보완한 source1fad003d/test17a3e598 후보가30개 테스트와 Sol 독립 리뷰를 통과했다. 게스트 로그인 복귀·세션 만료·재시도·캐시된 비공개 정보와 보호 동작을 보완한다. 현재 PR head의 CI/Copilot → dev 머지 → Alpha Ego 결과는 재개 시 다시 확인한다. 로컬 후보 PASS를 배포 완료로 해석하지 않는다.
