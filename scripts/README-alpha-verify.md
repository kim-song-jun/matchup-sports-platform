...
| `verify_alpha_league_theme_badges.mjs` | 다크 모드(`localStorage 'tm-theme'`) 강제 전환 후 리그 목록·상세의 computed 색 실측. `LEAGUE_IDS` 로 대상 지정 | 불필요 |
...
| `verify_alpha_chat_realtime.mjs` | 채팅방 **실시간 수신** — 관찰자가 방을 연 채로 다른 계정이 REST 로 메시지를 보내고, **새로고침 없이** DOM 에 뜨는 시간을 잰다. 30초 staleTime 폴백과 구분하려고 대기 상한을 12초로 둔다(그 안에 뜨면 소켓 경로가 산 것). WebSocket 프레임 수신 여부도 함께 기록 | 같은 방 참가자 2계정(`OBSERVER_TOKEN`/`SENDER_TOKEN`) + `ROOM_ID` |
| `verify_alpha_radius_tokens.mjs` | radius 토큰이 **런타임에 해석되는지** — `:root` 의 `--radius-*` 8개 실측값 대조 + 인라인 `var()` 를 쓰는 요소의 computed borderRadius 가 0px 으로 무너지지 않았는지. `@theme` 가 방출되지 않으면 화면 전체 모서리가 각지는데 유닛 테스트도 정적 대조도 이걸 못 잡는다. 3폭 캡처 + 가로 오버플로·4xx/5xx 동반 기록. `TARGETS` 로 대상 경로 지정 | 선수 |
