...
const REVIEWED_NON_ADDITIVE = [
  {
    ...
    statement: "DO $$ DECLARE orphan_fixtures integer; orphan_promotions integer; league_count integer; matched_count integer; BEGIN SELECT count(*) INTO orphan_fixtures FROM v1_team_matches m WHERE m.league_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM v1_tournaments t WHERE t.id = m.league_id); SELECT count(*) INTO orphan_promotions FROM v1_league_promotions p WHERE NOT EXISTS (SELECT 1 FROM v1_tournaments t WHERE t.id = p.from_league_id); SELECT count(*) INTO league_count FROM v1_leagues; SELECT count(*) INTO matched_count FROM v1_leagues l JOIN v1_tournaments t ON t.id = l.id AND t.kind = 'regular_league'; RAISE NOTICE 'task164-be5-drop: leagues=% matched=% orphan_fixtures=% orphan_promotions=%', league_count, matched_count, orphan_fixtures, orphan_promotions; IF orphan_fixtures > 0 OR orphan_promotions > 0 THEN RAISE EXCEPTION 'TASK164_BE5_ORPHAN: \ud1b5\ud569 \ucd95\uc5d0 \ub300\uc751 \ud589\uc774 \uc5c6\ub294 \ucc38\uc870\uac00 \uc788\ub2e4 (\ub300\uc9c4 % \u00b7 \uc2b9\uac15 %) \u2014 \uc7ac\ud0c0\uae43\ud558\uba74 \uadf8 \ud589\ub4e4\uc774 \ub04a\uae34\ub2e4. \uac70\uc6b8 \ubc31\ud544\uc744 \uba3c\uc800 \ud655\uc778\ud558\ub77c.', orphan_fixtures, orphan_promotions; END IF; END $$",
    reason:
      "This is the contract half of Task 164 BE-5, which moves leagues onto the unified competition axis (V1Tournament kind='regular_league' + V1TournamentRegistration). The rewiring release (#1005) moved every READ; the commits before this one in this same release moved the dual-write writes and the alpha QA seed. No code reads or writes v1_leagues / v1_league_teams any more -- the surface gate counts v1League writes at 0 against a baseline of 0, and that gate is bidirectional so the count cannot silently grow back. The mirror row has always carried the SAME id as its league (leagueMirrorCreateData sets id: league.id, and the D8 backfill did the same), which is why the FK retarget needs no data backfill: the column values already point at rows that exist in v1_tournaments. That assumption is not taken on faith -- the first statement in this migration checks it and aborts the whole transaction if it does not hold (measured locally: with one orphan fixture the migration fails with TASK164_BE5_ORPHAN, psql rc=3 / prisma rc=1, and every table, enum and FK is left exactly as it was). The self-check itself: counts fixtures and promotions whose league id has no row in v1_tournaments, RAISE NOTICEs the totals, and RAISE EXCEPTIONs when any is non-zero. Read-only -- it writes nothing. Without it, a violated assumption would surface as an opaque FK violation halfway through, with no way to tell which rows were wrong. Reviewed 2026-09-03.",
  },
  {
    file: "apps/v1_api/prisma/migrations/20260903180000_v1_drop_league_tables/migration.sql",
    statement: "ALTER TABLE \"v1_team_matches\" DROP CONSTRAINT IF EXISTS \"v1_team_matches_league_fk\"",
    reason:
      "This is the contract half of Task 164 BE-5, which moves leagues onto the unified competition axis (V1Tournament kind='regular_league' + V1TournamentRegistration). The rewiring release (#1005) moved every READ; the commits before this one in this same release moved the dual-write writes and the alpha QA seed. No code reads or writes v1_leagues / v1_league_teams any more -- the surface gate counts v1League writes at 0 against a baseline of 0, and that gate is bidirectional so the count cannot silently grow back. The mirror row has always carried the SAME id as its league (leagueMirrorCreateData sets id: league.id, and the D8 backfill did the same), which is why the FK retarget needs no data backfill: the column values already point at rows that exist in v1_tournaments. That assumption is not taken on faith -- the first statement in this migration checks it and aborts the whole transaction if it does not hold (measured locally: with one orphan fixture the migration fails with TASK164_BE5_ORPHAN, psql rc=3 / prisma rc=1, and every table, enum and FK is left exactly as it was). Drops the old FK on v1_team_matches.league_id so it can be recreated against v1_tournaments in the next statement. Removing a constraint only RELAXES the schema for the instant between the two statements, and both run inside one transaction, so no concurrent writer can observe the gap. Reviewed 2026-09-03.",
  },
  {
    file: "apps/v1_api/prisma/migrations/20260903180000_v1_drop_league_tables/migration.sql",
    statement: "ALTER TABLE \"v1_team_matches\" ADD CONSTRAINT \"v1_team_matches_league_fk\" FOREIGN KEY (\"league_id\") REFERENCES \"v1_tournaments\"(\"id\") ON DELETE SET NULL ON UPDATE CASCADE",
    reason:
      "This is the contract half of Task 164 BE-5, which moves leagues onto the unified competition axis (V1Tournament kind='regular_league' + V1TournamentRegistration). The rewiring release (#1005) moved every READ; the commits before this one in this same release moved the dual-write writes and the alpha QA seed. No code reads or writes v1_leagues / v1_league_teams any more -- the surface gate counts v1League writes at 0 against a baseline of 0, and that gate is bidirectional so the count cannot silently grow back. The mirror row has always carried the SAME id as its league (leagueMirrorCreateData sets id: league.id, and the D8 backfill did the same), which is why the FK retarget needs no data backfill: the column values already point at rows that exist in v1_tournaments. That assumption is not taken on faith -- the first statement in this migration checks it and aborts the whole transaction if it does not hold (measured locally: with one orphan fixture the migration fails with TASK164_BE5_ORPHAN, psql rc=3 / prisma rc=1, and every table, enum and FK is left exactly as it was). Recreates that FK against v1_tournaments with the same ON DELETE SET NULL -- a league disappearing must not take its fixtures with it. The column values are untouched. Reviewed 2026-09-03.",
  },
  {
    file: "apps/v1_api/prisma/migrations/20260903180000_v1_drop_league_tables/migration.sql",
    statement: "ALTER TABLE \"v1_league_promotions\" DROP CONSTRAINT IF EXISTS \"v1_league_promotions_league_fk\"",
    reason:
      "This is the contract half of Task 164 BE-5, which moves leagues onto the unified competition axis (V1Tournament kind='regular_league' + V1TournamentRegistration). The rewiring release (#1005) moved every READ; the commits before this one in this same release moved the dual-write writes and the alpha QA seed. No code reads or writes v1_leagues / v1_league_teams any more -- the surface gate counts v1League writes at 0 against a baseline of 0, and that gate is bidirectional so the count cannot silently grow back. The mirror row has always carried the SAME id as its league (leagueMirrorCreateData sets id: league.id, and the D8 backfill did the same), which is why the FK retarget needs no data backfill: the column values already point at rows that exist in v1_tournaments. That assumption is not taken on faith -- the first statement in this migration checks it and aborts the whole transaction if it does not hold (measured locally: with one orphan fixture the migration fails with TASK164_BE5_ORPHAN, psql rc=3 / prisma rc=1, and every table, enum and FK is left exactly as it was). Drops the old FK on v1_league_promotions.from_league_id, same reasoning as the fixture FK above. Reviewed 2026-09-03.",
  },
  {
    file: "apps/v1_api/prisma/migrations/20260903180000_v1_drop_league_tables/migration.sql",
    statement: "ALTER TABLE \"v1_league_promotions\" ADD CONSTRAINT \"v1_league_promotions_league_fk\" FOREIGN KEY (\"from_league_id\") REFERENCES \"v1_tournaments\"(\"id\") ON DELETE CASCADE ON UPDATE CASCADE",
    reason:
      "This is the contract half of Task 164 BE-5, which moves leagues onto the unified competition axis (V1Tournament kind='regular_league' + V1TournamentRegistration). The rewiring release (#1005) moved every READ; the commits before this one in this same release moved the dual-write writes and the alpha QA seed. No code reads or writes v1_leagues / v1_league_teams any more -- the surface gate counts v1League writes at 0 against a baseline of 0, and that gate is bidirectional so the count cannot silently grow back. The mirror row has always carried the SAME id as its league (leagueMirrorCreateData sets id: league.id, and the D8 backfill did the same), which is why the FK retarget needs no data backfill: the column values already point at rows that exist in v1_tournaments. That assumption is not taken on faith -- the first statement in this migration checks it and aborts the whole transaction if it does not hold (measured locally: with one orphan fixture the migration fails with TASK164_BE5_ORPHAN, psql rc=3 / prisma rc=1, and every table, enum and FK is left exactly as it was). Recreates it against v1_tournaments with the same ON DELETE CASCADE -- a promotion record is meaningless without the league it was decided in. Reviewed 2026-09-03.",
  },
  {
    file: "apps/v1_api/prisma/migrations/20260903180000_v1_drop_league_tables/migration.sql",
    statement: "DROP TABLE IF EXISTS \"v1_league_teams\"",
    reason:
      "This is the contract half of Task 164 BE-5, which moves leagues onto the unified competition axis (V1Tournament kind='regular_league' + V1TournamentRegistration). The rewiring release (#1005) moved every READ; the commits before this one in this same release moved the dual-write writes and the alpha QA seed. No code reads or writes v1_leagues / v1_league_teams any more -- the surface gate counts v1League writes at 0 against a baseline of 0, and that gate is bidirectional so the count cannot silently grow back. The mirror row has always carried the SAME id as its league (leagueMirrorCreateData sets id: league.id, and the D8 backfill did the same), which is why the FK retarget needs no data backfill: the column values already point at rows that exist in v1_tournaments. That assumption is not taken on faith -- the first statement in this migration checks it and aborts the whole transaction if it does not hold (measured locally: with one orphan fixture the migration fails with TASK164_BE5_ORPHAN, psql rc=3 / prisma rc=1, and every table, enum and FK is left exactly as it was). Drops v1_league_teams. Its contents were mirrored into V1TournamentRegistration rows with status='confirmed' (that pairing was established by the D8 backfill and every write path since has maintained it), and all roster reads now go through those registrations. Reviewed 2026-09-03.",
  },
  {
    file: "apps/v1_api/prisma/migrations/20260903180000_v1_drop_league_tables/migration.sql",
    statement: "DROP TABLE IF EXISTS \"v1_leagues\"",
    reason:
      "This is the contract half of Task 164 BE-5, which moves leagues onto the unified competition axis (V1Tournament kind='regular_league' + V1TournamentRegistration). The rewiring release (#1005) moved every READ; the commits before this one in this same release moved the dual-write writes and the alpha QA seed. No code reads or writes v1_leagues / v1_league_teams any more -- the surface gate counts v1League writes at 0 against a baseline of 0, and that gate is bidirectional so the count cannot silently grow back. The mirror row has always carried the SAME id as its league (leagueMirrorCreateData sets id: league.id, and the D8 backfill did the same), which is why the FK retarget needs no data backfill: the column values already point at rows that exist in v1_tournaments. That assumption is not taken on faith -- the first statement in this migration checks it and aborts the whole transaction if it does not hold (measured locally: with one orphan fixture the migration fails with TASK164_BE5_ORPHAN, psql rc=3 / prisma rc=1, and every table, enum and FK is left exactly as it was). Drops v1_leagues. Every league row has a same-id mirror in v1_tournaments (the self-check above proves it for this database at deploy time), and both FKs that pointed here were retargeted four statements earlier. Reviewed 2026-09-03.",
  },
  {
    file: "apps/v1_api/prisma/migrations/20260903180000_v1_drop_league_tables/migration.sql",
    statement: "DROP TYPE IF EXISTS \"V1LeagueState\"",
    reason:
      "This is the contract half of Task 164 BE-5, which moves leagues onto the unified competition axis (V1Tournament kind='regular_league' + V1TournamentRegistration). The rewiring release (#1005) moved every READ; the commits before this one in this same release moved the dual-write writes and the alpha QA seed. No code reads or writes v1_leagues / v1_league_teams any more -- the surface gate counts v1League writes at 0 against a baseline of 0, and that gate is bidirectional so the count cannot silently grow back. The mirror row has always carried the SAME id as its league (leagueMirrorCreateData sets id: league.id, and the D8 backfill did the same), which is why the FK retarget needs no data backfill: the column values already point at rows that exist in v1_tournaments. That assumption is not taken on faith -- the first statement in this migration checks it and aborts the whole transaction if it does not hold (measured locally: with one orphan fixture the migration fails with TASK164_BE5_ORPHAN, psql rc=3 / prisma rc=1, and every table, enum and FK is left exactly as it was). Drops the V1LeagueState enum, which existed only for the column on the table dropped one statement earlier. The response vocabulary draft/active/completed survives, but as a hand-written union in league-state.ts -- storage is V1TournamentStatus, and LEAGUE_STATE_BY_STATUS maps between them. Reviewed 2026-09-03.",
  },
