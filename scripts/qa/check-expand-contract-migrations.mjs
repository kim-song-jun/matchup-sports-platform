...
const REVIEWED_NON_ADDITIVE = [
  {
    ...
    reason:
      "Contract half of Task 164 BE-5: leagues moved onto the unified competition axis. Drops the V1LeagueState enum, which existed only for the column on the table dropped one statement earlier. The response vocabulary draft/active/completed survives as a hand-written union in league-state.ts; storage is V1TournamentStatus and LEAGUE_STATE_BY_STATUS maps between them. No code imports the enum from @prisma/client any more (the two source files that did were moved to the hand union in this release -- local tsc did not catch them because the shared generated client still carries the old schema). Alpha measurement 2026-09-03: leagues 89 / same-id mirrors 89 / orphan fixtures 0 / orphan promotions 0, and the migration re-checks that itself, aborting the whole transaction on any orphan. Reviewed 2026-09-04.",
  },
  {
    file: "apps/v1_api/prisma/migrations/20260903180000_v1_drop_league_tables/migration.sql",
    ...
    reason:
      "Contract half of Task 164 BE-5: leagues moved onto the unified competition axis (V1Tournament kind='regular_league' + V1TournamentRegistration). The rewiring release moved every READ; earlier commits in this release moved the dual-write writes and the alpha QA seed. No code touches v1_leagues / v1_league_teams any more (surface gate: v1League writes 0 against baseline 0, and that gate is bidirectional). Mirror rows carry the SAME id as their league, so the FK retarget needs no data backfill -- and the migration proves that for the database it is running against instead of assuming it. Alpha measurement 2026-09-03 (read-only): leagues 89 / same-id mirrors 89 / orphan fixtures 0 / orphan promotions 0 / league_teams 215 / confirmed registrations 211 -- the 4-row gap is handled by the backfill step below. Every statement is guarded so a hand re-run is a no-op (measured: run 1 inserts 4 and drops, run 2 exits 0 changing nothing). The orphan self-check: counts fixtures and promotions whose league id has no row in v1_tournaments, RAISE NOTICEs the totals, and RAISE EXCEPTIONs when any is non-zero so the whole transaction aborts. Read-only. Without it a violated assumption surfaces as an opaque FK violation halfway through. Reviewed 2026-09-03.",
  },
  {
    file: "apps/v1_api/prisma/migrations/20260903180000_v1_drop_league_tables/migration.sql",
