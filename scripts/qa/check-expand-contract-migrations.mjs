...
const REVIEWED_NON_ADDITIVE = [
  ...
  },
  {
    ...
    reason: "Privacy v1.4 review 2026-09-26: inserts only the new immutable privacy v1.4 row (30-day withdrawal grace period in clause 7) with ON CONFLICT DO NOTHING. Existing legal history is retained; requires_reconsent=false preserves current sessions and old app instances keep rendering the managed document they fetch.",
  },
  {
    file: "apps/v1_api/prisma/migrations/20260926091000_privacy_policy_v14_withdrawal_grace/migration.sql",
    statement: "DO $privacy_v14_guard$ BEGIN\n IF NOT EXISTS (SELECT 1 FROM \"v1_managed_terms_documents\" WHERE \"id\" = 'a1150000-0000-4000-8000-000000000004' AND \"policy_id\" = 'a1100000-0000-4000-8000-000000000004' AND \"version\" = 'v1.4' AND \"content_hash\" = '404181b22a3de942ccea2437bb9d45b9998b84bdc07ece59fdaea86028cfd4d7' AND md5(\"content\") = '40b9337398018be409e3106f9080ea73' AND \"status\" = 'published' AND \"supersedes_document_id\" = 'a1140000-0000-4000-8000-000000000004') THEN RAISE EXCEPTION 'Privacy v1.4 canonical document conflict' USING ERRCODE = '23514'; END IF;\nEND $privacy_v14_guard$",
    reason: "Privacy v1.4 review 2026-09-26: read-only canonical-document guard verifying the exact id/version/body digest and predecessor (v1.3). It raises on a conflicting pre-existing row rather than rewriting legal history; no data or schema change.",
  },
  {
