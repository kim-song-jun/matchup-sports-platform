...
const taskNineR3OverridePath =
  '.omo/start-work/host-pressure-override-task-9-plan-r3.json';
const taskNineR3OverrideSHA =
  '026b453a615973b71ace9734f21f895df58397c8a3fb995f9d259c47cd6feb7c';
const taskNineR3PlanSHA =
  '191e5fd616b96d31ab84fd83777489859640e6c1611b91b2cf09174a3cd2da0b';
const taskNineR3OwnedPaths = [
...
const taskNineR10OwnedPaths = [
  ...taskNineR9OwnedPaths,
...
function hostPressureOverrideFunction({
  ...
  ownedPaths = taskNineR6OwnedPaths,
} = {}) {
  const source = readFileSync(
    join(repoRoot, 'scripts/qa/run-v1-task-verification.mjs'),
    'utf8',
  ...
  return Function(
    ...
    'TASK_NINE_HOST_PRESSURE_OVERRIDE_R5_PATH',
    ...
    'TASK_NINE_R7_PLAN_SHA256',
    'TASK_NINE_R7_AUTHORIZATION_TEXT',
    'TASK_NINE_R7_CONSTRAINTS',
    'OVERRIDE_SHA256',
    ...
    taskNineR6OverridePath,
    taskNineR6OverrideSHA,
    taskNineR6AuthorizationText,
    taskNineR6Constraints,
    taskNineR6OverridePath,
    taskNineR6OverrideSHA,
    taskNineR6AuthorizationText,
    taskNineR6Constraints,
    taskNineR7OverridePath,
    taskNineR7OverrideSHA,
    taskNineR7PlanSHA,
    taskNineR6AuthorizationText,
    taskNineR6Constraints,
    ...
    () => ({ ownership: [{ todo: 9, outputs: ownedPaths }] }),
...
test('Task 9 R11 descriptor keeps R5-R10 receipts immutable while appending only jobs integration discovery', () => {
  ...
  assert.ok(ledgerMatch);
  const ledgerOutputs = JSON.parse(ledgerMatch[1]).ownership.find(
    (entry) => entry.todo === 9,
  ).outputs;
  const planRow = readFileSync(
    join(repoRoot, '.omo/plans/teameet-team-tournament-operations-v1.md'),
    'utf8',
  ).split('\n').find(
    (line) =>
      line.startsWith('| 9 |') &&
      line.includes('apps/v1_api/prisma/schema.prisma'),
  );
  assert.ok(planRow);
  ...
  try {
    ...
    assert.equal(ledgerOutputs.at(25), 'apps/v1_api/src/jobs/v1-game-operations-worker.service.spec.ts');
    assert.equal(ledgerOutputs.at(26), 'apps/v1_api/test/jobs/v1-game-operations-worker.integration-spec.ts');
    ...
    assert.deepEqual(canonicalReceipt.ownedPaths, taskNineR6OwnedPaths);
    assert.equal(canonicalReceipt.planSHA256, taskNineR6PlanSHA);
    assert.equal(canonicalReceipt.authorizationText, taskNineR6AuthorizationText);
    assert.deepEqual(canonicalReceipt.constraints, taskNineR6Constraints);
    assert.equal(sha256(readFileSync(resolve(repoRoot, taskNineR6OverridePath))), taskNineR6OverrideSHA);
    assert.equal(productionSHA, taskNineR6OverrideSHA);
    const verifyDescriptor = secureImmutableDescriptorFunction();
    assert.doesNotThrow(() => verifyDescriptor(
      resolve(repoRoot, taskNineR6OverridePath),
      taskNineR6OverrideSHA,
      'HOST_PRESSURE_OVERRIDE_INVALID',
      75,
      true,
    ));
    const hardLinkDirectory = mkdtempSync('/private/tmp/teameet-task9-r5-hardlink-');
    const hardLinkPath = join(hardLinkDirectory, 'receipt.json');
    try {
      linkSync(resolve(repoRoot, taskNineR6OverridePath), hardLinkPath);
      assert.equal(lstatSync(resolve(repoRoot, taskNineR6OverridePath)).nlink, 2);
      assert.throws(
        () => verifyDescriptor(
          resolve(repoRoot, taskNineR6OverridePath),
          taskNineR6OverrideSHA,
        ...
        (error) => error.code === 'HOST_PRESSURE_OVERRIDE_INVALID',
        'the exact Task 9 R5 receipt must reject a hard-linked inode',
      );
    } finally {
      rmSync(hardLinkDirectory, { recursive: true, force: true });
    }
    const symlinkDirectory = mkdtempSync('/private/tmp/teameet-task9-r5-symlink-');
    try {
      const symlinkPath = join(symlinkDirectory, 'receipt.json');
      symlinkSync(resolve(repoRoot, taskNineR6OverridePath), symlinkPath);
      assert.throws(
        () => verifyDescriptor(symlinkPath, taskNineR6OverrideSHA, 'HOST_PRESSURE_OVERRIDE_INVALID', 75, true),
        (error) => error.code === 'HOST_PRESSURE_OVERRIDE_INVALID',
      'the exact Task 9 R5 receipt must reject a symlink',
      );
    } finally {
      rmSync(symlinkDirectory, { recursive: true, force: true });
    }
    process.env.V1_HOST_PRESSURE_OVERRIDE_RECEIPT = resolve(repoRoot, taskNineR6OverridePath);
    process.env.V1_VERIFICATION_SESSION_ID = verificationSessionId;
    const verifyOverride = hostPressureOverrideFunction({
      receipt: canonicalReceipt,
      sha256: taskNineR6OverrideSHA,
    });
    const accepted = verifyOverride(
      repoRoot,
      { rawSHA: 'different-raw-plan-sha', normalizedSHA: taskNineR6PlanSHA },
      9,
      ['NODE_MCP_PROCESS_PROLIFERATION'],
    );
    ...
    const rejected = (name, {
      path = taskNineR6OverridePath,
      sha256: receiptSHA = taskNineR6OverrideSHA,
      session = verificationSessionId,
      planSHA = taskNineR6PlanSHA,
      task = 9,
      receipt = canonicalReceipt,
    }) => {
    ...
    rejected('wrong path', { path: '.omo/start-work/arbitrary-receipt.json' });
    ...
    rejected('missing the new migration path', {
      receipt: { ...canonicalReceipt, ownedPaths: taskNineR6OwnedPaths.filter((path) => path !== 'apps/v1_api/prisma/migrations/20260802000300_v1_result_escalation_lifecycle') },
    });
    rejected('missing the new game-operations path', {
      receipt: { ...canonicalReceipt, ownedPaths: taskNineR6OwnedPaths.filter((path) => path !== 'apps/v1_api/src/game-operations') },
    });
    rejected('missing transferred global contract path', {
      receipt: { ...canonicalReceipt, ownedPaths: taskNineR6OwnedPaths.filter((path) => path !== 'docs/api/global-contract.md') },
    });
    rejected('missing transferred auth contract path', {
      receipt: { ...canonicalReceipt, ownedPaths: taskNineR6OwnedPaths.filter((path) => path !== 'docs/api/domains/tournament-operations-auth.md') },
    });
    rejected('reordered transfer path', {
      receipt: { ...canonicalReceipt, ownedPaths: [...taskNineR6OwnedPaths].reverse() },
    });
    rejected('extra unrelated path', {
      receipt: { ...canonicalReceipt, ownedPaths: [...taskNineR6OwnedPaths, 'apps/v1_api/src/unrelated'] },
    });
  } finally {
    if (previousReceipt === undefined) {
      delete process.env.V1_HOST_PRESSURE_OVERRIDE_RECEIPT;
...
test('Task 9 R5 remains immutable 19-path evidence and is rejected as stale after the R6 transfer', () => {
  ...
  assert.deepEqual(r5.ownedPaths, taskNineR6OwnedPaths.slice(0, 19));
  ...
  const rejected = runNode([
    ...
    env: {
      ...process.env,
      OMO_SELECTED_PLAN_SHA: taskNineR6PlanSHA,
...
test('Task 9 R4 remains immutable 18-path evidence and is rejected as stale after the R6 transfer', () => {
  ...
  assert.equal(r4.ownedPaths.length, 18);
  assert.equal(r4.ownedPaths.includes('docs/api/global-contract.md'), false);
  assert.deepEqual(taskNineR6OwnedPaths.slice(0, 18), r4.ownedPaths);
  const rejected = runNode([
    'scripts/qa/run-v1-task-verification.mjs',
    '--pressure-receipt-only',
...
test('Task 9 R3 remains immutable evidence and is rejected after the R6 ownership rebind', () => {
  ...
  try {
    ...
    assert.throws(
      () => verifyOverride(
        repoRoot,
        { rawSHA: taskNineR3PlanSHA, normalizedSHA: taskNineR6PlanSHA },
        9,
...
test('Task 9 R6 native receipt channel rejects stale R6 binding under the R7 plan and misleading-success payloads', () => {
  const command = [
    'scripts/qa/run-v1-task-verification.mjs',
    '--pressure-receipt-only',
    taskNineR6OverridePath,
  ...
  const environment = {
    ...process.env,
    OMO_SELECTED_PLAN_SHA: taskNineR7PlanSHA,
    V1_HOST_PRESSURE_OVERRIDE_RECEIPT: taskNineR6OverridePath,
    V1_VERIFICATION_SESSION_ID: verificationSessionId,
  };
  const accepted = runNode(command, { env: environment });
  assert.equal(accepted.status, 75, accepted.stdout);
  assert.match(accepted.stderr, /Override receipt fields do not bind this plan/);
...
test('Task 9 R7 native receipt channel accepts the exact 21-path authority and normalized plan', () => {
  const r7Receipt = JSON.parse(readFileSync(resolve(repoRoot, taskNineR7OverridePath), 'utf8'));
  assert.deepEqual(r7Receipt.ownedPaths, taskNineR7OwnedPaths);
  const command = [
    'scripts/qa/run-v1-task-verification.mjs',
    '--pressure-receipt-only',
    taskNineR7OverridePath,
  ...
  const environment = {
    ...process.env,
    OMO_SELECTED_PLAN_SHA: taskNineR7PlanSHA,
    V1_HOST_PRESSURE_OVERRIDE_RECEIPT: taskNineR7OverridePath,
    V1_VERIFICATION_SESSION_ID: verificationSessionId,
  };
  const accepted = runNode(command, { env: environment });
  assert.equal(accepted.status, 0, accepted.stderr);
  assert.match(accepted.stdout, /TASK9_PRESSURE_RECEIPT_R7_PASS/);
  assert.match(accepted.stdout, /normalized_plan_sha=37aee528db382ebf72442333676f5d50b65387f61a647a54a568bdfa8086829b/);
  assert.match(accepted.stdout, /owned_paths=21/);

  const rejected = (name, {
    path = taskNineR7OverridePath,
    sha = taskNineR7OverrideSHA,
    plan = taskNineR7PlanSHA,
    session = verificationSessionId,
    receiptEnv = path,
    expectedStatus = 75,
  } = {}) => {
    const result = runNode([
      ...
      env: {
        ...process.env,
        OMO_SELECTED_PLAN_SHA: plan,
        V1_HOST_PRESSURE_OVERRIDE_RECEIPT: receiptEnv,
        V1_VERIFICATION_SESSION_ID: session,
      },
    });
    assert.equal(result.status, expectedStatus, `${name}: ${result.stdout}\n${result.stderr}`);
    return result;
  };
  rejected('wrong R7 SHA', { sha: '0'.repeat(64) });
  rejected('wrong R7 path', {
    path: '.omo/start-work/host-pressure-override-task-9-plan-r6.json',
  });
  rejected('wrong R7 normalized plan', { plan: '108a6cf1e8a5bbacef86928d28be8f7867525460e0af91d5882d72723ca72b84', expectedStatus: 65 });
  rejected('wrong R7 session', { session: 'codex:wrong-session' });
  const misleading = runNode([
    ...command,
    '--',
    'node',
  ...
  assert.equal(misleading.status, 75, misleading.stdout);
  assert.match(misleading.stderr, /Descriptor-only pressure receipt validation accepts only the exact path\/SHA pair/);
});
...
test('Task 9 R2 host pressure receipt is rejected after the R3 ownership rebind', () => {
  ...
  try {
    ...
    assert.throws(
      () => verifyOverride(
        repoRoot,
        { rawSHA: taskNineR2PlanSHA, normalizedSHA: taskNineR3PlanSHA },
        9,
        ['NODE_MCP_PROCESS_PROLIFERATION'],
      ),
      (error) => error.code === 'HOST_PRESSURE_OVERRIDE_INVALID',
      'V9 must reject the R2 descriptor before any fallback validation',
    );
    const verifyDescriptor = secureImmutableDescriptorFunction();
    assert.doesNotThrow(() => verifyDescriptor(
