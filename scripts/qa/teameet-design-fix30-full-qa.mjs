// Headless QA for Teameet Design prototype fix30.
...
const ARTIFACT_PATH = path.join(OUT_DIR, 'teameet-design-fix30-full-qa.json');
...
async function main() {
  ...
  for (const target of SCREENSHOT_TARGETS) {
    ...
    if (!node) continue;
    const dest = path.join(OUT_DIR, `teameet-design-fix30-${target.id}.png`);
