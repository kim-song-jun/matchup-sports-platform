...
export async function runFullResponsiveQa(config) {
  ...
  const authSessionCache = { sessions: new Map() };
  const results = [];
  ...
  for (const viewport of target.viewports) {
    ...
    try {
      ...
      const session = target.authMode === 'authenticated'
        ? await getDevLoginSession(authSessionCache, target.route)
        : null;
      ...
      if (consoleErrors.length > 0) {
        viewportFindings.push({
          ...
          check: 'console-error',
          viewport: viewport.name,
          message: consoleErrors.map((message) => message.text).join('; '),
        });
      }
...
async function getDevLoginSession(cache, route) {
  const email = emailForRoute(route);
  if (cache.sessions.has(email)) return cache.sessions.get(email);
  ...
  cache.sessions.set(email, session);
  return session;
}
