...
async function runRouteTarget(browser, baseUrl, target) {
  ...
  for (const viewport of target.viewports) {
    ...
    try {
      ...
      const session = target.authMode === 'authenticated' ? await devLoginSession(target.route) : null;
      ...
      const viewportFindings = findWalkthroughFindings({ target, viewport, metrics, functionProbeResults });
      viewportFindings.push(...browserSignalFindings({
        viewport,
        pageErrors,
        consoleMessages,
        failedRequests,
...
function browserSignalFindings({ viewport, pageErrors, consoleMessages, failedRequests, responseErrors }) {
  const findings = [];
...
async function devLoginSession(route) {
  ...
  const session = normalizeDevLoginSession(JSON.parse(text), email);
